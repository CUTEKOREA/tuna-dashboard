      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        {[
          ...
          <div key={i} style={{ background: 'rgba(0, 0, 0, 0.2)', padding: '1.25rem', borderRadius: '8px', border: `1px solid ${k.color}33`, borderTop: `3px solid ${k.color}`, display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <div style={{ fontSize: '0.8rem', color: 'var(--w-slate-400)', display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 600 }}>{k.icon} {k.label}</div>
            <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--w-slate-50)', letterSpacing: '0.5px' }}>{k.value}</div>
