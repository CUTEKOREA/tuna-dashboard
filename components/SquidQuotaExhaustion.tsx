...
export default function SquidQuotaExhaustion() {
  ...
  return (
    <div className={styles.glassCard} ref={containerRef}>
      ...
      <TakeawayBox source="포클랜드 어업국 / 원양 선단" situation="글로벌 ESG 규제 압박으로 남반구 핵심 조업국들의 개별할당제(ITQ) 쿼터가 5~6월경 조기 고갈(Depletion)되며 어업이 강제 셧다운(Shutdown) 조치되는 빈도가 급증했습니다. 직후 글로벌 시세가 수직 폭등하는 공급 공백 국면이 상시화되었습니다." actionPlan="[Preemptive Global Buyout] 타국 선단의 쿼터 소진율 텔레메트리를 실시간 트래킹 하십시오. 특정 메이저 어장의 ITQ 소진율이 80% 임계치를 돌파하는 즉시, 1개월 내 발생할 시세 폭등을 겨냥해 남미/대만 등 제3국 선단들이 보유한 잔여 해상 선적 물량을 선도가(Premium)에 전량 싹쓸이(Buyout)하는 글로벌 알박기 전략을 지시합니다." />
    </div>
  );
}
