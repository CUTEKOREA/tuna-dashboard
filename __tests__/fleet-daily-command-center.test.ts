...
describe('FleetCommandCenter daily operations', () => {
  it('renders the latest daily report as the hero KPI source', () => {
    ...
    expect(markup).toContain('2026-09-17 보고 · 2026-09-16 조업 기준');
    // 9월 첫 보고라 일간과 월간 누계가 같은 295 MT다.
    expect(markup).toContain('data-kpi-value="715"');
    expect(markup).toContain('data-kpi-value="87207.8"');
  ...
  it('renders public deltas and fail-closed quality coverage without private schedules', () => {
    ...
    for (const value of [
      ...
      '+259 (MT)', 'SYNCED',
      '전체 보고 158건', '전기간 검산 632회', '완전 검산 632회', '미보고 포함 0회 / 0문서',
      ...
      '중복 선박 행 4건', '좌표 형식 이슈 6건', '연승 구역 미기재 19건',
      '최신 상세 행 검산 일치',
    ]) {
