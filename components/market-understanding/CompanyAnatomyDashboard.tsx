...
const SOLTUNA_SPEC: CommoditySpec = {
  ...
  secondaryKpis: [
    ...
    },
    {
      ...
      value: ST_H.합,
      decimals: 0,
      unit: `(SBD · 물품세 ${stNum(ST_H.LN590)}(${ST_H.기간}) + 관세 ${stNum(ST_H.LN591)}(서명일 ~ 2026-01-30). 공캔·뚜껑·라벨·포장재·실험 자재가 열거돼 있고 **두 명령 다 2026-01-30 에 끝난다.** NFD 도 선박 한 척에 SBD ${stNum(ST_H.LN411, 2)} 를 면제받았다(LN 411). **특별대우로 읽지 않는다** — 관보 한 호(2025-02-14)에만 100 % 면제가 ${ST_H.같은관보}건이다. 캔참치 가격통제도 이 회사를 겨냥하지 않는다: 1987년부터 ${ST_H.가격통제_품목수}개 생필품 가운데 ${ST_H.가격통제_순번}번이고 통제구역 ${ST_H.가격통제_구역수}곳에 **노로가 없다.** 분기보고·양도금지 부관은 LN 411 원문에서만 확인됐고 LN 590·591 에 붙는지는 그 지면이 정한다)`,
    },
    {
...
const RD_SPEC: CommoditySpec = {
  ...
  subtitle:
    ...
    '마당은 묻지 않고(경제동반자협정 원산지 의정서 Ⅱ 제6조 6항 (b)의 전면적 조달 예외) 탐블러는 묻는다(일반특혜관세의 어선 기국·등록·소유 요건). 그리고 그 문을 실제로 여닫는 것은 원산지 서류가 아니라 위생 승인이다 — 피지는 위생 미비로 같은 문을 한 번도 쓰지 못했고, 이 공장도 2008년에 같은 이유로 한 해 동안 유럽 시장에서 배제됐다. ' +
