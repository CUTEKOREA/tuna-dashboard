...
export const tooltipStyle: React.CSSProperties = { backgroundColor: 'var(--w-navy-900)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: 'var(--w-slate-50)' };
...
export const WidgetCard = ({ title, icon: Icon, term, desc, source, situation, actionPlan, telemetry, children }: any) => (
  <div className="ds-card-insight" style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0, overflow: 'hidden' }}>
      ...
      <h3 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--w-slate-50)', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</h3>
      {term && <div style={{ flexShrink: 0 }}><TermTooltip term={term} description={desc} /></div>}
      {telemetry?.status && (
        <span style={{ flexShrink: 0, marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.04em', color: 'var(--w-slate-400)', background: 'rgba(140,170,255,0.12)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', padding: '2px 6px' }} title={telemetry.syncDate ? `동기화: ${telemetry.syncDate}` : undefined}>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: telemetry.status === 'LIVE' ? 'var(--color-success)' : 'var(--w-slate-500)' }} />
          {telemetry.status}{telemetry.syncDate ? ` · ${telemetry.syncDate}` : ''}
        </span>
      )}
    </div>
    <div style={{ flex: 1, minHeight: '280px', position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(140,170,255,0.10)', borderRadius: '8px', background: 'transparent', padding: '16px' }}>
        {children}
      </div>
    </div>
