...
> - 처리: 원 기록·플래그는 그대로 두고(정정하지 않음), 개관 「원어 시세 추이」에서 의심 플래그 주는 값 없는 주처럼 선을 끊는다(`lib/bangkok-price-overview.ts`). 캡션에 사유 명시, 테스트로 2024-01-10 행이 null 임을 고정. PR #876에 포함.
...
> - **보고서가 자체 계산해 싣던 자급률 145.6% 는 공표값이 아니었다.** 정부가 표 4-11 로 이미 공표한다 — 2018~2023년 **151.1 / 85.7 / 76.6 / 112.2 / 117.4 / 144.1%**. 정부 계열은 가공품에 환산수율을 적용하고 이입·이월을 국립수산물품질관리원 냉동창고 조사(전국 240개, 분기 1회)에서 가져오며 국내소비·자급률은 KMI 추정이다. 보고서는 통관 원물 톤수만 썼다. 04절을 공표 계열로 교체하고 2024년 자급률은 내놓지 않기로 했다.
> - **`s1_official_self_sufficiency`** — 공식 자급률 6년. **2023년 수출 111,431t 이 국내소비 83,350t 의 1.34배**다. 2020→2023 수출 3배, 소비 감소. 자급률은 국산 조달력이 아니라 수출과 재고가 움직인다(재고 ±1만 t 이면 163.7% / 128.6%).
> - **`s1_self_sufficiency_species_gap`** — **자급률이 높은 해일수록 통계에서 빠진 어종이 크다.** 분자는 망치고등어를 빼는데 수출은 HS 0303540000 한 칸에 세 종이라 못 뺀다. 2018년 생산의 **34.3%**(74,403t)가 망치고등어였고 넣으면 151.1 → **128.5%**. 2023년도 26.3% 에 144.1 → 129.3%. 같은 조사가 아프리카 수출 급증을 「망치고등어 및 소형어 어획 비중이 높아지면서」로 설명한다.
> - **`s4_value_chain_domestic_vs_import`** — 표 4-22 수입 값 사슬(감천항→소비지도매)이 보고서에 통째로 빠져 있었다. **어부 몫이 노르웨이 쪽보다 크다** — 국산 어가수취 40.9% vs 수입 CFR 35.2%, 소매는 36.2% vs 35.0% 로 거의 같다. 갈리는 곳은 도매·중도매 층으로 수입 22.6% 가 국산 13.3% 의 1.7배다. 위젯 39 → **42**, 4축 **90.0 (A 37 / B 5)**.
> - **표 5-4 로 10개 품목 비교가 가능해졌다.** 고등어 생산자수취율 40.9% 는 가중평균 36.5% 보다 **높고** 소매 36.2% 는 평균 44.8% 보다 **낮다**. 넙치는 소매 64.1%, 명태는 생산자가 23.6% 뿐이다. **고등어는 유통이 특별히 나쁜 품목이 아니다.**
> - **각주 함정을 스킬에 박았다** (`~/.claude/skills/commodity-report/references/data-lanes-kr.md`). 이 조사는 10개 품목이 한 장에 있어 각주 문구가 반복된다 — 「조사 시기는 각 연도 11월 중순」은 **활오징어 표(표 4-52)** 각주이고 고등어는 「각 연도별 **12월**」이다. 전역 grep 이 품목 경계를 넘어 잡는다. 표 번호도 판마다 밀린다(2020년판 4-13 → 2024년판 4-11) — **표 번호가 아니라 표 제목으로 인용할 것.**
> - **스스로 물린 것 둘.** 「정부비축을 늘리면 자급률이 올라간다」는 비축분이 냉동창고 조사에 잡힌다는 근거가 원문에 없어 삭제했다. 규격 논지에는 반대 증거를 함께 실었다 — 같은 조사가 노르웨이산 수요 유지 이유를 「맛, 가격 등에서 큰 차이를 느끼지 않아」라고 적어 둘을 대체재로도 본다.
> - 보고서 40쪽(35→40), 대조 **647건** 미확인 0, 관문 6/6. 표 4-11·4-2·4-19·4-22·5-4 를 원문과 기계 대조해 **불일치 0건**.
> - 게이트: provenance 42/42, mackerel:test 4/4, tsc clean, Vitest 157 files·1,209 passed/2 skipped.
> - **발행 직전 자체 검토에서 셋을 물렸다.** ① 「자급률 상위 두 해 = 망치 비중 상위 두 해」는 **거짓**이었다(2023년은 망치 비중 3위, 2위는 2022년). 상위 **세** 해 집합은 같으나 순서가 달라 인과로 읽지 않기로 했다. ② 어가수취 40.9% 와 CFR 35.2% 는 담긴 범위가 다르다 — 앞은 어부 한 사람의 산지 위판가, 뒤는 노르웨이 어획~부산항 도착 전 과정이다. 「한국 어부 한 사람 몫이 노르웨이 전 과정보다 크다」로 고쳤다. ③ 도매층 배수 1.7배는 층 나누기에 따라 1.4배도 나온다(수입 경로에 감천→서울 도매업자 단계가 하나 더 있다). **배수를 뺐고**, 어느 묶음에서도 성립하는 「수입 사슬 최대 단일 칸은 소비지중도매인 17.1%p」만 남겼다.
> - **위젯 제목에도 같은 오류가 남아 있었다.** `s1_self_sufficiency_species_gap` 제목이 「자급률이 높은 해일수록 빠진 어종이 크다」로 순위 검정이 기각한 방향을 그대로 담고 있었다. 산식이 뒷받침하는 방향인 「망치고등어가 많은 해일수록 자급률이 부풀려진다」로 뒤집고, 순위(자급률 2018>2023>2022 / 망치 2018>2022>2023)를 docstring 에 박아 재발을 막았다. **본문을 고치면 위젯 제목·KPI·sit·strat 까지 같이 훑을 것** — 같은 주장이 다섯 군데에 흩어져 있다.
> - **다음 단계**: 사용자가 배포를 명시해 `mackerel/ledger-sync` → PR gate → main 병합 → Vercel READY → `/mackerel` 라이브 확인. 2021~2023년 실태조사 개별판은 미확보(게시판 목록이 AJAX 라 `fileTypeSeq` 를 못 찾는다) — 다만 2024년판 표 4-11 이 그 연도 자급률을 담아 시계열엔 지장 없다.
>
> 🐟 **2026-08-27 — 소스 전수검토 마감 + `/mackerel` 노르웨이 어가 위젯** [Claude]:
> - **아카이브 소스 원장 활성 79개를 원문까지 열어 전수 판정 완료.** 인용 52 → **64**, 미인용 15, 판정 누락 **0**. 1차에서 메타데이터만 보고 「보류」로 뒀던 12건이 원문을 여니 반영감이었다 — 판정은 원문을 열고 내려야 한다는 게 수치로 나왔다.
> - **보고서 관문6 통과.** 정본이 원장에 없는 소스를 인용하는지, 원장 활성 소스가 판정 없이 남았는지를 매 빌드 대조한다. 발행분 15절·35쪽·대조 391건·관문 6/6.
> - 원고 오류 둘 정정 — 03절 ABC 30.4만 t 의 일한 합산 단서 누락, 03절 「잡을 물고기가 없다」가 NIFS 원문(고등어류 어획은 1980년대 이후 증가, 연근해 감소 원인은 조업해역 축소·고령화)과 상충. 인용 금지 하나 — FRA SC17 구두 발언 「NPFC 공해 마사바 TAC 10만 t」은 CMM 어느 값과도 안 맞는다.
> - **`s1_norway_catch_value`** — 노르웨이 수산총국 F06001 공식 어획통계 2018~2025, 노르웨이 선적 한정. 2025년 어획량 **156,234t(−37.6%)**인데 어가는 21.47 → **38.39 kr/kg(+78.8%)**로 올라 어획가치는 오히려 늘었다. 2018년 13.23 kr/kg 대비 2.9배. **노르웨이가 조이는 것은 어획량이지 매출이 아니다** — 한국 수입의 88%가 이 선단이라 협상 여지는 물량이 아니라 계약 시점과 규격에 있다. 위젯 38 → **39**, 4축 **90.6 (A 37 / B 2)**.
> - **빌드에 `CHART_TYPES` 관문 신설.** `MackerelWidgetV2.tsx` 의 switch 가 모르는 chartType 은 default 로 빠져 **빈 카드가 오류 없이 그려진다.** 이번에 `"ComposedChart"`(정답은 `"Composed"`)로 하마터면 나갈 뻔해 빌드가 거부하도록 막았다. 기존 39건 전수 통과.
...
> - **선장 실명은 파이프라인에서 제거** (개인정보 최소화, 교대는 건수만). 계약 테스트가 실명 필드 부재를 강제.
> - 위젯 1차 4종: 이번 주 변화(diff+전주) / 월별 입출항(전년 점선) / 업종별 체류일(전년 비교) / 입출항 타임라인(간트, 이월·체류중·당사 구분). 전부 SYNCED+기준일 (L-09).
> - 테스트: 데이터 계약 7건 + 렌더 스모크 5건 신규, registry 가드 앵커 갱신. `npm run verify` 통과 (152 files·1,180 passed).
> - 2차 후보: 선장 인력 시장(대기 풀·영입 기회)·입항 예상·3개년 선박 검색. 시각 검수는 로컬 dev 로그인 후 /port-intel. 배포는 사용자 지시 시.
...
> - 전일 대비 태평양 `+84`, 대서양 `+155`, 합계 `+239 MT`. 최신 4개 행은 보고량과 선박 행 합계가 모두 일치하고 최신 이슈 0건이다. 누계 검산은 560회이며 기존 과거 불일치 14건/12문서는 변하지 않았다.
> - 보호 상세은 공개 SHA `802127ca…75fd`에 결합됐고 행 수는 태평양/대서양/운반선/연승 `10/7/6/2`다. 상세 파일은 `artifacts/fleet-daily-detail.json`에만 있으며 Git·클라이언트 번들에 포함되지 않는다.
> - RED: 기존 8/24 계약이 남아 집중 Vitest 6건 실패를 확인했다. GREEN: 집중 Vitest 6파일 39 passed/1 skipped, Python fleet sync 11/11, `npm run verify` 통과(ESLint 0 errors·기존 warnings 12, Python 21건, Vitest 1,140 passed/1 skipped, API cache 158/158, Next 118페이지, 실제 보호 상세 32개 누출 0, bundle 33 routes).
> - 로컬 Production 빌드의 1440px·390×844 화면에서 최신 날짜·KPI·검산을 확인했고 문서/본문 overflow 0, console error 0이다. 운영자 상태는 `fleet-daily: verified`다.
> - **다음 단계**: 사용자가 `배포`를 명시하면 Production `FLEET_DAILY_DETAIL_JSON`을 새 상세 DTO로 stdin 교체 → PR gate → main 병합 → READY → 인증/비인증 API와 데스크톱·390px 라이브 QA를 수행한다.
>
> 마지막 업데이트: 2026-08-25 09:53 KST [Codex]

> 🩺 **2026-08-25 09:09 KST — `/fleet` 보호 상세 오류는 배포 전환 중 stale tab으로 확정·해소** [Codex]:
> - 사용자가 본 화면은 8/21 공개 번들을 유지한 열린 탭이었고 「선박 상세 데이터가 공개 집계와 맞지 않습니다」를 표시했다. 같은 탭에서 인증 API를 직접 확인하니 이미 200이며 상세는 보고일 8/24·기준일 8/23이었다. 즉 이전 8/21 클라이언트와 새 8/24 API가 배포 전환 중 일시적으로 갈린 split-brain이었다.
> - 8/21 DOCX `cc282e9b…10e65`로 옛 상세를 결정적으로 재생성해 로더 계약을 확인했지만, 현재 서버가 8/24 상세을 정상 제공한다는 증거가 나온 즉시 8/21 환경값 롤백을 중단했다. 롤백했다면 최신 8/24 공개 집계와 다시 불일치했을 것이다.
> - PR [#756](https://github.com/CUTEKOREA/tuna-dashboard/pull/756) main `1deaac25`, main gate `32791923821`, Production `dpl_ASzoSL4X3FqoqT5kNhwQKfYP2BS7`(GitHub deployment `6073740583`)이 READY이고 `leedonggun.co.kr` alias·`icn1`에 연결됐다.
> - 사용자의 기존 탭을 새로고침해 8/24 보고·8/23 조업 기준, 공개 KPI `316 / 7,173 / 78,730.8 MT`, 보호 상태·예정 상세와 최신 검산 일치를 확인했다. 인증 API는 200·행 수 태평양/대서양/운반선/연승 `10/7/7/2`, 비인증 페이지/API는 307/401·`private, no-store`다.
> - VDS·입어료 탭에서 국적선 `1,447 / 1,023.8 / 423.2 / 27.9일`, KFC `750 / 543 / 207 / 6.5일`과 두 PDF의 8/23 기준일을 확인했다. KFC 키리바시 인쇄 소계 352.7일·행 합 352.8일 차이 안내도 렌더됐다.
> - 390×844에서 문서·본문 overflow 0, console error 0이며 새 Production 최근 15분 error/fatal/5xx 로그도 0건이다. 소스·환경변수·애플리케이션 코드는 수정하지 않았다.
> - 운영자 상태는 배포 SHA·URL·사용자 승인 근거와 함께 `fleet-daily: live_verified`로 닫았다.
> - **다음 단계**: 같은 현상이 재발하면 공개 기준일보다 최신 상세가 도착한 경우 「새 버전 다시 불러오기」를 안내하거나 1회 자동 새로고침하는 UX를 별도 TDD로 검토한다.
>
> 마지막 업데이트: 2026-08-25 09:49 KST [Codex]
...
> - **오진 정정 2건.** ① KAMIS·agromarket 「서버 무응답」은 curl UA 차단이었다 — DNS 만 보고 단정했다. ② KAMIS 품목코드 `640`은 **마른오징어**다. 그대로 쓰면 새우 도매가가 94,300원/kg 으로 8배 튄다. 코드표(`productInfo`)로 `654/01`(흰다리 수입) 확인, 2026-08 도매 12,050원/kg 이 보고서 값과 일치.
> - **발견**: 2025년 국내 생산 33,640t 중 계통판매 19,775t(58.8%). 빠진 13,865t = 양식 7,901t(위판 명부에 애초 없음) + 자연산 5,964t(23.2%). 노량진 경매에는 3년 7개월간 **40건**뿐 — 위판장을 비켜 간 새우는 경매장도 비켜 간다. 도매가는 16개월 폭 5.3%로 거의 안 움직인다(같은 창 CIF 도 6.33~6.56달러로 조용 — 다섯 달로 인과 단정 불가, 본문에 한정).
> - 10차 감사 P0 6건 중 실질 둘: 13절이 2022→2024 수입 단가 하락을 2025-05~ KAMIS 창과 맞대고 있었다(기간 불일치 → 같은 창 월별 CIF 표로 교체, 굵은 결론 셋 철회). 05절 「40% 미지」가 양식분과 충돌(→ 양식/자연산 분해). 11차 P0 0.
> - 수집 함정 기록: FIPS 는 `Accept-Language: ko-KR` 없으면 라벨이 미치환 템플릿, resNo=08 은 `</tr>` 이 없어 시작 태그로 잘라야 함. 노량진은 포장 단위(S/P·box·미)가 제각각이라 kg 단가 환산 불가.
> - 오버레이 재실행이 윤문한 위젯 산문을 스크립트 원문으로 되돌렸다 → 스크립트 쪽 문자열을 고쳐 재발 차단. Drive `8_한국_새우_산업_해부/` 113건 갱신(0바이트 0).
> - **다음 단계**: 미해결 3건 그대로 — 관세 20%·FTA 양허 1차 출처, KMI 바스켓 13,477t 차, GLOBEFISH 2025년 109,020t. 위판 밖 5,964t 의 행방은 어촌계·양식장 출하 기록이 필요.
>
> 마지막 업데이트: 2026-08-23 03:40 KST [Claude]

> ⚖️ **2026-08-23 00:50 KST — `/mackerel` 산지 경매 규격별 단가 위젯** [Claude]:
> - 부산공동어시장 자체 사이트(`bcfm.co.kr`)의 일자별 경매 실적을 수집했다. 2023-01-01~2026-08-23, 2,865행. **규격별로 갈린 유일한 자료**다 — 고등어 상·중·하, 망치고등어 상·중·하가 별도 행이며 상자·무게·금액·단가가 함께 있다.
> - 위젯 `s1_auction_grade_spread`. 위젯 35 → **36**, 4축 90.5 (A 34 / B 2).
> - **보고서 중심 논지의 시계열 근거가 생겼다.** 「모자란 것은 총량이 아니라 규격」의 근거가 실태조사 한 시점(2023-12, 300~450g 중품)뿐이었는데, 이제 상급 물량 404 → 89t(**−78%**), 상급 단가 7,712 → 13,328원(**+73%**)이 연도별로 나온다. 국내에서 잡히는 것 중 식탁용 대형어 몫이 줄고 그 자리를 노르웨이산이 메우는 구조가 산지 경매장에서 그대로 보인다.
> - **함정 셋.** (1) 등급 체계가 2026-07-01 에 상·중·하 → 대·중·소로 바뀌었고 두 체계는 같은 구분이 아니다 — 2025 상급 89t 옆에 2026 대급 1,637t 을 놓으면 18배 늘었다는 헛것이 나온다. 위젯은 구 체계 2023~2025 만 싣는다. (2) 한 표에 구·신 품목코드 블록이 함께 있어 같은 라벨이 두 번 나오는 날이 이틀 있다. 첫 수집에서 순번을 안 저장해 사후 구분이 안 됐고 파서를 고쳐 다시 받았다. (3) 그런데 순번도 식별자가 아니다 — 품목이 추가·삭제되며 밀려 순번 34 가 2023 년엔 망치고등어(상), 2026-07 부터는 고등어(대)다. 라벨이 유일한 식별자다.
> - 한계: **어시장 한 곳**이다. 전국 계통판매에서 부산지역이 75.6% 지만 그 안에도 조합이 넷이고 이 어시장은 하나다. 전국 규격 구성으로 읽지 않는다고 본문·위젯·원장에 적었다. 갈고등어(*Decapterus*)·고등어잡어 제외, 수입산 분리. 사이트는 http 만 된다.
> - 등재: `source_registry.csv` 74 → **75행**(`BCFM_AUCTION_DAILY`). 보고서 06절에 「규격이 무너진 자리가 보인다」 소절 추가, 대조 343건 통과, 30쪽, Drive 55파일.
> - 게이트: provenance 36/36, mackerel:test 4/4, ESLint 0 errors, tsc clean, Vitest 146 files·1,115 passed/2 skipped.
>
> 마지막 업데이트: 2026-08-23 00:50 KST [Claude]

> 📈 **2026-08-23 00:10 KST — `/mackerel` UN Comtrade 2025 연장 위젯** [Claude]:
> - FAO FishStat 빈티지가 **1950~2024** 라 교역 위젯 14건이 전부 2024년에서 끊겨 있었다. UN Comtrade 에는 2025년이 있다.
> - **이어 읽을 근거를 먼저 잡았다** — 2024년을 같은 HS 세 코드(030244·030354·160415)로 Comtrade 집계하면 FAO 표(수입 39,454t·수출 78,714t·단가 2.37/1.10)가 소수점까지 재현된다. 그래서 기존 FAO 위젯 14건을 고쳐 출처를 섞는 대신 **연장 위젯을 따로** 만들었다.
> - 위젯 `s3_trade_comtrade_2025`. 위젯 34 → **35**, 4축 90.4 → **90.5 (A 33 / B 2)**.
> - **「수입량의 두 배」가 2025년엔 2.71배다.** 수출 78,584 → 143,842t(+83.0%), 단가 역조 1.28 → 1.42달러. 두 시장이 갈라지는 방향이 2024년 한 해로 끝나지 않았다.
> - 아프리카 수출은 2024년 −22.3% 뒤 **2025년 109,193t 으로 +76.8% 반등해 최고치**다. 보고서가 −22.3% 에서 끝나 하락으로 읽히던 것을 바로잡았다.
> - 노르웨이 의존 삼단이 채워졌다 — FAO 2024 **88.1%** → Comtrade 2025 **76.1%** → 관세청 2026.1~8 **64.8%**. 꺾임은 2026년이 아니라 2025년에 이미 진행 중이었다. 빈자리는 중국이 6.7→12.6% 로 메웠으나 임가공 재수입이 섞여 있어 그대로 「대체 소싱」으로 읽으면 안 된다.
> - 보고서도 같이 반영했다 — 04·05·07절에 2025 추가, 06절은 FAO 국가별이 2024까지라 세계표를 못 늘리는 대신 한국 2025(214,407t, 전년비 +58%)를 따로 적었다. 부록 A-6 은 **상위 30 → 100, 2023~2025 3개년**으로 넓혔다. 대조 275 → **323건** 통과, 29쪽, Drive 55파일.
> - 도중에 잡은 것 둘: provenance 게이트가 미등재 `source_id`(`UN_COMTRADE` → 원장은 `UN_COMTRADE_HS`)를 막았고, 단위를 kg/1000=톤인데 「천 톤」으로 적은 것을 자체 검산에서 고쳤다.
> - 게이트: provenance 35/35, mackerel:test 4/4, ESLint 0 errors, tsc clean, Vitest 146 files·1,115 passed/2 skipped.
>
> 마지막 업데이트: 2026-08-23 00:10 KST [Claude]

> 💰 **2026-08-22 23:40 KST — `/mackerel` 소매 원산지별 가격 위젯 신설** [Claude]:
> - aT KAMIS 품목 611 고등어를 수집했다. 7계열 2020-01~2026-08 — 도매 3종(생선·냉동·냉동수입)·소매 4종(국산 염장/신선냉장/냉동·수입산 염장). **국산과 수입산이 갈린다.** 05절 노르웨이 의존과 10절 값 사슬을 잇는 축이 여기서 나온다.
> - 위젯 `s4_retail_origin_spread`. 위젯 33 → **34**, 4축 90.3 → **90.4 (A 32 / B 2)**.
> - **「수입산이 싸다」는 통념이 이 품목엔 안 맞는다.** 소매 염장 기준 수입산이 국산의 1.5배(동시 관측 37개월 평균)이고 2026년 8월은 **1.9배**다. 국산은 2025년 6,761원까지 올랐다가 2026년 6천원 아래로 내려온 반면 수입산은 계속 올라 11,499원이다.
> - **단위 함정을 밟을 뻔했다.** 첫 집계에서 도매 냉동(수입)이 +140%로 나왔는데, 그 계열만 **20kg**이고 나머지 도매는 10kg이었다. 오징어 수집기 주석에 「두 계열을 나누면 유통마진이 아니라 헛것이 나온다」고 적혀 있던 그 함정을 같은 사이트에서 다시 만났다. 계열별 단위를 JSON 에 함께 싣고 원장·영수증에 경고로 박았다.
> - 소매 「환산」은 KAMIS 원문이 기준 중량을 안 밝힌다. kg 환산 미확정이라 도매와 배수로 견주지 않고 같은 소매 계열끼리만 비교한다.
> - 등재: `source_registry.csv` 73 → **74행**(`KAMIS_MACKEREL_PRICE`), 영수증 SHA-256.
> - 보고서 10절에도 반영했다(대조 275건 통과, 30쪽, Drive 55파일). 기존 KAMIS 소절이 국산 한 계열·2025년까지였던 것을 원산지 축과 2026년으로 넓혔다.
> - 게이트: provenance 34/34, mackerel:test 4/4, ESLint 0 errors, tsc clean, Vitest 146 files·1,115 passed/2 skipped.
> - PR #741(위젯 문구 윤문)은 main 직접 push 로 반영돼 CLOSED 처리했다 — 인증 복구 과정에서 rebase 로 해시가 바뀌어 자동으로 안 닫혔다.
>
> 마지막 업데이트: 2026-08-22 23:40 KST [Claude]

> 🎣 **2026-08-22 23:10 KST — `/mackerel` 위판·품종별 생산 위젯 2건 신설** [Claude]:
> - 보고서 06절이 「위판 자료는 아직 없다」, 13절이 「품종별 원표를 수령하지 못해 멈췄다」고 적고 있었다. 공공데이터포털 건별 위판은 다운로드에 보안문자가 걸려 막혀 있는데, **발행 기관이 같은 자료를 다른 문으로 열어 두었다** — 해양수산부 수산정보포털이다. 보안문자는 건드리지 않았다.
> - **수협계통판매통계정보**(S020601) 190파일 · 2008~2026 × 항목 10종, **지역·조합 단위**(09) 400파일 · 2023~2026 × 지역 10 × 조합 전량. **어업생산통계 품종별**(S020304) 36파일 · 2015~2026 × 어종 3종. 인증·보안문자 없음. 실패 0.
> - **9,158톤 물음이 닫혔다.** 2024년 국내 품종별 생산은 연근해 고등어 125,135t · 원양 520t · 망치고등어 9,676t 이고, FAO 한국 제출본은 MAS 북서태평양 124,928.012t 과 남동태평양 519.760t 두 행뿐이다. **원양 520t 과 남동태평양 519.760t 이 사실상 일치**해 범위 대응까지 잡히고, 남는 것이 망치고등어인데 그 행이 아예 없다. 교차 근거로 망치고등어 생산량과 계통판매 실적이 10개 연도에서 차이 2t 이내다.
> - 위젯 `s1_species_production_gap`(국내 품종별 생산과 FAO 제출값) · `s1_coop_landings_price`(위판 물량과 산지 단가). 위젯 31 → **33**, 4축 89.9 → **90.3 (A 31 / B 2)**.
> - **단가 정의를 보고서에 맞췄다.** 초안이 고등어류+망치고등어 혼합(1,478→2,558원, 73%)이었는데 06절 표는 고등어류 단독(1,617→2,555원, 58%)이다. 화면과 문서가 다른 숫자를 말하면 안 되므로 단독 기준으로 고쳤다.
> - 부산 집중이 2025년 **75.6%**(150,124/198,627t)다. 조합 단위까지 갈리지만 **어선별은 여전히 없다** — 정보공개청구 경로로 남는다.
> - 등재: `source_registry.csv` 71 → **73행**(`FIPS_COOP_SALES`·`FIPS_PRODUCTION_SPECIES`), 캘린더, 아카이브 596파일 30MB.
> - 게이트: provenance 33/33, mackerel:test 4/4, ESLint 0 errors, tsc clean, Vitest 146 files·1,115 passed/2 skipped.
> - 상태: 브랜치 `mackerel/landings-widgets`.
>
> 마지막 업데이트: 2026-08-22 23:10 KST [Claude]

> ✍️ **2026-08-22 22:40 KST — `/mackerel` 위젯 문구 31건 전량 윤문** [Claude]:
> - 화면 문구가 빌더 파이썬 f-string 안에 박혀 있어 렌더된 JSON 을 윤문하면 식으로 되돌릴 수 없다. `ast` 로 `JoinedStr` 를 잡아 `FormattedValue` 를 `⟦n⟧` 토큰으로 얼리고 글자만 뽑는 파이프라인을 새로 만들었다(`/tmp/mk_prose/`).
> - **윤문 전 왕복 무결성을 먼저 봤다** — 원문을 그대로 재주입해 산출 JSON 31건이 전건 동일함을 확인한 뒤에 진행했다. 이게 안 되면 윤문은 의미가 없다.
> - strict 5인 파이프라인 2회차. 탐지 109 span 중 **83개(76%)가 `strat`** 한 칸에 몰려 있었다. 「통념 부정 → 처방」 2박자가 31개 중 19개, 조건절 「~면 …한다」가 22회. 위젯 하나만 보면 날카롭지만 화면을 세로로 훑으면 같은 틀이 31칸에 찍혀 있다. 처방은 개별 교정이 아니라 **쿼터 감축**이었다.
> - **em대시 25 → 0.** subtitle 의 「출처명 — 범위」 20회는 탐지기가 조판 규칙으로 판정해 제외했으나, 20번 반복되면 서식도 틀이라 전량 정리했다. 쉼표·마침표·속격·「기준」·관형·문장 편입 등 7종으로 분산해 어느 처리도 3회를 넘지 않게 했다.
> - **1차는 지표만 통과했다.** f005(2문장 수렴) 20→12 로 풀렸는데 줄어든 8칸이 전부 두 문장을 봉합한 장문이었고, 칸 길이 표준편차가 소수점까지 그대로였다(21.6→21.3). 2차에서 진짜 단문을 만들어 최단 칸 53→39자, sd 22.9 가 됐다.
> - **어미 축에서 새 공식이 생겼다** — 「~고,」 3 → 12(4배). 어간이 매번 달라(했고/이고/붙고/없고) 어절 n-gram 검사에 원리적으로 안 잡힌다. 2차에서 5 로 줄였고, 봉합은 전부 문장 분리로 해 연결어를 하나도 넣지 않았다.
> - 충실성 감사 롤백 7건: 강도 하락 4(「다루면 안 되는 시점」→「이른 시점」 등 금지→유예, 불가→근거 부재), 없던 인과 2(em대시를 문장으로 세우며 연결어 주입), 프레임 소실 1. **출처 귀속은 25건 전수 통과** — 위험은 귀속이 아니라 연결어였다.
> - 쿼터 2건(대구 11/≤10, 통념 인용 3/≤2)은 넘긴 채 뒀다. 셋 다 감사관 복원 문면 그대로이고, 다른 데를 깎아 숫자를 맞추면 감사관이 방금 처벌한 것과 같은 의미 이동이 된다. **의미 불변이 쿼터 상한보다 위다.**
> - 재주입 후 전용 검증기로 확인: 문구 67칸 변경, **수치·고유명·`data`·`provenance` 위반 0**. 위젯 31·4-Axis 89.9(A) 불변.
> - 게이트: ESLint 0 errors, tsc clean, Vitest 146 files·1,114 passed/2 skipped, provenance 31/31, mackerel:test 4/4.
> - 상태: 브랜치 `mackerel/widget-prose`.
>
> 마지막 업데이트: 2026-08-22 22:40 KST [Claude]
...
> - 운영 인증 API는 200·`private, no-store`로 HIKARI 당일 `601.420`, 누계 `1,404.460`, 잔량 `1,524.540 MT`, 8/23 예정 `87 MT`를 반환한다. 비인증 API는 401 `authentication_required`·`Vary: Cookie`, 비인증 `/unloading`은 로그인으로 307 이동한다.
> - 운영 보호 화면 1440×1000·390×844에서 HIKARI 최종 보고 8/22, 통합 `37,787 MT`, 완료 12척, 당일 수하처 4건·원선/어창/온도·명일 87톤과 자동보고를 확인했다. overflow/page/console errors 모두 0이며 최근 1시간 Vercel error/fatal 로그도 0건이다.
> - **다음 단계**: 8/23 HIKARI 1 하역 원본 수신 시 같은 멱등 절차로 갱신한다.
>
> 마지막 업데이트: 2026-08-22 21:19 KST [Codex]
...
> - 위젯 `s2_domestic_processors`(신고 대 생산 업소 2016~2025) · `s2_mackerel_share_of_processing`(품목보고번호 연결로 고등어 주재료 몫). 위젯 29 → **31**, 4축 89.7 → **89.9 (A 29 / B 2)**.
> - **교차검증에서 라벨 4건이 틀렸다.** Codex 독립 재계산과 adversarial-reviewer 가 각자 도달했고 산수는 소수점까지 전건 일치했다. 틀린 것은 이름이었다.
>   - `BSSH_NM` 은 법인이 아니라 **업소명**이다(인허가번호 기준 751 사업장). 「업체 748곳」이라 쓸 수 없다.
>   - 2025년 신고 **746곳** 중 실제 생산은 **453곳**. 나머지는 생산량 0 신고다. 신고는 603→746 으로 늘었지만 **생산 업소는 10년째 438~471 구간에서 안 움직인다.** 「가공업체가 늘고 있다」는 없는 성장 서사였다.
>   - 주재료 연결분 16,753톤은 **완제품 중량**이지 고등어 투입량이 아니다. 함량 비공개라 상한으로만 읽는다.
>   - 상호 정규화 정규식이 `[\s()（）]` 를 앞세워 「(주)씨앤블루」를 「주씨앤블루」로 만들고 있었다. 수입 겸업 판정이 과소였다(신고 기준 9곳 / 생산 기준 7곳).
> - **필터는 양방향으로 틀렸다.** 과대: 키워드 「자반」이 콩자반·김자반을 끌어와 44% 혼입, 포장재 14행(신고 식품유형이 폴리에틸렌·PP·PET), 「고등어빵」 제과 6행. 과소: 제품명에 어종을 안 적었지만 원재료가 고등어인 품목 12행이 빠져 있었다(Codex 적발). 배제 내역은 부록 A-9 로 공개했다.
> - 보고서 쪽 관문 하나를 신설했다 — **관문5 정본 동기화**. 정본(canon.json)은 손으로 쓰고 부록은 매 빌드 재생성이라, 필터를 고치면 재생성분만 따라가고 정본이 낡는다. 실제로 미연결 문턱이 5.1%→4.3% 로 바뀌었는데 본문이 5.1% 를 들고 있었고 관문 1(본문↔정본)은 이를 못 잡았다.
> - 게이트: ESLint 0 errors, tsc clean, Vitest 146 files·1,113 passed/2 skipped, provenance 31/31, mackerel:test 4/4.
> - 상태: 브랜치 `mackerel/processor-roster`. **프로덕션 미배포.**
>
> 마지막 업데이트: 2026-08-22 20:40 KST [Claude]
...
> 🐟 **2026-08-22 18:43 KST — `/mackerel` 보고서 집필에서 나온 배포본 정정 3건** [Claude + Codex + Grok + Gemini]:
> - 고등어 산업 해부 보고서(13절 + 부록 A, A4 25쪽)를 쓰면서 같은 아카이브로 배포본을 재검증했고, **라이브에 틀린 값 3건**이 확인돼 정정했다.
> - **아프리카 집합**: 손으로 고른 7개국이었다. FAO 국가그룹 기준 한국이 실제로 수출하는 곳은 **30개국**이고, 7개국 집합은 2024년 물량의 **12.5%(7,742t)** 를 빠뜨렸다. 베냉 하나가 4,522t 으로 중국보다 크다. 모리타니는 집합에 있었으나 2024년 실적이 0t. 누적 +137.1% → **+140.6%**, 2024 YoY −16.9% → **−22.3%**. 코드를 짐작으로 32개 적었다가 수치가 안 맞아(−18.0%) 원본에서 실측 30개로 교체했다.
> - **NPFC 어획한도**: CMM 2025-07 의 66,740t 을 인용하고 있었다. **CMM 2026-07**(2026-06-01 발효)이 2026년 **47,940t**·2027년 42,300t 으로 대체했다. 제11항 제외분이며 EU 3,060t 을 더하면 51,000t.
> - **원산지 구조**: FAO 2024 노르웨이 88.1% 만 띄우고 있었다. 관세청 통관 실적 2026-01~08 은 **64.8%** 다. 의존이 깊어진 게 아니라 **꺾였고** 영국(12.9%)·중국(13.5%)이 그 자리를 메웠다. 통관은 FAO 보다 1~2년 앞서므로 두 값을 나란히 두되 범위가 다름(냉동 단일 코드 vs Scomber 속 전체)을 명시한 위젯을 신설했다.
> - 두 정정은 `test_build.py` 의 assert 로 고정했다(−22.3 / +140.6). 집합을 다시 바꾸면 빌드가 깨진다.
> - **4벤더 교차검증**: Codex=교역 수치 원자료 재계산(소수점까지 전건 일치), Grok=실시간 반증(**컷오프 고정** 적발 — 2024 자료로 2026년 시장을 현재형 서술), Gemini=구조 검토(절 배치가 자료 확보 순서를 따름), 일관성 검사=기관명 혼용. 숫자 조작은 0건이었고 틀린 것은 **범위와 시점**이었다.
> - 위젯 28 → **29**, O-04 4축 89.5 → **89.7 (A 27 / B 2)**. 신선도 축 76.8 → 77.4.
> - 게이트: ESLint 0 errors(경고 13), TypeScript, Vitest **189/189**, mackerel 29/29, mackerel:test 4/4.
> - 보고서 정본은 Drive `8_한국_고등어_산업_해부/`(35파일, SHA-256 전건 일치).
> - 상태: 브랜치 `mackerel/report-sync` 커밋 `43260c13`. **프로덕션 미배포.**
...
> 🔒 **2026-08-21 — Thai Union 보고서에서 신라 사내 거래자료 언급을 뺐다** [CC]:
> - 사용자 지시. 07 한국 관점 콜아웃의 「신라 내부 거래자료에는 2026년 TUM·TUG 송장과
>   TUG 하역·사이징 보고가 기록돼 있다」 문단을 통째로 제거했다. 뒤에 붙어 있던 동원·사조
>   계약 상대방·PVR 선망선 7척 문장도 같은 문단이라 함께 뺐다.
> - 콜아웃 제목 「한국 기업 공시와 **사내 거래자료가** 보여주는 접점」 → 「한국 기업 공시가
>   보여주는 접점」. 판권 각주의 출처 목록에서도 「DART와 신라 내부 거래자료다. 내부 자료의
>   민감정보와 금액은 공개 대상이 아니다」를 「DART다」로 줄였다.
> - **다른 여섯 편은 확인 결과 같은 성격의 언급이 없다**(내부/사내 자료·송장·내부 문서 0건).
> - Drive 발행본(HTML·PDF 35쪽)과 대시보드 evidence 사본 갱신. 표 JSON 은 그대로다 —
>   지운 것이 표가 아니라 콜아웃이다.
...
## 2026-08-22 — 「시장 이해 > 오징어」 에 최종 보고서 15절 이식 (단계 09~15)
- `lib/squid-industry-content.ts` STAGE_09~15 (산지·조달선과 수입 주체·원양·가공·연결과 재무·값·제도와 남은 물음) — 보고서 `8_한국_오징어_산업_해부` 본문 문장을 옮김. 초안 7기 + 검증 7기(숫자 전건 원문 대조·문체·영문) 워크플로.
- 큐레이션: `curate_squid_industry_widgets.py` 에 단계 7개 + s08 자리 + F 위젯 15개 승격(thesis·현황·실행지침) + 열 라벨. 위젯 22개 선별, 탭 18개.
- 요약 8항목(보고서 01절)·자료 한계 2건 추가. 수입 수치 둘(바스켓 172,449t vs 10자리 164,676t) 주석.
- 검증 중 잡힌 데이터 정정: 시도 병합(경상·충청)→16 시도 분리, 0307.43-2010 역내협정 미양허(빈칸), 수입 유형 재판정(표 20: 73/312 23.4% … 원물 중심 4/25, 직수입 원물 중심 3곳·조제품 전업 아라움 1) — 보고서·통합정본·Drive 동시 정정.
...
- 2026-08-23 aT/KAMIS 수집: UA 차단만 있고 로그인·보안문자 없음. 월별 지역(도매 6·소매 15지역, 72계열) + 경락 일별 888거래일(2022-12-30~2026-08-21, 요청 1,331일). Drive `02_출처원본/aT_KAMIS_지역별/`. 위젯 `F_kamis_region_2025`·`F_kamis_daily_monthly`·`F_kamis_origin_premium` 신설, 시장 이해 14 값 단계에 배치. 핵심: 마른오징어 지역 격차 35.7%, 스프레드 중앙값 2023 23%→2025 60%, 연근해가 원양보다 34% 비쌈(2025-08 부터 구분). 함정: 표 방향이 도매/소매 반대, 휴장일엔 첫 열이 전날, 소매 물오징어는 지역별 관측 월수가 다름(강릉 1개월).
