> 🚀 **2026-09-26 23:20 KST — #1260 9/26 하역 배포 (체선 초과 진입)** [CC]:
> - 병합(squash): #1260 `3102664f`. Production `tuna-dashboard-papu1wup2` READY, alias `https://leedonggun.co.kr`. 배포 로그 error 0.
> - 라이브 실측(새 탭): `/unloading` 누계 **1,831.03** · 잔량 **14.97** · 일평균 **228.9**. 9/26 행·9/27 계획 80 MT 는 API 로 확인.
> - 체선 카드가 처음으로 초과로 넘어갔다 — 「허용 8.4일(1,846MT/220) − 사용 9일(일요일 1 제외) = **초과 0.6일 · 추정 $6,000**(조율 전)」, 등급 **High**.
> - 이 PR 은 CI 에서 두 번 빨개졌고 둘 다 내 데이터 변경이 아니라 **시간·상태 경계** 문제였다: ① 묵은 기준일 기준선(날짜 경과로 sashimi 위젯 2건이 18개월 초과) ② 체선 문구가 「여유」 → 「초과」로 바뀌며 e2e 정규식 불일치. 각각 기준선 갱신·스펙 수정으로 풀었다.
