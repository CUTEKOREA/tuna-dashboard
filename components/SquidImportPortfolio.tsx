...
export default function SquidImportPortfolio() {
  ...
  return (
    <div className={styles.glassCard} style={{ borderColor: 'rgba(245, 158, 11, 0.3)' }}>
      <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#fbbf24', marginBottom: '6px', fontWeight: 700, fontSize: '1.1rem', position: 'relative' }}>
        <Package size={20} /> 한국 오징어 수입 품목 포트폴리오 (2010-2023)
        
      </h3>
      <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.82rem', marginBottom: '16px' }}>
        냉동 원물 중심에서 양념·가공 오징어의 비중이 꾸준히 확대 — 양념 오징어 수입 <span style={{ color: 'var(--color-danger)', fontWeight: 700 }}>+{seasonedGrowth}%</span> 급증
      </p>

      <div ref={chartRef} style={{ width: '100%' }}>
        {chartWidth > 0 && (
          <BarChart width={chartWidth} height={380} data={portfolioData} margin={{ top: 10, right: 20, left: 20, bottom: 20 }}>
      ...
      <div style={{ marginTop: '20px' }}>
        <TakeawayBox
          ...
          actionPlan="**[Actionable Insight]** 페루 및 칠레의 대왕오징어를 단순 수입하는 차원을 넘어, 가공 단가가 저렴한 베트남(혹은 현지)을 경유하여 완전 가공된 HMR(가정간편식) 및 양념육 형태로 반입하는 우회 포트폴리오를 다각화하십시오. 수입 후 국내 가공 방식은 최저임금 상승과 조정관세(22%) 부담으로 인해 역마진 리스크가 크므로, 완제품 해외 OEM 소싱 비율을 60% 이상으로 끌어올리는 것이 핵심입니다."
        />
      </div>
    </div>
  );
