...
export function InsightTimeGapArbitrage() {
  return (
    <WidgetCard
      ...
      takeaway={{
        situation: `<div>
<p>"HPAI(High Pathogenicity Avian Influenza, 고병원성 조류인플루엔자)"란 발병 즉시 해당국 가금육 수입을 100% 차단하는 SPS(위생검역) 트리거. "TCU(총이용원가)"란 단순 kg 단가가 아닌 수율·인건비·로스까지 포함한 진짜 사용 원가.</p>
<p>추정: <strong>브라질 HPAI 수입금지(5~7월) → 국내 도매가 1,950 → 2,650원/kg +36% 폭등(KAMIS 기준). 태국산 CIF 2,750원이 명목 단가 비싸 보이나 순살 수율(잔뼈 제로) + 주방 인건비 감안 시 TCU 동등</strong>. 단가 단순 비교 자체가 잘못된 게 본질.</p>
</div>`,
        actionPlan: `<div>
...
export function InsightChannelMatrix() {
  return (
    <WidgetCard
      ...
      takeaway={{
        situation: `<div>
<p>채널 매트릭스란 마진율·볼륨·진입난이도 3축으로 유통 채널을 분석해 자원 배분 우선순위를 결정하는 기법. 단일 채널 의존은 손익 변동성을 즉시 키움.</p>
<p>업계추정: <strong>편의점 HMR 마진 35% 최고 (캐시카우) · 프랜차이즈 B2B 마진 28% + 볼륨 85 (1순위) · 대형마트 생닭은 국내산 독점 진입 불가 · 식자재마트는 중국산 13~14% 저가 덤핑으로 마진 박</strong>. 채널별로 소싱·스펙·결제조건이 완전히 다른 별개 사업.</p>
</div>`,
        actionPlan: `<div>
...
export function InsightVMILockin() {
  return (
    <WidgetCard
      ...
      iconColor="#3b82f6"
      pillar="S3"
      cardDesc='닭고기가 아니라 "결품 없는 공급 안정성"을 판다. VMI(벤더재고관리) 도입 24개월 후 바이어 재계약률 95%+ 달성 (illustrative 모델링)'
      telemetry={{ status: 'STATIC', syncDate: '2026-05-21' }}
      chartHeight={320}
      chart={
