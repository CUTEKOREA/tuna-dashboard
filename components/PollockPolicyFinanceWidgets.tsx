  ...
  return (
    <div className={styles.card} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <CardHeader 
        ...
        icon={ShieldAlert} 
        term="HSK Micro-bottleneck Score" 
        desc="2026년 4대 HSK 분류에 따른 세관 통관 및 식약처(MFDS) 행정 지연율의 파이프라인 부하 수치" 
      />
      <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <TakeawayBox 
          ...
          actionPlan="**[Actionable Insight]** 원물 그대로 수입하는 통마리의 비중을 극단적으로 축소하십시오. 방사능 전수 검사로 보세구역에 컨테이너가 묶이는 즉시 재고 조달(Lead Time)이 무너집니다. 규제 마찰력이 적은 가공품(필레/연육) 중심으로 HSK 통관 코드를 우회 집중하는 것이 수급 방어의 핵심입니다."
        />
      </div>
    </div>
  );
  ...
  return (
    <div className={styles.card} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <CardHeader 
        title="보세창고 정밀투하 지수" 
        icon={Crosshair} 
        term="Precision Warehousing" 
        desc="냉동 보세 창고에 비축된 재고를 언제 시장에 방출(Release)할지 결정하는 수익 극대화 스나이핑 지표" 
      />
      <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
         <div style={{ height: '260px', width: '100%', flexShrink: 0 }}>
          <SafeResponsiveContainer width="100%" height="100%">
        ...
        <TakeawayBox 
          ...
          actionPlan="**[Actionable Insight]** 매입한 물량을 들어오는 대로 단순 유통(Flow)시키지 마십시오. 정부의 명절 비축물 방출 1주일 전까지는 재고를 창고에 묶어놓고(Hold) 품귀 프리미엄을 먹고, 방출 직전에 재고를 전량 투하(Sniping)하여 정부 물가 하락장 펀치를 완벽히 빗겨가야 합니다. (Conviction Buy)"
        />
      </div>
    </div>
  );
  ...
  return (
    <div className={styles.card} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <CardHeader 
        ...
        icon={FileCheck} 
        term="Virtual Docking System Integrity" 
        desc="선적지 현지에서 생산된 Single-frozen Lot별 VDS 정보의 식약처 통관 프리패스(Pre-pass) 확률" 
      />
      <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
         <div style={{ height: '220px', width: '100%', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          {/* 하프 도넛 차트 모방을 위한 퍼센티지 렌더 */}
        ...
        <TakeawayBox 
          ...
          situation="러시아산 및 알래스카산 원물 수입 시, 불법/비보고(IUU) 조업 및 방사능 오염 우려로 인해 통관 세관의 현장 실사가 극도로 까다로워졌습니다. 사전에 전자적으로 증명된 VDS 무결성이 부족하면 전량 반송됩니다."
          actionPlan="**[Actionable Insight]** 당사의 구매 파트너 명단에서 VDS 데이터 무결성 지표가 90% 미만인 러시아 선사는 즉시 블랙리스트(Black-list) 처리하여 입찰에서 배제하십시오. 1차 선상 동결(Single-frozen) 후 바로 블록체인 기반의 VDS 원산지 정보가 박히는 프리미엄 선단에게만 오더를 줘야 리드타임 붕괴를 막습니다."
        />
      </div>
    </div>
  );
