...
describe('SEIN VENUS unloading data', () => {
  ...
  it('matches the seven daily reports through August 15 without duplicating the date', () => {
    const db = loadDb();
    const reports = db.unloading_reports.filter((item) => item.vessel_id === 'sein-venus');

    expect(reports.map((item) => item.report_date)).toEqual(['8/7', '8/8', '8/10', '8/11', '8/13', '8/14', '8/15']);
    ...
    expect(reports.map((item) => item.daily_amount)).toEqual([174.64, 109.07, 331.47, 462.81, 159.59, 424.78, 350.74]);
    expect(reports.map((item) => item.cumulative_amount)).toEqual([174.64, 283.71, 615.18, 1077.99, 1237.58, 1662.36, 2013.1]);
    ...
    expect(reports.reduce((sum, item) => sum + item.daily_amount, 0)).toBeCloseTo(2013.1, 6);
    expect(reports.at(-1)!.cumulative_amount - reports.at(-2)!.cumulative_amount).toBeCloseTo(350.74, 6);
    expect(3275 - reports.at(-1)!.cumulative_amount).toBeCloseTo(1261.9, 6);

    const latest = reports.at(-1)!;
    expect(latest.target_holds).toBe('N/STAR(#2-B:160.600,#2-C:41.990), N/SUN(#3-C:60.100), S/SPR(#4-C:88.050)');
    expect(latest.consignee).toBe('TUM · ISA');
    expect(latest.remaining_amount).toBeCloseTo(1261.9, 6);
    expect(latest.adjusted_remaining_amount).toBeCloseTo(1261.17, 6);
    expect(latest.daily_adjustment_amount).toBeCloseTo(-21.16, 6);
    expect(latest.cumulative_adjustment_amount).toBeCloseTo(-0.73, 6);
    expect(latest.allocations).toEqual([
      {
        consignee: 'TUM',
        amount: 202.59,
        loads: [
          { source_vessel: 'N/STAR', hatch: '#2-B', amount: 160.6 },
          { source_vessel: 'N/STAR', hatch: '#2-C', amount: 41.99 },
        ],
      },
      {
        consignee: 'ISA',
        amount: 148.15,
        loads: [
          { source_vessel: 'N/SUN', hatch: '#3-C', amount: 60.1 },
          { source_vessel: 'S/SPR', hatch: '#4-C', amount: 88.05 },
        ],
      },
    ]);
    const allocationLoads = latest.allocations!.flatMap((allocation) => allocation.loads);
    expect(latest.allocations!.reduce((sum, allocation) => sum + allocation.amount, 0)).toBeCloseTo(350.74, 6);
    expect(allocationLoads.reduce((sum, load) => sum + load.amount, 0)).toBeCloseTo(350.74, 6);
    expect(allocationLoads.filter((load) => load.hatch.startsWith('#2-')).reduce((sum, load) => sum + load.amount, 0)).toBeCloseTo(202.59, 6);
    expect(allocationLoads.filter((load) => load.hatch.startsWith('#3-')).reduce((sum, load) => sum + load.amount, 0)).toBeCloseTo(60.1, 6);
    expect(allocationLoads.filter((load) => load.hatch.startsWith('#4-')).reduce((sum, load) => sum + load.amount, 0)).toBeCloseTo(88.05, 6);
    expect(latest.observations).toEqual([
      { source_vessel: 'N/STAR', hatch: '#2-B', temperatures_c: [-22, -23] },
      { source_vessel: 'N/STAR', hatch: '#2-C', temperatures_c: [-22, -23] },
      { source_vessel: 'N/SUN', hatch: '#3-C', temperatures_c: [-21, -22] },
      { source_vessel: 'S/SPR', hatch: '#4-C', temperatures_c: [-21, -22] },
    ]);
    ...
    expect(latest.source_sha256).toBe('2b295ca629ace7e9aa0b3d50c00992e8f0bdec13b7f1c80d8fb2e6d89fa39b6b');
    expect(latest.source_workbook_sha256).toBe('a52a830b86c022936bf0c727d617073a548c5df466b1111223adab16e6c175b0');
    expect(latest.status_workbook_sha256).toBe('0f0fe538043a59b9bf75f2cab299e161f1dd6299f41b96661bb642c497993dbd');
    expect(latest.species_amounts).toEqual({ SJ: 318.14, YF: 32.6 });
    ...
    expect(reports.at(-3)!.target_holds).toBe('N/STAR(#2-B:159.590)');
    expect(reports.at(-3)!.consignee).toBe('MMP');
    expect(reports.at(-3)!.quality_notes).not.toContain('제품 MMP');
    expect(reports.at(-3)!.quality_notes).toContain('-22.0℃ ~ -23.0℃');
    expect(reports.at(-3)!.quality_notes).toContain('8/14 약 400톤');
    expect(reports.at(-4)!.target_holds).toContain('#1-A:80.670');
    expect(reports.at(-4)!.target_holds).toContain('#1-B:41.170');
    expect(reports.at(-4)!.quality_notes).toContain('8/12 공휴일');
  ...
  it('uses the workbook-confirmed August 15 species split without inventing hold-level species', () => {
    const db = loadDb();
    const vessel = db.unloading_vessels.find((item) => item.vessel_id === 'sein-venus')!;
    ...
    expect(species.reduce((sum, item) => sum + item.actual_amount, 0)).toBeCloseTo(2013.1, 6);
    expect(vessel.unclassified_actual_amount).toBe(0);
    expect(vessel.species_breakdown_as_of).toBe('2026-08-15');
    expect(vessel.species_breakdown_note).toContain('일일 결과보고 XLS');
    expect(vessel.species_breakdown_note).toContain('8/15');
    expect(vessel.hold_species_breakdown_available).toBe(false);
    expect(latestReport.species_amounts).toEqual({ SJ: 318.14, YF: 32.6 });
    expect(species.reduce((sum, item) => sum + item.actual_amount, 0)).toBeCloseTo(latestReport.cumulative_amount, 6);
  });

  it('returns the structured August 15 facts through the current API', async () => {
    const response = await getUnloadingData();
    const payload = await response.json();
    const vessel = payload.data['sein-venus'];
    const latest = vessel.timeline.at(-1);

    expect(vessel.actualTotal).toBeCloseTo(2013.1, 6);
    expect(vessel.unclassifiedActual).toBe(0);
    expect(vessel.speciesBreakdownAsOf).toBe('2026-08-15');
    expect(vessel.holdSpeciesBreakdownAvailable).toBe(false);
    expect(vessel.species).toEqual([
      expect.objectContaining({ id: 'SJ', actual: 1715.5 }),
      expect.objectContaining({ id: 'YF', actual: 297.6 }),
    ]);
    expect(latest.date).toBe('8/15');
    expect(latest.remainingAmount).toBeCloseTo(1261.9, 6);
    expect(latest.adjustedRemainingAmount).toBeCloseTo(1261.17, 6);
    expect(latest.dailyAdjustmentAmount).toBeCloseTo(-21.16, 6);
    expect(latest.cumulativeAdjustmentAmount).toBeCloseTo(-0.73, 6);
    ...
    expect(latest.observations).toHaveLength(4);
    expect(latest.speciesAmounts).toEqual({ SJ: 318.14, YF: 32.6 });
  });

  it('builds the replay series from exact daily workbook species amounts through August 15', async () => {
    const response = await getUnloadingData();
    const payload = await response.json();
    const vessel = payload.data['sein-venus'];
  ...
  it('renders the August 15 office report from structured consignee and observation facts', async () => {
    const response = await getUnloadingData();
    const payload = await response.json();
    const vessel = payload.data['sein-venus'];
    ...
    expect(markup).toContain('202.590 MT');
    expect(markup).toContain('ISA:');
    expect(markup).toContain('148.150 MT');
    ...
    expect(markup).toContain('명일(8/16)은 공휴일로 하역작업이 없습니다. 재개일(8/17)은 약 300톤 하역 작업 예정입니다.');
    expect(markup).not.toContain('명일(8/16)은 약 300톤');
    expect(markup).not.toContain('350톤');
    expect(markup).not.toContain('* SJ:                350.740 MT');
  });
