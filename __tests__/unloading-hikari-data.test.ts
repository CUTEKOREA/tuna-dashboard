...
describe('HIKARI 1 Bangkok unloading plan and daily report', () => {
  ...
  it('keeps capacity, total cargo, and the FCF discharge target on separate bases', async () => {
    ...
    expect(vessel.surplus).toBeCloseTo(57.06, 6);
  ...
  it('records the 8/28 final report and closes the voyage (전 물량 하역 완료)', async () => {
    const vessel = await loadHikari();
    expect(vessel.dateRange).toBe('2026.08.20 ~ 2026.08.28');

    const reports = vessel.timeline.filter((entry: { date: string }) => entry.date === '8/28');
    ...
    expect(reports[0].allocations).toEqual([
      {
        consignee: 'GPZ',
        amount: 119.25,
        loads: [{ sourceVessel: 'MOAKONA', hatch: '#4-C', amount: 119.25 }],
      },
    ]);
    expect(reports[0].observations).toEqual([
      { sourceVessel: 'MOAKONA', hatch: '#4-C', temperaturesC: [-19, 23] },
    ]);
    expect(reports[0].speciesAmounts).toEqual({ SJ: 101.45, YF: 17.8 });
    expect(reports[0].remainingAmount).toBeCloseTo(-57.06, 6);
    expect(reports[0].quality).toContain('TOTAL 8 TRUCKS');
    expect(reports[0].quality).toContain('전 물량 하역 완료');

    // 어종 최종 누계 - 8/28 결과보고 계 (SJ 2,602.74 / YF 383.32)
    const bySpecies = Object.fromEntries(
      vessel.species.map((item: { name: string; actual: number }) => [item.name, item.actual]),
    );
    expect(bySpecies['가다랑어']).toBeCloseTo(2602.74, 6);
    expect(bySpecies['황다랑어']).toBeCloseTo(383.32, 6);
  });
  ...
  it('keeps the structured August 20 discharge when the next report is added', async () => {
    ...
    expect(dischargeReports).toHaveLength(9);
  ...
  it('publishes the source-backed August 25 discharge and the user-provided August 26 plan', async () => {
    const vessel = await loadHikari();
    const reports = vessel.timeline.filter((entry: { date: string }) => entry.date === '8/25');

    expect(reports).toHaveLength(1);
    expect(reports[0]).toMatchObject({
      date: '8/25',
      time: '08:10 ~ 13:30',
      targetHol: 'N/STAR(#1-B:96.300), S/SPR(#2-C:185.720), MOAMARI(#3-C:74.670), MOAKONA(#4-C:77.590)',
      consignee: 'CMC · GPZ · ISA',
      dailyAmount: 434.28,
      cumAmount: 2323.64,
      remainingAmount: 605.36,
      speciesAmounts: { SJ: 411.38, YF: 22.9 },
      nextDay: {
        kind: 'work',
        date: '8/26',
        reason: null,
    ...
    expect(reports[0].allocations).toEqual([
      {
        consignee: 'CMC',
        amount: 96.3,
        loads: [{ sourceVessel: 'N/STAR', hatch: '#1-B', amount: 96.3 }],
      },
      {
        consignee: 'GPZ',
        amount: 185.72,
        loads: [{ sourceVessel: 'S/SPR', hatch: '#2-C', amount: 185.72 }],
      },
      {
        consignee: 'ISA',
        amount: 152.26,
        loads: [
          { sourceVessel: 'MOAMARI', hatch: '#3-C', amount: 74.67 },
          { sourceVessel: 'MOAKONA', hatch: '#4-C', amount: 77.59 },
        ],
      },
    ]);
    expect(reports[0].observations).toEqual([
      { sourceVessel: 'N/STAR', hatch: '#1-B', temperaturesC: [-20] },
      { sourceVessel: 'S/SPR', hatch: '#2-C', temperaturesC: [-21] },
      { sourceVessel: 'MOAMARI', hatch: '#3-C', temperaturesC: [-20, 24] },
      { sourceVessel: 'MOAKONA', hatch: '#4-C', temperaturesC: [-20, 21] },
    ]);
    expect(reports[0].quality).toContain('원문 온도 표기는 #3-C ‘-20,24’ 및 #4-C ‘-20,21’(단위: ℃)');
    expect(reports[0].quality).toContain('TOTAL 27 TRUCKS');
    expect(reports[0].quality).toContain('GPZ/H2C1(S.SPRINTER) 100 MT 08:00');
    expect(reports[0].quality).toContain('ISA/H3C1(MOAMARI) 150 MT 08:00');
  });

  it('publishes the source-backed August 26 discharge and the user-provided August 27 plan', async () => {
    const vessel = await loadHikari();
    const reports = vessel.timeline.filter((entry: { date: string }) => entry.date === '8/26');

    expect(reports).toHaveLength(1);
    expect(reports[0]).toMatchObject({
      date: '8/26',
      time: '08:10 ~ 13:40',
      targetHol: 'S/SPR(#2-C:79.900), MOAKONA(#4-C:34.810), MOAMARI(#3-C:144.550)',
      consignee: 'GPZ · ISA',
      dailyAmount: 259.26,
      cumAmount: 2582.9,
      remainingAmount: 346.1,
      speciesAmounts: { SJ: 236.26, YF: 23 },
      nextDay: {
        kind: 'work',
        date: '8/27',
        reason: null,
    ...
    expect(reports[0].allocations).toEqual([
      {
        ...
        amount: 114.71,
        loads: [
          { sourceVessel: 'S/SPR', hatch: '#2-C', amount: 79.9 },
          { sourceVessel: 'MOAKONA', hatch: '#4-C', amount: 34.81 },
        ],
      },
      {
        consignee: 'ISA',
        amount: 144.55,
        loads: [{ sourceVessel: 'MOAMARI', hatch: '#3-C', amount: 144.55 }],
      },
    ]);
    expect(reports[0].observations).toEqual([
      { sourceVessel: 'S/SPR', hatch: '#2-C', temperaturesC: [-21, 23] },
      { sourceVessel: 'MOAKONA', hatch: '#4-C', temperaturesC: [-22, 23] },
      { sourceVessel: 'MOAMARI', hatch: '#3-C', temperaturesC: [-21, 22] },
    ]);
    expect(reports[0].quality).toContain('원문 온도 표기는 #2-C ‘-21,23’, #4-C ‘-22,23’, #3-C ‘-21,22’(단위: ℃)');
    expect(reports[0].quality).toContain('TOTAL 17 TRUCKS');
    expect(reports[0].quality).toContain('ISA/H3C1(MOAMARI) 190 MT 08:00');
    expect(reports[0].quality).toContain('GPZ/H4B1(MOAKONA) 150 MT 08:00');
  });

  it('publishes the source-backed August 27 discharge and the user-provided August 28 plan', async () => {
    const vessel = await loadHikari();
    const reports = vessel.timeline.filter((entry: { date: string }) => entry.date === '8/27');

    expect(reports).toHaveLength(1);
    expect(reports[0]).toMatchObject({
      date: '8/27',
      time: '08:10 ~ 13:30',
      targetHol: 'MOAMARI(#3-C:171.720), MOAKONA(#4-C:112.190)',
      consignee: 'GPZ · ISA',
      dailyAmount: 283.91,
      cumAmount: 2866.81,
      remainingAmount: 62.19,
      speciesAmounts: { SJ: 250.91, YF: 33 },
      nextDay: {
        kind: 'work',
        date: '8/28',
        reason: null,
    ...
    expect(reports[0].allocations).toEqual([
      {
        consignee: 'GPZ',
        amount: 112.19,
        loads: [{ sourceVessel: 'MOAKONA', hatch: '#4-C', amount: 112.19 }],
      },
      {
        consignee: 'ISA',
        amount: 171.72,
        loads: [{ sourceVessel: 'MOAMARI', hatch: '#3-C', amount: 171.72 }],
      },
    ]);
    expect(reports[0].observations).toEqual([
      { sourceVessel: 'MOAKONA', hatch: '#4-C', temperaturesC: [-23] },
      { sourceVessel: 'MOAMARI', hatch: '#3-C', temperaturesC: [-22, 23] },
    ]);
    expect(reports[0].quality).toContain('원문 온도 표기는 #3-C ‘-22,23’(단위: ℃)');
    expect(reports[0].quality).toContain('TOTAL 19 TRUCKS');
    expect(reports[0].quality).toContain('GPZ/H4C1(MOAKONA) 100 MT 08:00');
    const rawDb = JSON.parse(readFileSync(join(process.cwd(), 'public/data/unloading/local_db.json'), 'utf8'));
    const rawReport = rawDb.unloading_reports.find((entry: { id: string }) => entry.id === 'hikari-bangkok-2026-07-8-27');
    expect(rawReport).toMatchObject({
      source_sha256: 'c33f9eb4f2e4b22b20c07c9976f52aa6cece2c3edeee9deac7a26a073e37fde5',
      source_workbook_sha256: '66e5b1653930a9062901a7a5504c72502bb1e714b753190f198a8d79ac476ddb',
      status_workbook_sha256: '0bdef9ead72015bbcedd819a60eff5566348778ba1676e70695db4c354bcc96e',
    });
  });
  ...
  it('matches the FCF breakdown by species and source vessel', async () => {
    ...
    expect(vessel.species).toEqual([
      expect.objectContaining({ id: 'SJ', name: '가다랑어', reported: 2515, actual: 2602.74 }),
      expect.objectContaining({ id: 'YF', name: '황다랑어', reported: 358, actual: 383.32 }),
    ...
    expect(vessel.species.reduce((sum: number, item: { actual: number }) => sum + item.actual, 0)).toBeCloseTo(2986.06, 6);
