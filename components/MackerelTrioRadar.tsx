...
export default function MackerelTrioRadar() {
  ...
  return (
    <WidgetCard
      ...
      takeaway={{
        situation: "글로벌 주요 국가의 밸류체인을 '조업-수입-수출' 3축으로 분석하면, 노르웨이(조업국), 칠레(조업/수입 기반 가공 허브), 한국(자급률 붕괴로 98% 수입 의존형 소비국)의 명확한 포지션이 드러납니다. 특히 한국은 자력 조업 기반을 사실상 상실한 채 아프리카/개도국행 재수출 유통 허브 역할에만 머무르는 '기형적 소비-중계국'으로 전락했습니다. 이는 환율 변동과 노르웨이 수출 정책에 국가 식량 안보가 직접적으로 노출된 최약체 포지션입니다.",
        actionPlan: "한국의 '기형적 소비-중계' 모델에서 완전히 탈피하여, 칠레의 '가공 산업(Processing)' 포지션으로 전환해야 합니다. 노르웨이산 H&G를 대량 보세 수입하여 영남권(부산/감포) 단지에 집중 투입하고, 고부가가치 순살/양념 제품으로 전환한 뒤 내수 방어 및 글로벌 C/S 채널(미국 한인마트, 아시안 슈퍼)에 역수출(Export)하여 수출 축 레이더를 인위적으로 비대화시키는 수익 모델을 강제 가동.",
