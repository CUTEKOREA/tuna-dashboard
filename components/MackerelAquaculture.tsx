...
export default function MackerelAquaculture() {
  ...
  return (
    <WidgetCard
      ...
      takeaway={{
        situation: `<div>
<p>고등어 양식은 표층 회유어 특성상 일본·노르웨이 외 상용화 사례가 극히 적은 첨단 분야. 양식 단가가 자연산보다 비싼 역전 현상은 횟감·생식용 프리미엄 수요 존재의 증거.</p>
<p>실측: <strong>${latest.year}년 자연산 ${(latest.capture_t / 1000000).toFixed(1)}M톤 vs 양식 ${latest.aquaculture_t.toLocaleString()}톤 — 양식 비중 ${latest.aqua_ratio_pct}% 미미. 단, 양식 단가 ${premiumPct}% 프리미엄 → 고부가 시장 존재</strong>. 자연산 어획 변동성에 무방비 노출된 산업 구조에서 양식 연구개발은 손익 방어의 핵심 지렛대.</p>
</div>`,
        actionPlan: `<div>
<p><strong>재정의</strong>: 양식은 단순 부가 연구개발이 아닌 <strong>"자연산 어획 급감 시 손익 방어 옵션 + ${premiumPct}% 프리미엄 수익화 이중 수단"</strong>.</p>
<p><strong>3단계</strong>: ① 고부가가치 양식 기술 연구개발 투자 확대 (제주·일본 합작 우선) ② 생식용·횟감 프리미엄 라인업 진출 — 마진 개선 여지 존재 ③ 자연산 어획 변동성 헤지 수단으로 양식을 자산 운용 측면에서 평가.</p>
</div>`,
        source: 'FAO FishStatJ 수산양식 생산통계(1984–2023) · FAO SOFIA 2024',
      }}
    />
  );
}
