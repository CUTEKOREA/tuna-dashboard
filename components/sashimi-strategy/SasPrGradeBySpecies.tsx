...
const SPECIES = [
  ...
  { ko: '날개다랑어', en: 'Albacore·빈초', rc: '#a78bfa', factor: '백색·식감', desc: '"흰 참치(white tuna)" — 통조림 + 일부 사시미(빈초마구로). 백색·식감으로 판정.' },
];
...
export default function SasPrGradeBySpecies() {
  return (
    <WidgetCard
      ...
      cardDesc="어종별 등급 주 결정요인(지방·색·선도)·미오글로빈 redox·야케 — Catalina·Easyfish·ScienceDirect"
      takeaway={{
        ...
        source: "Catalina Offshore·Easyfish(황다·눈다=색/참다랑어=지방 판정) / Springer 2026(미오글로빈 redox 변색) / ScienceDirect(어종별 야케 근단백 비교: 황다·눈다·가다) / 업계 관행(법정표준 아님)",
      }}
      customBody={
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%' }}>
          ...
          <div style={{ fontSize: '0.6rem', color: '#64748b', lineHeight: 1.5, textAlign: 'center' }}>
            🔬 색=미오글로빈 산화(옥시→메트=갈변, CO처리가 위장) · 야케=어획 발버둥 화상육 · 오토로 지방 최대 30%
          </div>
        </div>
      }
    />
