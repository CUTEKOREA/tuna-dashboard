...
import { Radar, TrendingUp, DollarSign } from 'lucide-react';
...
export default function SquidArbitrageRadar() {
  ...
  // If mobile, map is full width and list is below. If desktop, map is 65% and list is 35%
  const mapWidth = isMobile ? containerWidth : Math.floor(containerWidth * 0.65) - 20;
  // Fallback to prevent NaN before measure finishes
    ...
    <div className={styles.glassCard} style={{ marginTop: '2rem' }}>
      <div className={styles.cardHeader}>
        <h3 className={styles.cardTitle}>
          <Radar size={22} className={styles.pulse} color="#38bdf8" /> 
          오징어 글로벌 마진 레이더 (Squid Arbitrage Radar)
          
        </h3>
        <p className={styles.cardSubtitle}>
          전 세계 실시간 '원물 수입가'와 '가공품 수출가'의 Spread 차액을 감시합니다. 수익률이 극대화되는 가공 무역 경로를 B2B 트레이딩 봇처럼 시각화하여 가장 돈이 되는 해역을 한눈에 파악하세요.
        </p>
      </div>
        ...
        <div style={{ flex: isMobile ? 'none' : '0 0 65%', position: 'relative', overflow: 'hidden', borderRadius: '8px', border: '1px solid rgba(56, 189, 248, 0.2)' }}>
          {containerWidth > 0 && (
            ...
            >
              ...
              {/* Continents (Simplified dotted outlines) */}
              ...
              {sortedRoutes.map(route => {
                ...
                // Control point for Bezier curve to make them arc nicely across oceans
                const cx = (s.x + d.x) / 2;
                ...
                return (
                  <g key={route.id}>
                    ...
                    {/* Animated moving dot for trade flow */}
                    <path
                      d={pathData}
                      fill="none"
                      stroke={route.color}
                    ...
                    {/* Labels only show on hover for clarity if mobile, or always if hovered */}
                    {isHovered && (
                      <g>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', maxHeight: isMobile ? 'none' : `${h - 50}px` }}>
            {sortedRoutes.map((route, i) => (
              ...
              >
                ...
                {/* Spreadsheet like view for hovered item */}
                <div style={{ 
      ...
      <div style={{ marginTop: '20px' }}>
        <TakeawayBox
          ...
          situation="차익 마진(Spread) 레이더를 통해, 인도네시아·페루 등 생산기지에서 가공 원료로 매입한 뒤, 1차 가공(할복·탈피)을 거쳐 일본·중국 등 주요 소비국으로 재수출할 때 발생하는 톤당 $200~$410 수준의 '차익거래(Arbitrage) 밴드'가 실시간 포착됩니다. 특히 인도네시아-일본(Sashimi Grade) 라우트는 운송비를 제하고도 톤당 $410의 초과 Bottom-line(순이익) 구간을 보여 단순 원물 무역 대비 높은 Bottom-line(순이익) 구조를 입증합니다."
          actionPlan="**[Actionable Insight]** 페루 파이타(Paita) 항구나 인도네시아 수라바야 현지에 1차 전처리(할복/세척) 조인트벤처를 즉시 셋업하십시오. 해외에서 반제품 상태로 가공하여 일본 등에 직납(FOB)하는 3각 무역 모델을 통해 불필요한 국내 반입 관세 및 물류비를 아끼고 중간 마진을 100% 자사화할 수 있습니다."
        />
      </div>
      <style>{`
