...
export default function SupplierTrademoPhase2({ isAnalyzing, results, targetKeyword }: SupplierTrademoPhase2Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} style={{ marginBottom: '3rem' }}>
      <h3 style={{ borderLeft: '4px solid #10b981', paddingLeft: '0.75rem', marginBottom: '1.5rem', color: '#e2e8f0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div style={{ background: 'rgba(20, 28, 52, 0.8)', border: '1px solid #334155', borderRadius: '12px', padding: '2rem', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.5)' }}>
        ...
        <AnimatePresence>
          ...
          {isAnalyzing && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '2rem', color: '#10b981' }}>
          ...
          {results && !isAnalyzing && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '1px solid #334155', paddingBottom: '0.75rem', marginBottom: '1.5rem' }}>
                <h4 style={{ color: '#f8fafc', margin: 0, fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <ShieldCheck size={18} color="#10b981" /> 역추적 성공 (Unblinded Suppliers)
                </h4>
                <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>
                  타겟 품목: <strong style={{ color: '#f8fafc' }}>{targetKeyword}</strong> 글로벌 공급망
                </span>
              </div>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <div style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid #10b981', padding: '0.5rem 1rem', borderRadius: '6px', color: '#10b981', fontSize: '1.05rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          {item.unblindedName}
                        </div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                          <span style={{ color: '#94a3b8', fontSize: '0.75rem' }}>AI 확신도</span>
                          <strong style={{ color: '#f8fafc', fontSize: '1.1rem' }}>{item.confidence}</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '2rem', borderTop: '1px solid #334155', paddingTop: '1rem', flexWrap: 'wrap' }}>
                      <div>
                        <span style={{ color: '#94a3b8', fontSize: '0.8rem', display: 'block' }}>선적 경로 (POL &rarr; POD)</span>
                        <strong style={{ color: '#cbd5e1', fontSize: '0.9rem' }}>{item.pol} &rarr; {item.pod}</strong>
                      </div>
                      <div>
                        <span style={{ color: '#94a3b8', fontSize: '0.8rem', display: 'block' }}>관측 물동량 (KCS 교차)</span>
                        <strong style={{ color: '#cbd5e1', fontSize: '0.9rem' }}>{item.volume} ({item.hsCode})</strong>
                      </div>
