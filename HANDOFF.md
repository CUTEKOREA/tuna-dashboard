...
> - QR 상한 교정 뒤에도 운영 enroll 400이 재현돼 단계 진단을 추가했다. factor 조회·정리·enroll·QR·URI 단계를 분리하되 Supabase 원문·ID·토큰은 반환하지 않고, 명시적으로 허용한 공식 MFA 오류 코드만 stage에 병기한다. regex-only 코드 반사 finding은 allowlist와 unknown 축소 테스트로 해소했다. 다음 실계정 재시도에서 표시된 진단 코드로 잔여 원인을 확정한다.
>
