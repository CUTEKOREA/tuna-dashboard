...
export default function SquidValueChainMargin() {
  ...
  return (
    <div className={styles.glassCard} ref={containerRef}>
      ...
      <TakeawayBox
        situation="한국산 오징어 고갈로 대왕오징어가 대체재로 떠올랐지만, 중국 내 가공 공장의 인건비 상승(파란 영역)이 원물가(노란 영역) 이상으로 급증하며 최종 수입 단가를 밀어올리고 있습니다. 최근에는 소비자가(붉은 선) 상승이 매입원가(COGS) 인상을 따라가지 못해 국내 상사들의 채산성이 악화되었습니다."
        actionPlan="**[Actionable Insight]** 신라교역은 더 늦기 전에 중국 편중 가공망에서 탈피하여, '베트남' 또는 '인도네시아'로 직접 가공 공장 합작을 이전해야 합니다. 가공 밸류체인(Area)을 직접 통제해야만 막대한 B2C(진미채 등) 도매 마진을 지켜낼 수 있습니다."
      />
    </div>
  );
}
