...
/* ── 영국 가공참치(HS1604) 수입 공급국 점유·관세 비대칭 (2024) ──
   검증(IndexBox·영국 trade-tariff): 에콰도르 31%($168M)·모리셔스14%·세이셸12%,
   태국 등 비특혜국 UK Global Tariff 20%(확정), EPA 무관세 쿼터 10,000t(2024 증액).
...
export default function SasUkSupplierTariff() {
  return (
    <WidgetCard
      ...
      cardDesc="영국 가공참치 수입 공급국별 점유율·적용 관세 — IndexBox 2024 + 영국 공식 trade-tariff"
      takeaway={{
        situation: "영국 가공참치(HS1604) 수입은 무관세 특혜국이 장악합니다. 2024년 에콰도르가 31%($168M, DCTS 특혜 0%)로 1위, UK-ESA EPA로 무관세인 세이셸·모리셔스가 합산 26%를 점유합니다(EPA 캔 무관세 쿼터는 2024년 10,000t으로 증액). 반면 글로벌 캔참치 1위 수출국 태국은 영국과 FTA가 없어 UK Global Tariff 20% 확정세율을 부담합니다. 영국 캔참치 수입은 102K톤·약 $525~539M(+16% YoY) 규모입니다. 즉 '저원가 가공 태국 vs 무관세 특혜국'의 관세 비대칭이 공급국 순위를 결정합니다.",
        ...
        source: "IndexBox UK Preserved/Canned Tuna 2024(공급국 점유·수입규모) / trade-tariff.service.gov.uk(MFN 20%) / UK-ESA EPA Decision 1/2024(무관세 쿼터 10,000t). ※한국 FTA 0%는 SasUkMarket 참조",
      }}
      customBody={
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '14px', fontSize: '0.64rem', color: '#94a3b8', flexWrap: 'wrap' }}>
            ...
            <span style={{ color: '#64748b' }}>캔수입 102K톤 · +16% YoY</span>
          </div>
        </div>
      }
    />
