 ...
 *
 * 발행본이 정본인 이유는 정정이 거기 쌓이기 때문이다. 이 편은 한 이름(골뱅이) 아래 다섯 속이
 * 섞이고, 종 경계가 표마다 머리행·주석에 붙어 있다. 숫자만 다시 계산하면 경계가 떨어진다.
 * 이 편은 정본 md 가 없고 발행본 HTML(2026-09-12 제3판)이 정본이다.
 *
 * 아키텍처 가드: 원본 JSON import 는 이 인테이크 모듈에서만 한다.
 */
import raw from './whelk-tables.json';

export interface WhelkReportTable {
...
interface Stage {
  tables: WhelkReportTable[];
}

const STAGES = raw as Record<string, Stage>;

/** 단계 키(`s01`~`s09`)로 그 단계에 배치된 보고서 표를 가져온다. */
export function getWhelkTables(stage: string): WhelkReportTable[] {
  return STAGES[stage]?.tables ?? [];
}

export function getWhelkTableStages(): string[] {
  return Object.keys(STAGES);
}

/** 검증용 — 발행본에서 옮긴 표 총수. */
export function getWhelkTableCount(): number {
  let n = 0;
