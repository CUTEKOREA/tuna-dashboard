...
function KpiCard({ icon, label, value, sub, color }: any) {
  return (
    <div style={{ background: '#11182f', padding: '1.1rem', borderRadius: '8px', border: `1px solid ${color}33`, borderLeft: `3px solid ${color}` }}>
      <div style={{ fontSize: '0.8rem', color: 'var(--w-slate-400)', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>{icon} {label}</div>
      <div style={{ fontSize: '1.4rem', fontWeight: 700, color }}>{value}</div>
      {sub && <div style={{ fontSize: '0.72rem', color: 'var(--w-slate-500)', marginTop: '3px' }}>{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
        <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', color: 'var(--w-slate-400)', textAlign: 'left' }}>
          {['국가','연식제한','수입관세','부가세','특이사항','매력도'].map(h => <th key={h} style={{ padding: '10px 8px' }}>{h}</th>)}
        </tr></thead>
        <tbody>{data.map((r: any, i: number) => (
          <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
            <td style={{ padding: '10px 8px', color: 'var(--w-slate-200)', fontWeight: 600 }}>{r.country}</td>
            <td style={{ padding: '10px 8px', color: 'var(--w-slate-200)' }}>{r.ageLimit}년</td>
            <td style={{ padding: '10px 8px', color: 'var(--w-slate-200)' }}>{r.importDuty}</td>
            <td style={{ padding: '10px 8px', color: 'var(--w-slate-200)' }}>{r.vat}</td>
            ...
            <td style={{ padding: '10px 8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--w-slate-400)' }}>{r.score}</span>
              </div>
            </td>
          </tr>
        ))}</tbody>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', marginTop: '0.5rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', background: '#11182f', padding: '1.25rem', borderRadius: '8px', border: 'none' }}>
        <div>
          <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--w-slate-400)', marginBottom: '6px' }}>FOB 차량 매입가 (USD)</label>
          <input type="number" value={fobUsd} onChange={e => setFobUsd(Number(e.target.value))} style={{ width: '100%', padding: '10px', background: '#11182f', border: 'none', borderRadius: '8px', color: 'var(--w-slate-200)' }} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--w-slate-400)', marginBottom: '6px' }}>해상 운임 (USD)</label>
          <input type="number" value={freightUsd} onChange={e => setFreightUsd(Number(e.target.value))} style={{ width: '100%', padding: '10px', background: '#11182f', border: 'none', borderRadius: '8px', color: 'var(--w-slate-200)' }} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--w-slate-400)', marginBottom: '6px' }}>배기량 (CC)</label>
          <select value={engineCC} onChange={e => setEngineCC(Number(e.target.value))} style={{ width: '100%', padding: '10px', background: '#11182f', border: 'none', borderRadius: '8px', color: 'var(--w-slate-200)' }}>
        ...
        <div>
          <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--w-slate-400)', marginBottom: '6px' }}>제작 연도</label>
          <input type="number" value={year} onChange={e => setYear(Number(e.target.value))} style={{ width: '100%', padding: '10px', background: '#11182f', border: 'none', borderRadius: '8px', color: 'var(--w-slate-200)' }} />
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(var(--w-violet-500-rgb), 0.05)', padding: '1.5rem', borderRadius: '8px', border: 'none' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
          <span style={{ color: 'var(--w-slate-400)', fontSize: '0.85rem' }}>기본 수입 관세 ({(importDutyRate*100).toFixed(0)}%)</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
          <span style={{ color: 'var(--w-slate-400)', fontSize: '0.85rem' }}>연식 초과 페널티 ({age}년 경과, {(agePenaltyRate*100).toFixed(0)}%)</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', borderBottom: '1px dashed rgba(255,255,255,0.1)', paddingBottom: '12px' }}>
          <span style={{ color: 'var(--w-slate-400)', fontSize: '0.85rem' }}>기타 제세공과금 (VAT 15%, NHIL 2.5%, 등)</span>
...
export default function UsedCarExport() {
  ...
  if (!data) return <div style={{ padding: '2rem', color: 'var(--w-slate-400)', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60vh' }}>데이터를 불러오는 중입니다...</div>;
        ...
        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <div style={{ flex: 1, minWidth: '300px' }}>
            ...
            <div style={{ background: '#11182f', borderRadius: '8px', padding: '1rem', border: 'none' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--w-slate-300)', fontWeight: 600, marginBottom: '8px' }}>🔥 현지 Top 3 인기 모델 (스크래핑 데이터)</div>
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: '140px' }}>
                  <div style={{ color: '#60a5fa', fontSize: '0.75rem', fontWeight: 'bold' }}>Hyundai</div>
                  <ul style={{ margin: '4px 0 0', paddingLeft: '1.2rem', color: 'var(--w-slate-400)', fontSize: '0.8rem' }}>
                ...
                <div style={{ flex: 1, minWidth: '140px' }}>
                  <div style={{ color: 'var(--w-amber-400)', fontSize: '0.75rem', fontWeight: 'bold' }}>Kia</div>
                  <ul style={{ margin: '4px 0 0', paddingLeft: '1.2rem', color: 'var(--w-slate-400)', fontSize: '0.8rem' }}>
        ...
        {arbitrageRadar && arbitrageRadar.arbitrageOpportunities && (
          <div style={{ marginTop: '1rem', background: 'rgba(0,0,0,0.3)', borderRadius: '10px', padding: '1.25rem', border: 'none' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <h4 style={{ margin: 0, fontSize: '1rem', color: 'var(--w-emerald-400)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', color: 'var(--w-slate-400)', textAlign: 'left' }}>
                ...
                <tbody>
                  {arbitrageRadar.arbitrageOpportunities.map((opp: any, idx: number) => (
                    <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <td style={{ padding: '10px 8px', color: 'var(--w-slate-200)', fontWeight: 600 }}>{opp.model}</td>
                      <td style={{ padding: '10px 8px', color: 'var(--w-slate-400)' }}>${opp.fobKorea.toLocaleString()}</td>
                      <td style={{ padding: '10px 8px', color: 'var(--w-slate-400)' }}>${opp.retailGhana.toLocaleString()}</td>
      ...
      <div data-mobile-stack style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem' }}>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: 'var(--w-slate-400)', lineHeight: 1.5 }}>
          ...
          <div style={{ marginTop: '1rem', padding: '1rem', background: 'rgba(var(--w-red-500-rgb), 0.05)', border: 'none', borderRadius: '8px' }}>
            ...
            <div style={{ color: 'var(--w-slate-400)', fontSize: '0.82rem', lineHeight: 1.5 }}>
      ...
      <div data-mobile-stack style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {[
              ...
              <div key={i} style={{ padding: '1rem', background: '#11182f', borderRadius: '10px', borderLeft: `3px solid ${hub.color}` }}>
                <div style={{ fontWeight: 600, color: 'var(--w-slate-200)', fontSize: '0.9rem', marginBottom: '4px' }}>{hub.flag} {hub.name}</div>
                <div style={{ color: 'var(--w-slate-400)', fontSize: '0.82rem', lineHeight: 1.5 }}>{hub.desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: 'var(--w-slate-400)', lineHeight: 1.5 }}>
          ...
          <div style={{ padding: '1.25rem', background: '#11182f', borderRadius: '8px', border: 'none', marginBottom: '1rem' }}>
            <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--w-emerald-400)', marginBottom: '8px' }}>🏭 아프리카 최초 대규모 EV 공장 (2025.1 체결)</div>
            <ul style={{ margin: 0, paddingLeft: '1.2rem', color: 'var(--w-slate-400)', fontSize: '0.82rem', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <li>나이지리아 정부 × 한국 AEDC 파트너십 체결</li>
              <li>연간 <strong style={{ color: 'var(--w-slate-200)' }}>30만 대</strong> 생산 능력 EV 공장 + 전국 충전 인프라</li>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div data-mobile-stack style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '20px' }}>
          ...
          <div style={{ background: '#11182f', border: 'none', borderRadius: '8px', padding: '1.25rem', borderTop: '3px solid var(--w-amber-500)' }}>
            ...
            <ul style={{ margin: 0, paddingLeft: '1rem', color: 'var(--w-slate-300)', fontSize: '0.82rem', lineHeight: 1.7 }}>
              <li>한국 매입가 <strong>$2,000~5,000</strong> → 가나 현지 판매가 <strong style={{ color: 'var(--w-amber-400)' }}>$5,500~9,000</strong></li>
