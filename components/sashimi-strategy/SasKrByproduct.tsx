...
/* ── 한국 참치 가공수율·부산물 밸류업 격차 (FAO·NIFS) ──
   검증: 통조림 가공수율 50~55%(살코기 ~40%), 부산물 40~45%.
   동원 창원 ~200톤/일(실재·검증). 매스밸런스 구성비는 업계 통념 기반 자체 도식(1차 정량 출처 미특정).
...
export default function SasKrByproduct() {
  return (
    <WidgetCard
      ...
      takeaway={{
        situation: "한국 통조림 참치는 원물의 50~55%만 살코기로 제품화되고, 나머지 40~45%(머리·뼈·내장·적육·자숙액 등)는 대부분 사료·어분 등 저가 처분으로 빠집니다(일 300kg 초과 시 산업폐기물로 분류돼 처리비 발생). 아래 스트림별 구성비는 업계 통념 기반 자체 도식으로, 가공사·어종별로 달라 1차 정량 출처는 특정하지 않았습니다. 동원은 창원 가공장에서 일 약 200톤 규모를 처리하며 자숙액→참치액(훈연추출물), 적육→펫푸드로 일부를 밸류업하고 머리·뼈의 펩타이드·콜라겐·어골칼슘 사업을 예고했으나, 한국 가공사는 부산물 회계·통계조차 부재해 회수율 격차의 잠재 마진이 가시화되지 않습니다.",
        ...
        source: "FAO Fish Canning Processing(가공수율 50~55%) / NIFS·KMI(부산물 비중 40~45%) / 동원 창원 가공장 약 200톤/일(실재·검증) / 스트림별 구성비는 업계 통념 기반 자체 도식(1차 정량출처 미특정 — Springer ESPR 2023 명의도용 및 FMI 시장규모 귀속은 audit 2026-06-04로 제거)",
      }}
      customBody={
