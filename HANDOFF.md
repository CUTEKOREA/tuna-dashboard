...
> - 검증: 신규+기존 하역 회귀 테스트 **10/10**, 타입검사, ESLint 0 errors(기존 warnings 10), Next.js 전체 빌드 103페이지, 로컬 API 및 Playwright 데스크톱 시각 확인 통과. 전체 테스트의 기존 `/market` 기대값 실패 2건은 기준 커밋 `0ff6fcf`에서도 동일하며 미수정. 미배포(로컬).
