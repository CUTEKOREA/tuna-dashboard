 ...
 *
 * 발행본이 정본인 이유는 정정이 거기 쌓이기 때문이다. 2026-09-14 에 한·러 쿼터의
 * 2층 구조(1차 배정 / 추가 신청 한도)를 반영하면서 「2026년에 처음 줄었다」를
 * 철회했는데, 대시보드가 숫자를 다시 계산하면 그 정정 이전 값으로 되돌아갈 길이 생긴다.
 *
 * 아키텍처 가드: 원본 JSON import 는 이 인테이크 모듈에서만 한다.
 */
import raw from './pollock-tables.json';

export interface PollockReportTable {
...
interface Stage {
  tables: PollockReportTable[];
}

const STAGES = raw as Record<string, Stage>;

/** 단계 키(`s01`~`s09`)로 그 단계에 배치된 보고서 표를 가져온다. */
export function getPollockTables(stage: string): PollockReportTable[] {
  return STAGES[stage]?.tables ?? [];
}

export function getPollockTableStages(): string[] {
  return Object.keys(STAGES);
}

/** 검증용 — 발행본에서 옮긴 표 총수. */
export function getPollockTableCount(): number {
  let n = 0;
