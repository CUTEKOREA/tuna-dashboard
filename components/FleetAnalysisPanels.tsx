...
const rankData = [
  { r: 1, cap: '조태연', name: 'N/STAR', weekly: 190, daily: 27.14, badge: 'gold' },
  { r: 2, cap: '공준식', name: 'S/EXP', weekly: 145, daily: 20.71, badge: 'silver' },
  { r: 3, cap: '최용석', name: 'S/CHA', weekly: 145, daily: 20.71, badge: 'bronze' },
  { r: 4, cap: '김승현', name: 'S/PIO', weekly: 139, daily: 19.86, badge: '' },
  { r: 5, cap: '이평규', name: 'KONA', weekly: 88, daily: 12.57, badge: '' },
  { r: 6, cap: '김정훈', name: 'MARI', weekly: 75, daily: 10.71, badge: '' },
  { r: 7, cap: '김효원', name: 'S/SPR', weekly: 43, daily: 6.14, badge: '' },
  { r: 8, cap: '김형주', name: 'N/SUN', weekly: 40, daily: 5.71, badge: '' },
  { r: 9, cap: '오복근', name: 'S/HAR', weekly: 0, daily: 0, badge: '' },
    ...
    <div className={s.analysisGrid}>
      ...
      <div className={s.chartPanel}>
        <div className={s.chartTabs}>
          ...
          <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginLeft: 'auto', alignSelf: 'center' }}>26.07.05 보고 기준</span>
        </div>
        {activeTab === 'weekly' && (
          <>
            <WeeklyCatchChart />
            <div style={{ marginTop: 16 }}>
              <TakeawayBox
                situation={<>N/STAR(조태연) 190t 주간 1위. S/EXP(공준식)·S/CHA(최용석) 각 145t으로 공동 2위. 주간 총 어획량 865t(국적 472t, 합작 393t) 기록.</>}
                actionPlan={<>S/EXP 공준식 신임 선장 주간 2위 호실적 도약. S/HAR(오복근), S/JUP(강창훈) 주간 실적 없음 — 수리 및 대기 중.</>}
              />
            </div>
          </>
        )}
        {activeTab === 'monthly' && (
          <>
            ...
            <div style={{ marginTop: 16 }}>
              <TakeawayBox
                situation={<>N/STAR 7월 190t, S/CHA 130t, S/EXP 125t 기록. 7월 월간 총 어획량 637t(국적 324t, 합작 313t).</>}
                actionPlan={<>태평양 연간 누계 40,166t(국적 24,385t, 합작 15,781t). S/SPR 연간 6,115t으로 선단 최다.</>}
              />
            </div>
          </>
        )}
        {activeTab === 'cumulative' && (
          <>
            ...
            <div style={{ marginTop: 16 }}>
              <TakeawayBox
                situation={<>조태연(N/STAR) 일어획 35.9t으로 현어기 누계 전체 1위 유지. 김효원(S/SPR) 29.8t 2위, 김승현(S/PIO) 23.4t 3위.</>}
                actionPlan={<>S/EXP 공준식 신임 선장 일어획 14.1t으로 9위→상승세. 오복근(S/HAR) 8일차 어획 0t — 수리 후 조업 대기.</>}
              />
            </div>
          </>
        )}
