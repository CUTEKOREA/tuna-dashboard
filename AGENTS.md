...
이 저장소에서 작업하는 4개 AI 코딩 에이전트(**Claude Code · Codex · opencode go · Grok**)를 위한
...
**[`docs/agent/feature-map.md`](docs/agent/feature-map.md) 를 먼저 읽으세요.**
어느 주소로 가면 그 화면이 나오는지, 어디서 발을 헛디디는지가 거기 있습니다.
코드를 뒤져 라우트를 짐작하지 마세요 — 폴더가 있어도 `notFound()` 한 줄인 은퇴 화면이 19개입니다.

- 맵의 자동 구역은 `node scripts/feature_map.mjs --write` 가 코드에서 뽑습니다.
- 화면이 늘거나 줄었는데 맵이 그대로면 **`npm run verify` 가 실패합니다**(`check:feature-map`).
- 「반영했습니다」로 끝내지 말고 **증거를 남기세요**: `node scripts/verify_live.mjs /화면` 이
  배지 수·신선도 분포·스크린샷을 남깁니다.

## 에이전트 역할 분담

네 에이전트는 강점이 달라 담당 작업이 나뉩니다. 커밋 태그는 이 표를 따릅니다.
...
| **Grok** | `[Grok]` | 빠른 응답, 대안 관점 | **교차 검증·세컨드 오피니언.** 나머지 셋이 막혔을 때 다른 접근 시도, 결론 반증 |
...
2. 커밋 메시지 끝에 「에이전트 역할 분담」 표의 식별자를 붙입니다 — `[CC]` / `[Codex]` / `[OC]` / `[Grok]`.
   **자기 태그만 사용하고 남의 태그를 쓰지 마세요.** git 이력에는 `[AG]`(Antigravity) ·
   `[OMO]`(OpenCode+OMO 실험) · `[Hermes]` 태그도 남아 있습니다. **과거 기록일 뿐 현재 운영
