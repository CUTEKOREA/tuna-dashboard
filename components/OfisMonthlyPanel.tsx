...
function OfisTip({
  ...
  return (
    <div style={{
      ...
      <div style={{ color: '#c6c9d2', marginBottom: 4, fontWeight: 700 }}>{label}</div>
      {payload.map((entry) => (
        <div key={String(entry.name)} style={{ color: '#fff' }}>
    ...
    <section aria-labelledby="ofis-monthly-title" style={{ marginBottom: 40 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <WidgetCard
          ...
          kpiPanel={[
            { label: '6월 합계', value: `${fmt(OFIS_MONTH_TOTAL_T)} 톤`, sub: `전년 동월 ${fmtPct(ofisHeadline.monthYoyPct)}`, trendColor: '#f87171' },
            { label: '1~6월 누계', value: `${fmt(OFIS_YTD_TOTAL_T)} 톤`, sub: `전년 상반기 ${fmt(OFIS_PRIOR_YTD_T)} · ${fmtPct(ofisYtdYoyPct)}`, trendColor: '#fbbf24' },
            { label: '6월 선망', value: `${fmt(OFIS_PS_MONTH_T)} 톤`, sub: `전년 동월 ${fmtPct(ofisTuna.purseSeine.monthYoyPct)}`, trendColor: '#f87171' },
            { label: '6월 연승', value: `${fmt(OFIS_LL_MONTH_T)} 톤`, sub: `전년 동월 ${fmtPct(ofisTuna.longline.monthYoyPct)}`, trendColor: '#34d399' },
          ]}
          chartHeight={260}
          chart={(
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={volumeCompareRows} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
                ...
                <XAxis dataKey="구분" tickFormatter={truncateXAxis} tick={{ fill: 'rgba(255,255,255,0.62)', fontSize: 12 }} />
                <YAxis tick={{ fill: 'rgba(255,255,255,0.45)', fontSize: 11 }} />
                <Tooltip content={<OfisTip unit="톤" />} />
                <Legend />
                <Bar dataKey="평년" name="평년 5년" fill="#64748b" radius={[4, 4, 0, 0]} />
                <Bar dataKey="전년" name="2025년 6월" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                <Bar dataKey="올해" name="2026년 6월" fill="#38bdf8" radius={[4, 4, 0, 0]} />
              </BarChart>
        ...
        <WidgetCard
          ...
          telemetry={{ status: 'STATIC', syncDate: ofisMeta.published, label: 'OFIS 월보' }}
          kpiPanel={[
            { label: '가다랑어', value: `${fmt(skipjack?.current ?? 0)} 원/kg`, sub: `전년 6월 ${fmtPct(skipjack?.yoyPct ?? 0)}`, trendColor: '#94a3b8' },
            { label: '눈다랑어', value: `${fmt(bigeye?.current ?? 0)} 원/kg`, sub: `전년 6월 ${fmtPct(bigeye?.yoyPct ?? 0)}`, trendColor: '#34d399' },
            { label: '황다랑어', value: `${fmt(yellowfin?.current ?? 0)} 원/kg`, sub: `전년 6월 ${fmtPct(yellowfin?.yoyPct ?? 0)}`, trendColor: '#f87171' },
          ]}
          chartHeight={260}
          chart={(
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={tunaPriceCompareRows} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.08)" />
                <XAxis dataKey="어종" tickFormatter={truncateXAxis} tick={{ fill: 'rgba(255,255,255,0.62)', fontSize: 12 }} />
                <YAxis tick={{ fill: 'rgba(255,255,255,0.45)', fontSize: 11 }} />
                <Tooltip content={<OfisTip unit="원/kg" />} />
                <Legend />
                <Bar dataKey="전년" name="2025년 6월" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                <Bar dataKey="올해" name="2026년 6월" fill="#fbbf24" radius={[4, 4, 0, 0]} />
              </BarChart>
            </SafeResponsiveContainer>
          )}
          takeaway={{
        ...
        <WidgetCard
          ...
          telemetry={{ status: 'STATIC', syncDate: ofisMeta.published, label: 'OFIS 월보' }}
          kpiPanel={[
            { label: '태평양', value: `${fmt(pacific?.month ?? 0)} 톤`, sub: `전년 동월 ${fmtPct(pacific?.monthYoyPct ?? 0)}`, trendColor: '#f87171' },
            { label: '대서양', value: `${fmt(atlantic?.month ?? 0)} 톤`, sub: `전년 동월 ${fmtPct(atlantic?.monthYoyPct ?? 0)}`, trendColor: '#f87171' },
            { label: '인도양', value: `${fmt(indian?.month ?? 0)} 톤`, sub: `전년 동월 ${fmtPct(indian?.monthYoyPct ?? 0)}`, trendColor: '#34d399' },
            { label: '남빙양', value: `${fmt(southern?.month ?? 0)} 톤`, sub: `전년 동월 ${fmtPct(southern?.monthYoyPct ?? 0)}`, trendColor: '#34d399' },
          ]}
          chartHeight={260}
          chart={(
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={oceanCompareRows} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.08)" />
                <XAxis dataKey="해역" tickFormatter={truncateXAxis} tick={{ fill: 'rgba(255,255,255,0.62)', fontSize: 12 }} />
                <YAxis tick={{ fill: 'rgba(255,255,255,0.45)', fontSize: 11 }} />
                <Tooltip content={<OfisTip unit="톤" />} />
                <Legend />
                <Bar dataKey="전년" name="2025년 6월" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                <Bar dataKey="올해" name="2026년 6월" fill="#a78bfa" radius={[4, 4, 0, 0]} />
              </BarChart>
            </SafeResponsiveContainer>
          )}
          takeaway={{
