...
describe('FleetCommandCenter daily operations', () => {
  it('renders the latest daily report as the hero KPI source', () => {
    ...
    expect(markup).toContain('2026-08-25 보고 · 2026-08-24 조업 기준');
    expect(markup).toContain('data-kpi-value="555"');
    expect(markup).toContain('data-kpi-value="7728"');
    expect(markup).toContain('data-kpi-value="79285.8"');
  ...
  it('renders public deltas and fail-closed quality coverage without private schedules', () => {
    ...
    for (const value of [
      '+84 (MT)', '+155 (MT)', '+239 (MT)', 'SYNCED',
      '전체 보고 140건', '전기간 검산 560회', '완전 검산 560회', '미보고 포함 0회 / 0문서',
