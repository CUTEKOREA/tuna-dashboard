...
> 🚀 **2026-09-08 — `/fleet` 260908 반영 + 검산 허용 폭 배포. 시크릿 교체 순서 정정** [CC]:
> - PR [#951](https://github.com/CUTEKOREA/tuna-dashboard/pull/951) → main `c8ea55a4`, PR [#956](https://github.com/CUTEKOREA/tuna-dashboard/pull/956) → main `0069c328`. Production 배포 READY, alias `leedonggun.co.kr`, error log 0건.
> - 라이브 실측: `2026-09-08 보고 · 2026-09-07 조업 기준` · 일간 75 · 월간 1,855 · 연간 83,555.8 · 7,684.1 / 3,821.7 ·
>   「최신 상세 행 검산 일치」 · 9월 첫째주 주간 실적 · 8/31 차이 문장. 전부 확인.
>
> ⚠️ **`FLEET_DAILY_DETAIL_JSON` 교체는 병합 «전»이 아니라 «후»다. 이 문서의 옛 안내가 반대로 적혀 있었다.**
>
> - 이번에 옛 안내대로 병합 전에 교체했더니 **라이브에 「선박 상세 보호 — 선박 상세 데이터가 공개 집계와 맞지 않습니다」가 떴다.**
> - 이유: `detailSha256Compat` 은 **앞으로만** 호환된다. 새 공개 집계가 «직전» 해시를 compat 에 담으므로
>   **새 코드는 {신규, 직전} 둘 다 받고, 옛 코드는 {직전, 그 전} 만 받는다.**
>   병합 전에 시크릿을 신규로 바꾸면 아직 떠 있는 옛 코드가 그 값을 모른다.
> - **올바른 순서**: ① PR 병합 → ② Production 배포 READY 확인(새 코드는 옛 시크릿을 compat 으로 계속 받는다)
>   → ③ `FLEET_DAILY_DETAIL_JSON` 을 신규 DTO 로 교체 → ④ 보호 패널 확인.
> - 교체 전 canonical SHA 대조는 그대로 필수다 — `json.dumps(sort_keys=True, separators=(',',':'))` 해시가
>   공개 집계의 `_meta.detailSha256` 과 같아야 한다. **파일 raw 해시와는 다르다.**
