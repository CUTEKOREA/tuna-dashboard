...
const TakeawayBox = ({ id }: { id: string }) => {
  const takeawaysMap: Record<string, any> = {
    "enso": {
      ...
      "short": "엘니뇨 예보 시 중태평양(키리바시·마샬) 조업 허가권 선제 확보, 라니냐 전환 시 서태평양(PNG·솔로몬) 복귀 전략 병행.",
      "long": "표층 수온(SST) AI 모델을 도입하여 ENSO 국면별 최적 해역 자동 추천 및 선단 연료 소모율 15% 감축 극대화.",
      "risk": "엘니뇨/라니냐 전환 타이밍 예측 실패 시 빈 해역 조업으로 수익 손실 발생. 태풍 빈도 상승 시 대피 매뉴얼 점검 및 해상 통신 인프라(스타링크) 확보."
    },
    "fad": {
      ...
      "short": "생분해성 FAD 우선 도입으로 유럽/미주 리테일 바이어 계약 연장.",
      "long": "에코사운더 결합 분석으로 타겟 초정밀 탐색 시스템 내재화.",
      "risk": "FAD 유실률(Loss rate) 통제로 해양 오염 부과금 페널티 방어."
    },
    "port": {
      "source": "Global Port Demurrage DB & WCPO Unloading Log & Drewry Reefer Index",
      "situation": "7~9월 금어기(FAD Closure) 시즌은 하역 대기일(최대 14일)과 초저온 운반선 운임(최대 $7,200/FEU)이 동시에 폭증하는 '이중 병목' 구간입니다. 이 기간 대기 손실만 8월 최고 $850K에 이르며, 비수기(4월: 대기 3일, $4,800/FEU) 대비 총비용이 3~4배 팽창합니다. 금어기 직전 물량 집중과 콜드체인 수요 폭증이 겹치면서 체선료(Demurrage)가 하루 $10K 이상 발생하는 위험 구간입니다.",
      "short": "금어기 2개월 전 셔틀 선박 용선 완료 및 양륙 터미널·냉동 플러그 장기 임대 조기 락인.",
      "long": "마주로·방콕 등 주요 허브의 냉동 창고 지분 투자 + 직행 콜드 셔틀(안데스 ↔ 아시아) 통합 하역 인프라 내재화.",
      "risk": "체선료 폭등 + 운임 스파이크 동시 발생 시 대체 하역항(필리핀 등) 사전 발굴 및 하루 $10K 페널티 시뮬레이션 모델 가동."
    },
    "em_reg": {
      ...
      "short": "IUU 리스크 해역부터 카메라/전자 모니터링 시범 탑재 우선 지시.",
      "long": "불법 어획 적발 방어를 넘어 추적성(Traceability) 상품 라인업 확대로 프리미엄 마진 획득.",
      "risk": "불시 항만국 통제(PSC) 시 무결점 방어망으로 벌금 및 수출 차단 완전 회피."
    },
    "msc": {
    ...
    "pouch": {
      ...
      "situation": "전통 물/오일 캔은 성장이 정체(-2.5%)된 반면, 프리미엄 파우치(+14.5%)와 RTE 샐러드 팩(+22.0%)은 폭발적 성장 중입니다. 마진율로 보면 전통 캔(8%) 대비 RTE 팩(35%)은 4배 이상의 매출총이익을 달성하고 있어, 생산 라인 전환의 시급성이 데이터로 확인됩니다.",
      "short": "고마진 프리미엄 파우치 팩(갈릭/올리브 튜나) 생산 라인 용량 2배 즉각 증설.",
      "long": "헬스/단백질 소비 문화 트렌드를 타고 피트니스 레벨의 B2C 구독망 침투 계획.",
      "risk": "플라스틱 사용 규제(환경 부담금) 시뮬레이션으로 포장재 전환점 사전 조율."
    },
    "blackhole": {
      ...
      "short": "어가 반등 뉴스 캐치 시 즉시 2개월치 물량 창고 락인(Wait & Hold) 실행.",
      "long": "미국/EU 메이저 수입상의 재고 소진 시기(수요 공백기)를 계산하여 저점 매수 고점 방출.",
      "risk": "수입상 재고 조기 포화 시 데드캣 바운스 전 빠르게 덤핑/투매하는 역학 밸런스 통제."
    },
    "rival": {
      "source": "Global Seafood Major 3 Annual Reports (Maruha, Thai Union, Dongwon)",
      "situation": "글로벌 메이저 3사의 포트폴리오를 보면, Maruha는 이미 양식업(45%) 중심으로 전환 완료했고, Thai Union은 가치 부가(45%) 비중이 압도적입니다. 반면 Dongwon은 참치 비중이 55%로 단일 어종 의존도가 여전히 높아, 어가 폭락 사이클에 가장 취약한 구조임을 보여줍니다.",
      "short": "Maruha, Thai Union 등 메이저사의 비참치(Non-tuna)/양식업 지분 매입 흐름 추적.",
      "long": "순수 조업/가공 밸류체인을 넘어선 수직계열화(CVC 투자 병행)로 캐시카우 다변화.",
      "risk": "어가 폭락 사이클 진입 시, 단일 어종 의존도가 높은 기업은 흑자 부도로 직행할 수 있음을 경계."
    },
    "quota": {
      ...
      "short": "빅아이/옐로핀 차년도 삭감 가능성 대비 선제적 대체 해역 조업권 매집 시뮬레이션 가동.",
      "long": "할당량 기반 매매(Trading) 시장에서 장기 계약 기반의 쿼터 락인 옵션 조기 매수.",
      "risk": "쿼터 초과 시 징수되는 페널티 모델과 잉여 이익 간의 최적화 손익 분기점 확보."
    },
    "labour": {
      ...
      "short": "미 세관(CBP) 강제 노동 관련 억류를 피할 추적 가능 인증서 블록체인 체계 연동.",
      "long": "벤더사 및 인력 송출국 대상 분기별 자체 감사(Audit) 조직 신설로 리스크 내재화 방지.",
      "risk": "표적 캠페인에 의한 글로벌 NGO의 어선 블랙리스트 지정 가능성 원천 차단."
    },
    "ai_bep": {
      ...
      "short": "2025년 크로스오버 기점에서 전 선대에 스타링크 해상망 연동 AI 카메라 설치 돌입.",
      "long": "인공지능 어구 투척 최적화 알고리즘 기반 수확량 증대 딥러닝 고도화 달성.",
      "risk": "인간 옵서버 단절 시 비상 사태 대처가 즉각적이지 못하므로 육상 대기조 통신망 확보 강화."
    },
    "margin": {
      "source": "Super Freezer Premium Index & Fleet Fuel Cost Analytics & FAO/FFA Fleet Economics",
      "situation": "차트는 글로별 상위권 연승 업체(일본·대만 초저온 선단)의 사시미급 프리미엄 마진 추세를 보여줍니다. 단, 신라교역의 연승 사업이 적자인 것은 이 추세와 모순되지 않습니다. 핵심 괴리는 (1) 노후화된 선단의 연료 비효율, (2) -60°C 초저온 콜드체인 미비로 사시미 프리미엄 단가를 몇 수용하지 못하는 점, (3) 시장 직거래 대신 중간 중개상 납품으로 단가가 20~30% 할인되는 구조입니다. 즉, 마진 역전의 수혜는 초저온 시설+사시미 직거래망을 갖춤 업체에만 해당되며, 그렇지 못한 연승은 오히려 선망보다 나쁜 마진 구조에 갇힐니다.",
      "short": "우리 연승 선단의 초저온(-60°C) 콜드체인 능력 긴급 진단: 사시미 프리미엄 단가 획득이 불가하면 연승 철수/축소 결단 병행.",
      "long": "연승 유지 시: Super Freezer 설비 투자 + 일본 초소거 경매 시장 직거래망 구축으로 마진 역전 구간 진입. 철수 시: 선망 전용 스킵잭 증산에 외력 집중.",
      "risk": "현 연승 적자 지속 시 매년 손실이 누적되는 구조: 철수 vs Super Freezer 투자의 BEP(Break-Even Point) 시뮬레이션 즉각 수행 필수."
    },
    "finance": {
      ...
      "long": "해양 크레딧 배출권 시장 진출 시뮬레이션을 통해 유럽 택스 스왑 헤지 적용.",
      "risk": "향후 탄소 발자국 증빙 추적 실패 시 유럽 CBAM(탄소 국경세)으로 인한 수출 진입장벽 폭발 방어."
    },
    "gsp": {
      ...
      "short": "아프리카, 태평양 도서국 등 0% 관세 혜택 국가로의 1차 가공(Loin) 아웃소싱 테스트 오더.",
      "long": "태국(24% 관세) 대비 물류비(3%) 증가에도 불구하고 21% 마진 차익 확보를 위한 허브 재배치.",
      "risk": "정치적 이슈로 GSP+ 혜택 강제 박탈 시 즉각 대체 가능한 허브(필리핀 등) 투트랙 유지."
    },
    "byproduct": {
      "source": "Global Pet Food Margin Report & Fishmeal Byproduct Valorization DB",
      "situation": "참치캔 본품의 영업이익률(8%)은 원가 한계에 부딪힌 반면, 펫푸드(25%)와 오메가-3 오일(55%)은 동일 원재료(잔여 뼈/내장)에서 3~7배의 마진을 창출합니다. 현재 15%에 달하는 잔여물을 100% 활용할 경우 폐기비용 제로화와 함께 연간 신규 매출원이 발생하는 이중효과를 기대할 수 있습니다.",
      "short": "잔여 뼈/내장의 15%를 직접 펫푸드(Pet Care) 라인으로 이동시켜 폐기비용 즉각 0원 달성.",
      "long": "어분에 특화된 라인을 넘어 초프리미엄 펫푸드 자회사 인수합병(M&A) 전략 검토.",
      "risk": "원물 마진(8%) 한계를 뷰티/펫(30%~50%)로 방어하여 어상폭락 사이클 완충 파이프라인 생성."
    },
    ...
    "albacore": {
      ...
      "short": "스킵잭의 '건강한 단백질' 브랜딩 강화: 알바코어 대비 가격경쟁력+단백질 함량 우위 마케팅으로 북미 시장 방어.",
      "long": "스킵잭 원물의 RTE(즉석섭취) 파우치/샐러드 등 고부가가치 가공품 전환으로 단순 캔 vs 알바코어 직접 경쟁을 회피하고 신시장 개척.",
      "risk": "선망으로 알바코어 타겟 조업 사실상 불가 → 원물 조달(OEM 매입) 또는 남태평양 연승·트롤 협력사 파트너십으로 라인업 확보 검토."
    },
    "foodservice": {
      "source": "Global Foodservice vs Retail Seafood Consumption Trends",
      "situation": "24-1Q에는 외식(Foodservice) 수요가 120으로 소매 80을 압도했으나, 고금리 장기화로 25-1Q에는 외식이 80으로 급감하고 소매(통조림 B2C)가 140으로 역전되었습니다. 이 '수요 스위칭'은 경기침체기마다 반복되는 패턴으로, 외식 채널 의존도가 높은 기업은 즉각적인 채널 전환 없이 매출 40% 감소를 맞이하게 됩니다.",
      "short": "물가고로 외식(레스토랑) 수요가 끊길 때, 소매(통조림) B2C 판촉 예산 150% 선제 투입.",
      "long": "외식망 냉동 필릿 등 블록형 원물 계약을 취소하고 가공 파우치로 즉시 우회 돌릴 수 있는 가변 계약화.",
      "risk": "소매가 동결 시 이익 압박을 막기 위해 슈링크플레이션(무게 미세 감소) 적용 테스트 발동."
    },
    "hub": {
      ...
      "short": "바트화 전면 강세 및 인력난 대처를 위해 베트남/에콰도르 하역 턴어라운드 허브 우선 선제 적용.",
      "long": "동남태평양 원물 직행 코스(에콰도르 만타 포트) 다이렉트 OEM 활성화 구축.",
      "risk": "안데스/신규 허브 초기 수율 문제(클레임) 통제를 위해 자사 품질 감리관 상주 조직 체계 확보."
    },
    "altseafood": {
      ...
      "short": "배양 등 비건 참치 샌드위치가 북미 시장을 잠식하는 투입 속도 시장 척도 B2C 스킨십 파일럿 도입.",
      "long": "자생적 개발 10년 투입보다, 시장 리더형 스타트업 지분참여(CVC Buy-out)로 생존형 포트폴리오 편입.",
      "risk": "식물성 참치 시장 20% 점유 사이클(2030) 도달 시 폭발하는 전통 조업 감가상각 가치 훼손 헤지 전술."
    },
    "species_dom": {
      ...
      "short": "눈다랑어 쿼터가 줄어드는 구간에서 황다랑어 연승(Longline) 비중을 30% 이상 확대하여 톤당 마진을 극대화.",
      "long": "가다랑어 의존 포트폴리오에서 탈피하여 프리미엄 어종(황다랑어·알바코어) 비중을 40%로 올리는 5개년 선대 재편 로드맵 수립.",
      "risk": "눈다랑어 자원량 지속 감소 시 RFMO 쿼터 전면 삭감 가능성을 대비한 대체 어종 전환 파이프라인 확보."
    },
    "area_exhaust": {
      "source": "FAO Catch Area Density & CPUE Long-term Trends",
      "situation": "9개 FAO 주요 해역의 75년간 어획량 변동에서, 서중태평양(WCPO)이 358만 톤으로 전체의 약 50%를 차지하는 절대 강자입니다. 그러나 동태평양(58만 톤), 동인도양(58만 톤)이 빠르게 성장하고 있으며, 대서양 중서부는 2.6만 톤으로 사실상 고갈 해역입니다. 해역별 CPUE 밀도 포화도가 향후 입어권 전략의 핵심 변수입니다.",
      "short": "WCPO 포화 해역 의존도를 60%에서 45%로 감축하고, 동인도양·동태평양 입어권 조기 매입 교섭 착수.",
      "long": "FAO 해역별 CPUE 트렌드 데이터를 분기별 업데이트하여, 해역 분산 최적화 알고리즘 기반의 선단 배치 자동화 시스템 구축.",
      "risk": "대서양 중서부 수준의 자원 고갈이 인도양으로 전이될 가능성에 대한 조기 경보 체계 수립."
    },
    "hegemony": {
      ...
      "short": "인도네시아 어선 오너와의 합작 JV(조인트벤처) 형태로 WCPO 내 조업 거점을 확보하여 원가 경쟁력 확보.",
      "long": "한국 수산업의 글로벌 3위권 유지를 위해 필리핀·파푸아뉴기니와의 어업협정 갱신 우선 교섭 리스트 관리 체계화.",
      "risk": "대만과의 생산 격차 역전 시 RFMO 내 투표권 및 쿼터 배분에서 불리해질 수 있는 외교적 리스크 선제 대응."
    },
    "premium_cross": {
      ...
      "short": "양식 블루핀 가격 하락기에 일본향 자연산 눈다랑어의 '야생(Wild-Caught) 프리미엄' 마케팅 강화로 가격 방어.",
      "long": "참다랑어 양식 5대국(일본·호주·멕시코·몰타·스페인)의 생산량 모니터링 체계를 구축하여 가격 교차점 예측 모델링.",
      "risk": "양식 블루핀 가격이 자연산 눈다랑어 이하로 떨어질 경우, 사시미 시장 전체의 마진 압축을 방어할 헤지 포지션 확보."
    },
    "aqua_disrupt": {
      ...
      "long": "자사 양식 R&D 또는 양식 기업 지분 투자를 통해 '조업+양식' 하이브리드 포트폴리오를 구성하는 중기 전략 수립.",
    ...
    "korea_radar": {
      ...
      "short": "호주산 급감의 원인(SBT 수출 정책, 가격 경쟁력 변동)을 긴급 분석하고, 대체 공급원(몰타, 크로아티아)을 사전 확보.",
      "long": "한국 수입 시장의 단가 추이(2019년 $13.3/kg → 2023년 $14.6/kg)에 맞춘 가격대별 최적 원산지 매칭 알고리즘 구축.",
      "risk": "튀르키예·스페인 양대 공급원에 과도하게 의존하는 구조를 개선하여 지정학 리스크(EU-터키 관계) 방어."
    },
    "import_bh": {
      ...
      "long": "일본 의존도 50% 이상인 수출사의 리스크를 분산하기 위해 미국 프리미엄 시장(포케/스시바) 진출 로드맵 수립.",
      "risk": "일본 경기 침체기에 수입 블랙홀이 갑자기 수축할 때 발생하는 가격 급락 위험을 선물 헤지로 방어."
    },
    "export_risk": {
      "source": "Australia, Malta, Turkey Export Reliance Analytics",
      "situation": "주요 참다랑어 수출국 5개국의 수출 목적지를 분석하면, 호주·몰타·튀르키예 모두 일본 시장 비중이 60~85%에 달하는 극단적 편중 구조입니다. 호주는 일본향 39,644톤 vs 미국향 905톤으로 일본 의존도 96%이며, 이는 일본 수요 1% 감소가 호주 수출 전체를 흔드는 '단일 고객 집중 리스크'입니다.",
      ...
      "risk": "일본 엔저(¥150+) 장기화 시 엔화 기준 구매력 저하로 인한 물량 축소를 대비한 환율 헤지 포지션 확보."
    },
    "enso_skipjack": {
      ...
      "short": "기상청 ENSO 전망 발표 즉시, 라니냐 예보 시 WCPO 서부 선단 집중 배치, 엘니뇨 시 중부 이동 프로토콜 가동.",
      "long": "ISSF 세계 참치어업 보고서와 NOAA ENSO 예보를 결합한 자체 '기후-어장 AI 모델'을 년 2회 업데이트 체계 구축.",
      "risk": "예보 실패(엘니뇨 전환이 예상보다 빠를 때)에 대비한 2개 해역 이상 동시 운영 체제 확보."
    },
    "alba_habitat": {
      ...
      "long": "기후 모델(RCP 4.5/8.5)별 알바코어 서식지 이동 시뮬레이션을 연간 업데이트하여 10년 단위 선대 재배치 마스터플랜 수립.",
      "risk": "고위도 이동으로 새로운 연안국(남아공·호주)의 EEZ 진입 시 필요한 입어권 교섭을 사전 착수."
    }} as any;
...
const data_area = [
  { area: '중서부태평양(WCPO)', vol2000: 2240, vol2024: 3583 },
...
export default function TunaOperationalInsights() {
  const [activeTab, setActiveTab] = useState<'fleet'|'regulation'|'sales'|'strategy'|'research'>('fleet');
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>통합 조업 및 전략 인텔리전스 (Field Ops & Advanced Intel)</h1>
          <p className={styles.subtitle}>기본 작전 10선 + 심화 전략 10선 + 연구자료 기반 10선 = 30대(30-Chart) 핵심 솔루션 대시보드</p>
        </div>
        <div className={styles.lastUpdated}>
          <RefreshCcw size={14} className={styles.rotateIcon} />
          <span>Last updated: 2026.04.15</span>
        </div>
      </header>

            <div className={styles.tabs}>
        <button className={`${styles.tab} ${activeTab === 'fleet' ? styles.activeTab : ''}`} onClick={() => setActiveTab('fleet')}>
          <Anchor size={16} /> 선단 운영 & 자원 동향 <span className={styles.badge}>6</span>
        </button>
        <button className={`${styles.tab} ${activeTab === 'regulation' ? styles.activeTab : ''}`} onClick={() => setActiveTab('regulation')}>
          <ShieldCheck size={16} /> 규제 대응 & 글로벌 물류 <span className={styles.badge}>7</span>
        </button>
        <button className={`${styles.tab} ${activeTab === 'strategy' ? styles.activeTab : ''}`} onClick={() => setActiveTab('strategy')}>
          <Briefcase size={16} /> 재무 파이낸싱 & 기업 전략 <span className={styles.badge}>7</span>
        </button>
        <button className={`${styles.tab} ${activeTab === 'sales' ? styles.activeTab : ''}`} onClick={() => setActiveTab('sales')}>
          <TrendingUp size={16} /> 거시 경제 & 시장 수요 <span className={styles.badge}>9</span>
        </button>
      </div>
