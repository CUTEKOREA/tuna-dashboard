...
export const CASHEW_INTELLIGENCE_DATA: IntelligenceTabData[] = [
  {
    ...
    iconName: 'Globe',
    widgets: [
      { title: '아시아 가공 마진 독점률', value: '35', unit: '%', desc: '서아프리카 RCN(15% 마진)을 독식하여 베트남/인도 가공 허브가 포획하는 비대칭적 가공 부가가치 스프레드', source: 'NotebookLM 밸류체인 분석 [📐 AI Analysis]', reliability: 65, trend: 'down' },
      ...
      { title: '가나 1D1F 연간 면세 복리', value: '10', unit: '년', desc: '현지 법인 설립 시 법인세 및 부품 수입관세 완전 면제', source: '가나 공화국 1D1F 투자 장려 법령', reliability: 100, trend: 'up' },
      ...
      { title: '아프리카 개발 부담금 제재', value: '44.86', unit: 'GHS/ton', desc: '생캐슈넛(RCN) 단순 유출 업자 대상 징벌적 세금 부과치', source: '가나 현지 무역 수출세(Levy) 통칙', reliability: 100, trend: 'alert' },
      { title: '중국 캐슈넛 B2B 수입 파워', value: '+42', unit: '% YoY', desc: '글로벌 RCN 블랙홀로 부상한 중국의 해상 수입 물동량 증가 및 전략적 비축 물량 팽창 비율', source: 'NotebookLM 해상 무역 인덱스 [📐 AI Analysis]', reliability: 65, trend: 'up' },
      { title: '무기명 외국자본 진입 위반', value: '0', unit: 'Tolerance', desc: '현지 파트너십(JV) 없는 무분별 브로커의 직접 수매 적발률', source: '아프리카 조달청 매뉴얼 가이드', reliability: 70, trend: 'neutral' },
      { title: '가나/코트디부아르 원물 독점률', value: '90', unit: '%', desc: '글로벌 RCN(생캐슈넛) 생산량 중 서아프리카 주요국이 차지하는 압도적 원산지 지배력', source: 'NotebookLM 원물 생산 분포 [📐 AI Analysis]', reliability: 65, trend: 'neutral' },
      { title: '베트남 영세 가공공장 폐업률', value: '34', unit: '%', desc: '아프리카 원물 수급 대란 및 품질 통제 강황에 따른 줄파산', source: '베트남 캐슈넛 협회(Vinacas) 분기 통계', reliability: 100, trend: 'down' },
      { title: 'USD-GHS 환율 방어 파워', value: '+12', unit: '% Edge', desc: '로컬 마켓 수매가(Cedi) 고정 vs 달러(USD) 수출 환차익', source: '가나 현지 파이낸싱 환율 방어 ERP', reliability: 75, trend: 'up' }
    ]
  },
  {
    id: 'tab2',
    ...
    iconName: 'Target',
    widgets: [
      { title: 'KOR(수율) 민감도 가치', value: '+$1.50', unit: '/ lb', desc: '수매 시 KOR 48lbs 기준, 1lb 상승할 때마다 즉시 붙는 마진', source: '105. 캐슈넛 KOR 단위 파이낸셜', reliability: 100, trend: 'up' },
      { title: 'TCDA MPP 매수 버퍼 갭', value: 'GHS 2.4', unit: 'Gap', desc: '가나 최저수매가(GHS 12/kg) vs 투기적 브로커 간 방어 여유폭', source: '105. 캐슈넛 로컬 조달 가격 추적기', reliability: 100, trend: 'alert' },
      ...
      { title: '중개상(Middlemen) 누수율', value: '18', unit: '%', desc: '현지 LBA 직수매 실패 시 다단계 브로커를 거치며 증발하는 운전자본 누수 마진', source: 'NotebookLM 현지 파이낸싱 모델 [📐 AI Analysis]', reliability: 65, trend: 'down' },
      { title: '재고 묵힘(Hold) 부패손실', value: '-8.5', unit: '%', desc: '가격 반등 기대하며 홀딩 시, 산패/곰팡이로 깎이는 등급 감가', source: '창고 보관 리스크 오퍼레이션 매뉴얼', reliability: 70, trend: 'alert' },
      { title: '산지 직접 수거율 (LBA 락인)', value: '75', unit: '%', desc: '내륙 Bush 산지부터 항만까지 직통으로 장악하여 중개 수수료를 0%로 소거한 자체 조달망 확보율', source: 'NotebookLM 산지 조달 네트워크 [📐 AI Analysis]', reliability: 65, trend: 'neutral' },
      ...
      { title: '원물 조달 데스밸리 잔여일', value: 'D-30', unit: 'Days', desc: '팩토리 풀가동을 버텨낼 창고 내 원물 잔여량. 긴급 조달 요망', source: '스마트팩토리 가동률-재고 교차 API', reliability: 100, trend: 'down' }
    ]
  },
  {
    id: 'tab3',
    ...
    iconName: 'Cpu',
    widgets: [
      { title: '공장 조업 임계점 (BEP)', value: '5,000', unit: 'ton/년', desc: '고정비(감가, 전기, 인건) 상쇄를 위해 뽑아야 하는 최소 프로세싱 량', source: '가나 1D1F 공장 CAPEX 플랜', reliability: 75, trend: 'neutral' },
      { title: '원물 최종 전환율 (Yield)', value: '23~25', unit: '%', desc: '1톤 탈각 시 최종 상등급 커널 획득 비율 파취 누수 측정용', source: '캐슈넛 수율 및 탈각 프레임워크', reliability: 75, trend: 'up' },
      ...
      { title: '로봇 자동포장 무균 검수점', value: '0', unit: 'Defect', desc: '위생 지표. 휴먼 에러 개입을 100% 차단한 HACCP 규격 진공 지수', source: '글로벌 식품 위생 검열 기준서', reliability: 70, trend: 'neutral' },
      ...
      { title: 'B-Grade(스플릿) 강등 페널티', value: '-35', unit: '%', desc: '공정 실수로 W180 커널이 쪼개져 덤핑 가격에 버려지는 마이너스 금액', source: '커널 글로벌 등급(Grade) 시세 낙폭', reliability: 75, trend: 'down' }
    ]
  },
  {
    id: 'tab4',
    ...
    widgets: [
      ...
      { title: 'B2C 리테일 초가공 시너지', value: '+180', unit: '%', desc: '단순 생품 납품에서 \'Roasted/Salted\' 로 맛을 입혀 팔 때의 폭리 점프', source: '조미 HMR 리테일 마진 스프레드', reliability: 75, trend: 'up' },
      ...
      { title: '캐슈 애플 업사이클링 ROI', value: '+5.2', unit: '%', desc: '버려지던 열매(Apple)의 과즙 음료 잼 변환 공정 베타 테스트 수익율', source: '아프리카 캐슈 애플 부산물화 연구', reliability: 75, trend: 'up' },
  ...
  },
  {
    ...
    widgets: [
      { title: 'EUDR 농장 매핑 방어율', value: '100', unit: '%', desc: '유럽 삼림벌채방지법(EUDR) 입항 금지를 절대 방어하는 Geolocation 추적', source: '유럽의회 EUDR 규제 조항 대응기록', reliability: 100, trend: 'up' },
