/**
 * r4 시안 — 승자 r3-A(★4) 완전체.
 * 3라운드 판정 반영: ①스파크라인 hover 수치(다크 툴팁) ②허브 카드 클릭 → 상단 KPI·차트 전환
 * ③카드 hover 리프트(B 의견 + UI_RULES 1-3) ④12주 최고·최저 점선(E 접목).
 */
'use client';
...
export default function HeroCommand({ rows }: { rows: AtunaPriceRow[] }) {
  ...
  return (
    <div className="dsc-card" style={{ padding: '20px 22px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(118px, 1fr))', gap: 8, marginTop: 16 }}>
        {ALL_HUBS.map(({ hub, kind }) => {
          const hubPair = latestTwoForAtunaHub(rows, hub);
          const hubDelta = deltaView(calcAtunaDeltaPct(hubPair));
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: '0.7rem', fontWeight: 700, color: hubDelta.color }}>{hubDelta.text}</div>
            </button>
          );
        })}
      </div>
      <p style={{ margin: '10px 0 0', fontSize: '0.72rem', color: 'var(--text-muted)' }}>
        허브 클릭 = 상단 시세·추이 전환 · 그래프에 마우스를 올리면 주간 수치
      </p>
    </div>
  );
}
