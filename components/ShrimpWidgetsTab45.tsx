...
export const W45_DiseaseRisk = () => {
  ...
  return (
    <WidgetCard title="EMS/WSSV 질병 아포칼립스 경보" icon={AlertTriangle} term="Disease Collapse" desc="치사율 100% 전염병 창궐에 의한 붕괴 차트" source="노트북LM 심층 리서치 - EMS/EHP 충격" situation="[Biological Tail Risk (EMS)] 2013년 아시아 권역을 초토화시킨 조기폐사증후군(EMS) 사태 당시 글로벌 양식 물동량의 50%가 증발하며 기준 시세가 200% 수직 폭등하는 최악의 생물학적 블랙스완(Tail Risk)이 입증되었습니다." actionPlan="[Geographic Hedging Protocol] 양식업 최대 리스크인 팬데믹(WSSV/EHP) 리스크를 원천 봉쇄하십시오. 인도, 베트남, 에콰도르 3개 대륙에 소싱 파이프라인을 33%씩 완벽히 물리적 분산(Geographic Hedging)시켜, 경쟁사 공급망 마비 시 압도적 반사이익(Arbitrage)을 수취하는 시스템을 가동해야 합니다.">
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W47_DualSourcing = () => {
  ...
  return (
    <WidgetCard title="신라교역 소싱 트라이앵글 작전판" icon={Compass} term="Dual Sourcing Radar" desc="국가별 장단점 시가전 배틀 카드" source="Strategic Internal Assessment" situation="[Dual-Sourcing Arbitrage] 에콰도르는 원물 캐파(Volume)는 압도적이나 물류 리드타임과 1차 가공 퀄리티에 치명적 약점이 존재하며, 베트남은 물량 한계에도 불구하고 0% 무관세 혜택과 최고 수준의 가공 역량(Value-Add)을 보유한 비대칭(Asymmetric) 구조입니다." actionPlan="[Strategic Portfolio Balancing] 단일 국가 의존형 소싱 체계를 즉시 폐기하십시오. 무한리필 B2B 및 식자재 마켓에는 에콰도르산 벌크(HLSO) 물량을 투하(Dumping)하고, 하이엔드 오프라인 리테일(초밥/밀키트)에는 베트남산 프리미엄 가공품을 꽂아 넣는 철저한 '투 트랙 듀얼 소싱(Dual-Sourcing)' 포트폴리오를 락인(Lock-in)해야 합니다.">
      <SafeResponsiveContainer width="100%" height="100%">
