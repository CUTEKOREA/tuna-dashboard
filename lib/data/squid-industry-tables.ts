 ...
 *
 * 발행본이 정본인 이유는 정정이 거기 쌓이기 때문이다. 이 편은 종 경계(살오징어·대왕·
 * 일렉스·훔볼트)가 표마다 다르게 걸리고, 그 경계가 발행본의 캡션·주석에 붙어 있다.
 * 대시보드가 숫자만 다시 계산하면 경계가 떨어진다.
 *
 * 아키텍처 가드: 원본 JSON import 는 이 인테이크 모듈에서만 한다.
 */
import raw from './squid-tables.json';

export interface SquidReportTable {
...
interface Stage {
  tables: SquidReportTable[];
}

const STAGES = raw as Record<string, Stage>;

/** 단계 키(`s01`~`s09`)로 그 단계에 배치된 보고서 표를 가져온다. */
export function getSquidTables(stage: string): SquidReportTable[] {
  return STAGES[stage]?.tables ?? [];
}

export function getSquidTableStages(): string[] {
  return Object.keys(STAGES);
}

/** 검증용 — 발행본에서 옮긴 표 총수. */
export function getSquidTableCount(): number {
  let n = 0;
