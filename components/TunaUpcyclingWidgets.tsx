...
return (
    <div className={styles.insightCard}>
      ...
      <TakeawayBox
        situation="[부산물 가치 재발견] 참치 가공 시 총 중량의 40~55%가 머리(35%), 내장(20%), 혈합육(15%), 껍질(10%) 등 부산물로 발생합니다. 한국 다랑어 어획량 290천톤(MOF, 2024) 기준 부산물 발생량은 116~160천톤. 글로벌 EPA/DHA 오메가-3와 해양 콜라겐 펩타이드 시장 모두 두 자릿수 CAGR로 성장 중이며(시장 규모는 보고서별 편차 큼 — Grand View Research 2025 등 재확인 필요), 기존 폐기 비용(톤당 $50~80)을 고부가 매출로 전환할 수 있는 전략적 기회입니다."
        actionPlan="[2단계 상용화 로드맵] Phase 1 (즉시): DHA/EPA 추출(마진 65%) + 해양 콜라겐 펩타이드(마진 72%) 라인 증설로 연간 ₩50억+ 추가 매출 확보. Phase 2 (3년): KFAS/NIFS 공동 R&amp;D로 ACE 억제 기능성 펩타이드(마진 80%) 상용화. 부산물 10% 고부가 전환 시 연매출 $50M+ 창출 가능."
...
export function TunaUpcyclingMarginMap() {
  ...
  return (
    <div className={styles.insightCard}>
      ...
      <TakeawayBox
        situation="[마진 역전 법칙] 기능성 펩타이드(마진 80%)가 DHA(65%), 콜라겐(72%)을 상회하나 현재 R&amp;D 단계입니다. 시장 규모 순서는 DHA &gt; 콜라겐 &gt; 바이오사료 순(보고서별 편차 큼)이며, 마진율과 시장 규모는 역(逆)상관 경향을 보입니다. 상용화 완료 제품 중에서는 콜라겐 펩타이드(마진 72%)가 투자 효율성 최적입니다."
        actionPlan="[우선순위 투자 전략] ① 즉시 수익: 해양 콜라겐 펩타이드 라인 증설(마진 72%, 상용화 단계). ② 중기 성장: 칼슘제 양산화(마진 45%, 성장기 — 원료 자급 우위). ③ 장기 고수익: KFAS/NIFS 공동 R&amp;D로 기능성 펩타이드(ACE 억제 활성 82%) 상용화 3년 파이프라인 구축. 바이오사료(마진 25%)는 대량 부산물 처리용으로 투트랙 유지."
