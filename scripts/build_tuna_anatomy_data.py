...
132척이 남의 바다에서 잡아 바다 위에서 판다. 그래서 축은
  ...
  4. **국내 캔과 수입** — 캔·조리 80,374톤(세 브랜드가 대부분, 그 밖 1,517, 합계와 1톤 차이·등식 금지), 수입은 참다랑어·자숙 로인·PB 캔. HS 1604.14는 캔·조제품.
...
def trade():
    ...
    return {
        "_meta": {"출처": "관세청 수출입무역통계 2019~2026년 7월 (원어 0303.41~49·필레 0304.87·캔·조제품 1604.14), 10자리 분해는 2022~2025 스냅샷", "단위": "톤, 백만 달러, 달러/kg", "기준연도": 2025,
...
def cans():
    ...
    return {
        "_meta": {"출처": "식품안전나라 식품 생산실적(I0300) 2022~2025, 품목유형 수산물가공품·품목명 참치/다랑어", "단위": "톤", "기준연도": 2024,
                  "캔2024": s["kind_t"]["캔·조리"]["2024"], "업체수": s["company_count"], "주의": "브랜드는 품목명 표기로 귀속. 세 브랜드가 대부분(동원 38,135·오뚜기 22,937·사조 17,784)이고 그 밖은 1,517톤. 원문 합계 80,374톤과 등식으로 맞추지 않는다. 동원 캔의 45%는 삼진물산·신진물산 위탁. 오뚜기에스에프 2024년 급증은 원문 그대로(사유 미공개)"},
        "연도별": rows,
        "공장별": fac,
    }


# Thai Union IR raw-material-price-trend, 2026-09-10 조회 (팩 C-1).
# 아카이브 price_chain.json 은 2025-11이 끝이라 여기로 덮어 잇는다. Atuna 시계열은 넣지 않는다.
IR_BKK_20260910 = {
    "2025-08": 1550, "2025-09": 1600, "2025-10": 1670, "2025-11": 1580, "2025-12": 1470,
    "2026-01": 1500, "2026-02": 1580, "2026-03": 2000, "2026-04": 1875,
    "2026-05": 1730, "2026-06": 1700, "2026-07": 1850, "2026-08": 2100,
}


def prices():
    p = load("price_chain.json")
    merged = dict(p["bangkok_skj_usd_t"])
    merged.update(IR_BKK_20260910)
    bkk = [{"월": k, "방콕": v} for k, v in sorted(merged.items())]
    ...
    return {
        "_meta": {
            "출처": "방콕 가다랑어: Thai Union IR 월별 원료가(Bangkok landings, WPO, 2026-09-10 조회); 원화 단가: 원양어업 생산동향 월보 어종별 생산단가",
            "단위": "달러/톤, 원/kg",
            "방콕저점": {"월": "2024-08", "값": 1250},
            "방콕2026_03": 2000,
            "방콕2026_08": 2100,
            "조회일": "2026-09-10",
            "주의": "IR 표는 해당 월 계약가이며 원료는 약 1개월 뒤 방콕 도착. 인도 시점 가격으로 읽지 않는다. 원화 단가는 선사 보고 생산금액÷생산량. Atuna 시계열(마지막 관측 2026-07-28, skjmnt=에콰도르 만타)은 대외 페이지에 쓰지 않는다.",
        },
        "방콕": bkk,
        "원화단가": krw,
    }
