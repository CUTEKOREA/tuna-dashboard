...
import { 
  ...
  Database, Ship, Zap, Leaf, Cpu, Layers, Clock
} from 'lucide-react';
...
export default function SquidDashboard() {
  ...
  const newResearchWidgets: any[] = [
    ...
    // ─── KMI 「FTA체결국 수산물 수입동향」 2021 Q1 ~ 2026 Q1 (21개 분기) 교차분석 위젯 ───
    {
      ...
      strat: "①페루산 $2.2/kg 단가는 2018년 이후 최저 — 6개월 선도(forward) 매입을 즉시 단행하여 평균 단가 락-인, ②에콰도르산은 \"상대적 저품질\"(KMI 명시) 이지만 $1.9/kg 가격 메리트가 뚜렷한이므로 B2B 진미채·자숙 원료로 분리 매입 라인 신설, ③페루-에콰도르 합산 신규 점유율이 47%로 급증한 시점 — 단일 산지(중국 40.5%) 의존 시대를 끝내고 3극 균형(중국·페루·에콰도르) 헷지 룰로 KPI 재편.",
