...
describe('2026-09-13 fleet operations sources', () => {
  it('reconciles the national VDS report at vessel and area grain', () => {
    expect(nationalVds.asOf).toBe('2026-09-20');
    expect(nationalVds.vessels).toHaveLength(6);
    expect(nationalVds.areas).toHaveLength(8);
    expect(nationalVds.totals).toEqual({ allocated: 1_457, consumed: 1_177.6, remaining: 279.4, weekly: 39.8 });
    ...
    // 09-20 판도 키리바시 소계 소진을 755.00 으로 그대로 찍었다(행 합 754.9) - 두 주째 같은 어긋남이다.
    ...
    expect(counted.reduce((sum, area) => sum + area.rowSums.consumed, 0)).toBeCloseTo(1_177.6, 2);
    ...
    // 09-20 판에 솔로몬/S/HAR(-0.37)이 새로 초과로 돌아서 11칸
