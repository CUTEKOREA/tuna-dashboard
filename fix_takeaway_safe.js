...
for (const file of files) {
  ...
  content = content.replace(/takeaway="([^"]+)"/g, (match, p1) => {
    return `takeaway={{ situation: "${p1}", actionPlan: "모니터링 유지", source: "Sashimi Market Report 2025" }}`;
  });
  ...
  if (!content.includes('takeaway={')) {
    ...
    content = content.replace(/(cardDesc="[^"]+")/, (match, p1) => {
...
console.log('Fixed missing and incorrectly formatted takeaway props safely.');
