...
export const W47_DualSourcing = () => {
  ...
  return (
    <WidgetCard title="신라교역 소싱 트라이앵글 작전판" icon={Compass} term="Dual Sourcing Radar" desc="국가별 장단점 시가전 배틀 카드" source="Strategic Internal Assessment" situation="에콰도르는 원물 캐파(Volume)는 이나 물류 리드타임과 1차 가공 퀄리티에 치명적 약점이 존재하며, 베트남은 물량 한계에도 불구하고 0% 무관세 혜택과 최고 수준의 가공 역량(Value-Add)을 보유한 비대칭(Asymmetric) 구조입니다." actionPlan="[Strategic Portfolio Balancing] 단일 국가 의존형 소싱 체계를 재검토 필요. 무한리필 B2B 및 식자재 마켓에는 에콰도르산 벌크(HLSO) 물량을 투하(Dumping)하고, 하이엔드 오프라인 리테일(초밥/밀키트)에는 베트남산 프리미엄 가공품을 꽂아 넣는 철저한 '투 트랙 듀얼 소싱(Dual-Sourcing)' 포트폴리오를 락인해야 합니다.">
      <SafeResponsiveContainer width="100%" height="100%">
