...
export const W01_Megatrend = () => (
  <WidgetCard title="수생태계 메가트렌드 (1970-2022)" icon={Globe} term="Production Megatrend" desc="글로벌 전체 생산량" source="FAO FishStatJ - Global Production Timeline" situation="[Aquaculture Hyper-Growth] 2000년대 이후 인공 양식(Aquaculture) 인프라의 폭발적 증가로 새우 생산량 곡선이 구조적 수직 팽창(Vertical Expansion) 단계에 진입하며, 글로벌 식량 밸류체인의 판도가 영구적으로 재편되었습니다." actionPlan="[Value-Chain Migration] 자연산 어획에 의존하는 낡은 비즈니스 모델(Legacy Model)에서 즉각 탈피하십시오. 전사 CAPEX를 글로벌 양식 밸류체인 편입 및 스마트 아쿠아팜 지분 확보에 집중하여 원가 주도권(Cost Leadership)을 탈환해야 합니다.">
...
export const W04_AquaShare = () => (
  <WidgetCard title="양식 침투율(Aquaculture Share)" icon={TrendingUp} term="Aqua Penetration" desc="전체 생산 중 양식이 차지하는 비중" source="FAO FishStatJ - Timeline Analysis" situation="[Aquaculture Penetration] 글로벌 새우 공급망(Supply Chain)의 65% 이상이 양식업 베이스로 완전히 재편(Market Penetration)되며, 양식 원가 경쟁력이 시장의 표준(Standard)으로 자리 잡았습니다." actionPlan="[Two-Track SCM Strategy] 포트폴리오를 철저히 이원화(Two-track)하십시오. 자연산은 최고가 파인다이닝향 니치 마켓(Niche Market) 전용으로 격리하고, 볼륨을 책임지는 B2B 프랜차이즈 체인은 100% 원가 통제가 가능한 양식 기반 SCM으로 전면 개편(Restructuring)해야 생존 가능합니다.">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W08_Top10AquaVal = () => (
  <WidgetCard title="양식업 부가가치 창출 Top 10" icon={DollarSign} term="Aqua Value Gen" desc="양식업으로 달러를 쓸어담는 국가" source="FAO FishStatJ - Aquaculture Value" situation="[Value Capture Monopoly] 글로벌 물량 생산량(Volume) Top 3 국가가 창출하는 부가가치(Value-Add) Top 3 순위와 완벽히 동기화(Synchronization)되며 극단적인 이익 과점 체제(Oligopoly)를 완성했습니다." actionPlan="[Pricing Power Hedging] 메이저 3국은 물량 공세(Dumping)를 넘어 글로벌 마켓 프라이싱 권한(Pricing Power)마저 독점하고 있습니다. 특정 국가에 대한 소싱 의존도를 30% 이하로 통제(Diversification)하여 공급망 병목 리스크(Bottleneck)를 완벽히 헤징(Hedging)해야 합니다.">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W09_CapCeiling = () => (
  <WidgetCard title="자연산 어획량 정체 한계선(Ceiling)" icon={Activity} term="Catch Ceiling" desc="환경적 요인으로 인한 어획 한계" source="FAO FishStatJ - Capture Timeline Analysis" situation="[Capture Yield Stagnation] 극심한 해양 생태계 파괴 여파로 지난 20년간 글로벌 자연산 어획량은 3.5M 톤(Tonnes)의 완벽한 횡보 박스권(Stagnation Box)에 갇혀 한계치(Ceiling)에 도달했습니다." actionPlan="[ESG Premium Exploitation] 자연산 새우의 희소 가치(Scarcity Value)는 영구적으로 치솟을 것입니다. MSC(해양관리협의회) 지속가능성 라벨링이 가능한 합법적 조업 쿼터(Quota)를 프리미엄 가격에 선제 싹쓸이(Buyout)하여 럭셔리 VVIP 라인업을 즉각 런칭하십시오.">
    <SafeResponsiveContainer width="100%" height="100%">
