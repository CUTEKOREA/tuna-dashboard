...
export const W22_KoreaSelfSuff = () => (
  <WidgetCard title="대한민국 자급률 (Self-sufficiency)" icon={ShieldCheck} term="Korea Self-Sufficiency" desc="내수 생산 대비 수입 의존율" source="FAO Data Derived" situation={`[Domestic Autonomy Deficit] 대한민국의 현재 로컬 새우 자급률은 ${koreaSelfSufficiency}%라는 절망적 수준으로, 사실상 글로벌 트레이더와 환율 변동성에 영혼이 묶여 있는 리스크 극대화(Maximum Exposure) 상태입니다.`} actionPlan="[Strategic Self-Sufficiency CAPEX] 수입 덤핑에 의존하는 리스크를 타개해야 합니다. 즉각 스마트 바이오플락(Bio-floc) 육상 양식장 R&D에 벤처 투자를 단행하여, 국내 무균 프리미엄(SPF) 오프라인 라인업의 부분적 수직 자립화(Vertical Independence)를 이루십시오." telemetry={{ status: 'STATIC', syncDate: '2026-05-29' }}>
...
export const W29_KoreaTradeBal = () => {
  ...
  return (
    <WidgetCard title="한국 새우 무역수지(Trade Balance)" icon={Scale} term="Deficit Gravity" desc="수입 결제 대금 vs 수출 획득 대금 갭" source="KITA 한국무역협회 데이터" situation="수입 92 대 수출 5 라는, 국가 거시 경제 지표를 왜곡시킬 수준의 비정상적 무역수지 적자(Trade Deficit Anomaly)를 창출하는 치명적 자본 유출 핵심 품목입니다." actionPlan="[K-Food Reverse Export Arbitrage] 수입한 원물에 밸류를 입혀 다시 수출하는 리버스 엔지니어링(Reverse Engineering)을 가동하십시오. 국내 클린룸에서 K-안주/하이엔드 간편식으로 밸류업(Value-up)한 뒤, K-팝 뷰티 패키징을 입혀 미주 H-Mart 등 교민 마켓에 수 배의 마진으로 역수출하는 라인을 뚫어야 합니다." telemetry={{ status: 'STATIC', syncDate: '2026-05-29' }}>
      <SafeResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
          <ChartPatternDefs />
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" horizontal={false} />
