...
const KPI_THEMES = [
  ...
  { border: 'none', glow: 'none', text: 'var(--color-success)', icon: Factory },
  { border: 'none', glow: 'none', text: 'var(--color-warning)', icon: DollarSign },
  { border: 'none', glow: 'none', text: 'var(--color-danger)', icon: Scale },
...
const SECTIONS = [
  { id: "S1", title: "🌱 Part I — 원물 생산 (Raw Material)", desc: "FAOSTAT 매크로 데이터 기반 — 종자 역수출 및 여름 단경기 집중 공략 (달랏 vs 제주)", color: "#f97316" },
  { id: "S2", title: "🏭 Part II — 가공 산업 (Processing)", desc: "B2B 전처리(IQF) 가공을 통한 원가 절감 및 식물검역 우회", color: "var(--color-success)" },
  { id: "S3", title: "🚢 Part III — 물류 및 무역 (Logistics)", desc: "MA 특수 포장과 한-베 FTA 0% 관세 프리패스 우위", color: "#38bdf8" },
  { id: "S4", title: "🛒 Part IV — 판매 및 수요 (Sales & Demand)", desc: "장기 수매 계약을 통한 단가 변동성 헤징 및 B2B 점유율 역전", color: "#8b5cf6" },
  { id: "S5", title: "🌍 Part V — ESG 및 미래 농업 (Sustainability)", desc: "비규격 폐기 방지 푸드 업사이클링 및 Scope 3 감축 연계", color: "#ec4899" },
];

const EstimateBadge = () => (
  <span style={{ display:'inline-flex', alignItems:'center', gap:'0.3rem', padding:'2px 8px', borderRadius:'500px', border:'none', backgroundColor:'var(--surface-2)', color:'var(--color-warning)', fontSize:'0.66rem', fontWeight:700, letterSpacing:'0.2px', marginLeft:'0.5rem', flexShrink:0, textTransform:'uppercase' as const }}>
    ESTIMATE
  </span>
);
    ...
    <div style={{ padding:'0 1.5rem 3rem', minHeight:'100vh' }}>
        ...
        <div className="ds-card" style={{ marginBottom:'2rem', padding:'1.2rem', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1.5rem' }}>
          ...
          <div style={{ borderRight:'1px dashed rgba(255,255,255,0.1)', paddingRight:'1.5rem' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.8rem' }}>
              <h3 style={{ margin:0, fontSize:'0.9rem', color:'var(--text-primary)', display:'flex', alignItems:'center', gap:'0.5rem' }}><RefreshCcw size={16} color="var(--color-success)" /> 실시간 차익거래 봇 (BOT & KAMIS API)</h3>
              <span style={{ fontSize:'0.66rem', background:'var(--color-success)', color:'var(--bg-color)', padding:'2px 8px', borderRadius:'500px', fontWeight:700, textTransform:'uppercase' as const }}>{liveArbitrage?.timestamp ? 'LIVE' : 'SYNCING'}</span>
            </div>
            {liveArbitrage ? (
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div>
                  ...
                  <div style={{ fontSize:'0.75rem', color:'var(--color-danger)', marginTop:'4px' }}>국내 도매가: {liveArbitrage.domesticWholesalePrice_KRW_per_kg.toLocaleString()}원</div>
                </div>
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontSize:'0.75rem', color:'var(--color-success)', fontWeight:700 }}>{liveArbitrage.recommendation.action}</div>
          ...
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.8rem' }}>
              <h3 style={{ margin:0, fontSize:'0.9rem', color:'var(--text-primary)', display:'flex', alignItems:'center', gap:'0.5rem' }}><ShieldCheck size={16} color="var(--color-warning)" /> KCS 통관 TRQ 모니터링 (관세청 API)</h3>
              {liveTrq?.alerts?.length > 0 && (
                <span style={{ fontSize:'0.65rem', background:'rgba(239,68,68,0.2)', color:'var(--color-danger)', padding:'2px 6px', borderRadius:'4px', display:'flex', alignItems:'center', gap:'3px' }}>
            ...
            {liveTrq ? (
              <div>
                ...
                <div style={{ width:'100%', height:'8px', background:'rgba(255,255,255,0.1)', borderRadius:'4px', overflow:'hidden' }}>
                  <div style={{ width:`${liveTrq.trqStatus.exhaustionRate_percent}%`, height:'100%', background: liveTrq.trqStatus.exhaustionRate_percent > 80 ? 'var(--color-danger)' : 'var(--color-success)', transition:'width 1s ease-in-out' }} />
                </div>
                {liveTrq.alerts?.length > 0 && (
                  <div style={{ fontSize:'0.7rem', color:'var(--color-danger)', marginTop:'8px', display:'flex', alignItems:'center', gap:'4px' }}>
      ...
      <div style={{ marginBottom:'2rem', background:'linear-gradient(135deg, rgba(249, 115, 22, 0.1) 0%, rgba(16, 185, 129, 0.05) 100%)', border:'1px solid rgba(249, 115, 22, 0.2)', borderRadius:'8px', padding:'1.5rem', boxShadow:'rgba(0,0,0,0.3) 0px 8px 8px' }}>
        ...
        <div className="ds-grid-3">
          ...
          <div>
            <h3 style={{ fontSize:'0.9rem', color:'var(--color-success)', marginBottom:'0.5rem', display:'flex', alignItems:'center', gap:'0.4rem' }}>
          ...
          <div>
            <h3 style={{ fontSize:'0.9rem', color:'#38bdf8', marginBottom:'0.5rem', display:'flex', alignItems:'center', gap:'0.4rem' }}>
              <Database size={14} /> 데이터 기반 아비트리지 락인 (Lock-in)
            </h3>
            <p style={{ margin:0, fontSize:'0.8rem', color:'var(--text-secondary)', lineHeight:1.5 }}>
          ...
          <div className="ds-card" style={{ animation:'fadeIn 0.3s' }}>
            ...
            <div className="ds-card-insight" style={{
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'1rem' }}>
                <div style={{ background:'#181818', padding:'0.8rem', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center' }}><Database size={20} color="var(--color-success)" /></div>
                <div>
                  <h3 style={{ color:'var(--text-primary)', margin:'0 0 0.3rem', fontSize:'1rem', fontWeight:700 }}><Zap size={16} color="var(--color-success)" style={{marginRight:'6px'}} />NotebookLM 당근 AI 챗봇</h3>
      ...
      <div className="ds-card" style={{ marginBottom:'2.5rem', marginTop: '1.5rem' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'0.8rem', marginBottom:'1.2rem', borderBottom:'1px dashed rgba(255,255,255,0.1)', paddingBottom:'0.8rem' }}>
          <div style={{ background:'var(--color-success)', padding:'0.6rem', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'rgba(0,0,0,0.3) 0px 4px 4px' }}>
        ...
        <div className="ds-grid-3">
          ...
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'0.5rem' }}>
              <label style={{ fontSize:'0.85rem', fontWeight:600, color:'#38bdf8' }}>IQF 가공 전환율 (CapEx 투자율)</label>
          ...
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'0.5rem' }}>
              <label style={{ fontSize:'0.85rem', fontWeight:600, color:'#8b5cf6' }}>초기 단가 양보율 (Vendor Margin Sacrifice)</label>
            ...
            />
            <p style={{ margin:'0.3rem 0 0', fontSize:'0.7rem', color:'#64748b' }}>초기 마진을 양보할수록 <strong style={{color:'#f8fafc'}}>W23 벤더 락인 LTV</strong>의 이탈률이 방어되어 장기 현금흐름(FCF)이 급증합니다.</p>
          </div>

          {/* Slider 3 */}
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'0.5rem' }}>
              <label style={{ fontSize:'0.85rem', fontWeight:600, color:'#f43f5e' }}>국내(KAMIS) 도매가 스트레스 (원/20kg)</label>
...
{/* Section 1: Raw Material */}
      <div style={{ marginBottom:'1rem', display:'flex', alignItems:'center', gap:'0.8rem', marginTop: '3rem' }}>
        <div style={{ width:'4px', height:'28px', background:'var(--color-success)', borderRadius:'2px' }} />
        <div>
      ...
      <div className="ds-grid-2" style={{ marginBottom:'2.5rem' }}>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={faoProdLive}>
                ...
                <Bar yAxisId="left" dataKey="중국_면적" fill="var(--color-warning)" name="중국 수확면적(ha)" />
                <Bar yAxisId="left" dataKey="베트남_면적" fill="var(--color-success)" name="베트남 수확면적(ha)" />
                <Line yAxisId="right" type="monotone" dataKey="중국_수율" stroke="var(--color-danger)" strokeWidth={2} dot={false} name="중국 수율(t/ha)" />
                <Line yAxisId="right" type="monotone" dataKey="베트남_수율" stroke="#38bdf8" strokeWidth={2} dot={false} name="베트남 수율(t/ha)" />
              </ComposedChart>
          ...
          actionPlan="'양적 팽창'의 한계점이 임계에 달했음을 고려할 때, 좁은 면적에서도 수율이 뛰어난 대체 산지 발굴이 글로벌 소싱 전략의 핵심임."
          ...
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: FAOSTAT</span>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <AreaChart data={faoPriceLive}>
                <defs>
                  <linearGradient id="colorKorea" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#38bdf8" stopOpacity={0.4}/><stop offset="95%" stopColor="#38bdf8" stopOpacity={0.0}/></linearGradient>
                  <linearGradient id="colorChina" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-danger)" stopOpacity={0.4}/><stop offset="95%" stopColor="var(--color-danger)" stopOpacity={0.0}/></linearGradient>
                  <linearGradient id="colorVietnam2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-success)" stopOpacity={0.4}/><stop offset="95%" stopColor="var(--color-success)" stopOpacity={0.0}/></linearGradient>
                </defs>
                {grid}
                <XAxis dataKey="year" {...xAxisTextProps} />
                <YAxis {...yAxisProps} />
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Area connectNulls type="monotone" dataKey="한국" stroke="#38bdf8" fill="url(#colorKorea)" strokeWidth={2} name="한국 생산자 가격" />
                <Area connectNulls type="monotone" dataKey="중국" stroke="var(--color-danger)" fill="url(#colorChina)" strokeWidth={2} name="중국 생산자 가격" />
                <Area connectNulls type="monotone" dataKey="베트남" stroke="var(--color-success)" fill="url(#colorVietnam2)" strokeWidth={2} name="베트남 생산자 가격" />
              </AreaChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="해발 1,500m 항시 냉량 기후(15~25도)를 유지하는 베트남 달랏(Dalat)의 연중 고정 단가를 활용, 대형 B2B 바이어에게 '가격 변동성 제로(Zero-Volatility)' 장기 공급 락인(Lock-in) 모델을 제시하여 시장 지배력을 탈취할 것."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <span style={{ fontSize:'0.75rem', color:'#64748b', fontWeight:400 }}>(단위: USD/톤)</span>
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: KAMIS & KCS</span>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <AreaChart data={w1Live}>
                <defs>
                  <linearGradient id="colorKoreaJeju" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#38bdf8" stopOpacity={0.8}/><stop offset="95%" stopColor="#38bdf8" stopOpacity={0.1}/></linearGradient>
                  <linearGradient id="colorKoreaGangwon" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-danger)" stopOpacity={0.8}/><stop offset="95%" stopColor="var(--color-danger)" stopOpacity={0.1}/></linearGradient>
                  <linearGradient id="colorVietnam" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-success)" stopOpacity={0.8}/><stop offset="95%" stopColor="var(--color-success)" stopOpacity={0.1}/></linearGradient>
                </defs>
                {grid}
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Area connectNulls={true} type="monotone" dataKey="한국(제주)" stroke="#38bdf8" fill="url(#colorKoreaJeju)" name="한국(제주) 도매가" />
                <Area connectNulls={true} type="monotone" dataKey="한국(강원)" stroke="var(--color-danger)" fill="url(#colorKoreaGangwon)" name="한국(강원) 도매가" />
                <Area connectNulls={true} type="monotone" dataKey="베트남(달랏)" stroke="var(--color-success)" fill="url(#colorVietnam)" name="베트남 수입가" />
              </AreaChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="최대 마진 스프레드가 발생하는 7~9월 구간에 한-베트남 FTA(VKFTA 0%) 무관세 특혜를 적용받는 달랏산 물량을 집중 투입하여 단기 차익 거래(Arbitrage)를 극대화하고 벤더 이탈률을 방어할 것."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={w2Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar yAxisId="left" dataKey="생산수율(%)" fill="#38bdf8" name="생산수율(%)" barSize={40} />
                <Line yAxisId="right" type="monotone" dataKey="당도(Brix)" stroke="var(--color-warning)" strokeWidth={3} dot={{r: 4}} name="당도(Brix)" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="한국형 고당도 종자를 베트남 달랏(해발 1,000m+ 최적 기후)으로 역수출하여 계약 재배할 경우, 92.5%의 높은 수율과 8.9 Brix의 압도적 당도를 동시에 달성할 수 있습니다."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: NOAA</span>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={w15Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar yAxisId="left" dataKey="기후리스크지수" fill="var(--color-danger)" opacity={0.6} name="동북아 기후 리스크(태풍/폭우)" />
                <Line yAxisId="right" type="monotone" dataKey="동북아_스팟가격폭등률" stroke="var(--color-warning)" strokeWidth={3} dot={{r: 4}} name="도매 스팟가 폭등률(%)" />
                <Area yAxisId="left" type="step" dataKey="달랏_생산안정성" stroke="var(--color-success)" fill="var(--color-success)" fillOpacity={0.15} name="달랏(해발1500m) 생산안정성" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="지정학적/기후적 타격권에서 완전히 벗어난 베트남 달랏을 '기후 프리미엄 콜옵션' 산지로 포지셔닝하여, 공급망 붕괴 시점에도 100% 이행 가능한 거시적 조달망을 무기로 프리미엄 단가를 확보할 것."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: MFDS</span>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={w20Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar yAxisId="left" dataKey="중국_통관불합격건수" fill="var(--color-danger)" name="중국산 통관 불합격 건수" barSize={30} />
                <Bar yAxisId="left" dataKey="베트남_통관불합격건수" fill="var(--color-success)" name="베트남산 불합격 건수" barSize={30} />
                <Line yAxisId="right" type="monotone" dataKey="중국_회수물량_톤" stroke="var(--color-warning)" strokeWidth={3} dot={{r:4}} name="중국산 긴급 회수 물량(톤)" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="중국산의 치명적 식품 안전성(Food Safety) 붕괴 사태를 반면교사 삼아, 파종부터 수확까지 엄격히 통제된 달랏의 무결점 인증(VietGAP) 당근을 '대체 불가한 안전 프랜차이즈 원료'로 프리미엄화할 것."
      ...
      <div className="ds-grid-2" style={{ marginBottom:'2.5rem' }}>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={w3Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar dataKey="원물가" stackId="a" fill="#38bdf8" name="원물(생물) 수입비" />
                <Bar dataKey="전처리 인건비" stackId="a" fill="var(--color-danger)" name="B2B 구매자 자체 인건비" />
          ...
          actionPlan="베트남 현지 공장에서 IQF(다이스) 가공 직수입 시 바이어의 최종 원가를 약 45% 절감(81k → 45k)시키는 강력한 영업 우위를 점유함."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={w11Live} layout="vertical" margin={{ left: 20 }}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar dataKey="수율" stackId="a" fill="var(--color-success)" name="유효 수율(%)" />
                <Bar dataKey="손실" stackId="a" fill="var(--color-danger)" name="원물/검역 폐기 손실(%)" />
                <Bar dataKey="IQF" stackId="a" fill="#38bdf8" name="IQF 전처리 수율(%)" />
              </BarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="생산지(베트남 달랏)에서 10x10mm 깍둑썰기 및 IQF(급속냉동) 전처리를 거쳐 수입할 경우, 생물 방역(PLS) 규제를 완전히 우회하며 100% 무손실 수율 구조를 달성함."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <LineChart data={w16Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Line type="monotone" dataKey="한국_전처리비용" stroke="#38bdf8" strokeWidth={2} name="한국 (한계 도달)" strokeDasharray="3 3" />
                <Line type="monotone" dataKey="중국_전처리비용" stroke="var(--color-danger)" strokeWidth={3} name="중국 (수직 상승)" />
                <Line type="monotone" dataKey="베트남_전처리비용" stroke="var(--color-success)" strokeWidth={3} name="베트남 달랏 (인구 보너스)" />
              </LineChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="평균 임금 상승률(CAGR 8~10%)을 감안해도 중국 대비 압도적으로 낮은 베트남으로 전처리(탈피/절단) 기지를 이전하는 것은, 향후 15년 이상의 장기 구조적 인건비 차익(Labor Arbitrage)을 확정 짓는 필수 전략임."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <AreaChart data={w21Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Area type="monotone" stackId="1" dataKey="IQF냉동_비중" stroke="#8b5cf6" fill="#8b5cf6" name="IQF 냉동" />
                <Area type="monotone" stackId="1" dataKey="전처리_비중" stroke="#f97316" fill="#f97316" name="전처리(절단)" />
                <Area type="monotone" stackId="1" dataKey="세척_비중" stroke="var(--color-success)" fill="var(--color-success)" name="세척 당근" />
          ...
          actionPlan="베트남 현지에 전처리 가공/냉동 설비를 선제적으로 투자할 경우, 가장 빠르게 성장하는 수익성 높은 시장(High-margin segment)을 독식하게 됨."
      ...
      <div className="ds-grid-2" style={{ marginBottom:'2.5rem' }}>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          actionPlan="단일 공급망 리스크(PLS 사태, 수출 통제 등)를 방어하기 위해 즉각적인 베트남 등 '중국 외 공급 다변화(China+1)' 포트폴리오 구축이 필수적임."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={w5Live}>
                ...
                <Bar dataKey="포장/물류비" stackId="a" fill="var(--color-warning)" name="일반 물류비" />
                <Bar dataKey="포장/물류비(MA)" stackId="a" fill="var(--color-success)" name="MA 특수 포장비" />
                <Bar dataKey="관세(30%)" stackId="a" fill="var(--color-danger)" name="관세 (중국 30%)" />
              </BarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="포장재 개선(MA)에 따른 원가 상승분은 VKFTA의 압도적인 0% 관세 효과로 완벽히 흡수되며, 최종 도착가(Landed Cost)에서 중국산을 역전합니다."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={w6Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Radar name="일반 상자 포장" dataKey="일반포장" stroke="var(--color-danger)" fill="var(--color-danger)" fillOpacity={0.3} />
                <Radar name="MA 특수 포장" dataKey="MA특수포장" stroke="var(--color-success)" fill="var(--color-success)" fillOpacity={0.3} />
              </RadarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="호흡량을 억제하는 MA 포장 전면 도입으로 저온 보관일을 최장 45일까지 연장, 물류 지연 리스크를 완전히 제거하는 '기술적 방어' 실현."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <AreaChart data={w17Live}>
                <defs>
                  <linearGradient id="colorChinaStorage" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-danger)" stopOpacity={0.5}/><stop offset="95%" stopColor="var(--color-danger)" stopOpacity={0.0}/></linearGradient>
                  <linearGradient id="colorVietStorage" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#38bdf8" stopOpacity={0.5}/><stop offset="95%" stopColor="#38bdf8" stopOpacity={0.0}/></linearGradient>
                </defs>
                {grid}
                <XAxis dataKey="day" {...xAxisTextProps} />
                <YAxis {...yAxisProps} />
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Area type="monotone" dataKey="중국산_재고유지비" stroke="var(--color-danger)" fill="url(#colorChinaStorage)" strokeWidth={2} name="중국산 (조기도착/창고비 급증)" />
                <Area type="monotone" dataKey="베트남산_재고유지비" stroke="#38bdf8" fill="url(#colorVietStorage)" strokeWidth={3} name="베트남산 (해상창고 10일 무료)" />
              </AreaChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="베트남발 화물의 10일 항해를 '무료 해상 창고'로 취급하여 초기 7일간의 재고유지비($140/컨테이너)를 세이브하고, 한국 시황 폭락 시 항로를 틀어 일본/대만으로 전매(Arbitrage)하는 지리적 콜옵션 발동이 가능함."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              <Scale size={17} />WTO TRQ 배분 의존도 vs FTA 영구 차익 (Arbitrage)
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={dynamicW22Data}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Area yAxisId="left" type="monotone" dataKey="베트남산_유통마진" fill="var(--color-success)" fillOpacity={0.4} stroke="var(--color-success)" name="베트남산 수입 마진 (USD/t)" />
                <Area yAxisId="left" type="monotone" dataKey="중국산_유통마진" fill="var(--color-warning)" fillOpacity={0.2} stroke="var(--color-warning)" name="중국산 수입 마진 (USD/t)" />
                <Line yAxisId="left" type="step" dataKey="국내도매가_환산" stroke="#f43f5e" strokeWidth={2} strokeDasharray="5 5" dot={false} name="KAMIS 국내 도매가 환산 (USD/t)" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="VKFTA 0% 수혜를 받는 베트남 거점은 TRQ 발동과 무관하게 상시적(Permanent)으로 최대 유통 마진을 보장하는 완벽한 구조적 차익(Arbitrage) 머신으로 작동합니다."
      ...
      <div className="ds-grid-2" style={{ marginBottom:'2.5rem' }}>
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: KCS</span>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <LineChart data={w7Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Line type="monotone" dataKey="스팟시장(제주)" stroke="#38bdf8" strokeWidth={2} name="국산 스팟 단가" />
                <Line type="monotone" dataKey="스팟시장(중국)" stroke="var(--color-danger)" strokeWidth={2} name="중국산 스팟 단가" strokeDasharray="5 5" />
                <Line type="monotone" dataKey="베트남_장기계약" stroke="var(--color-success)" strokeWidth={4} name="베트남 연간 계약 고정단가" />
              </LineChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="달랏-한국 간 직결형 하이브리드 파이프라인(KCS 수입 단가 실측)을 바탕으로 원가 불확실성을 0%로 통제하고, 경쟁사가 모방 불가능한 '연중 고정 공급가' 계약으로 대형 바이어를 영구 종속시킬 것."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <AreaChart data={w8Live}>
                <defs>
                  <linearGradient id="gradBull" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-success)" stopOpacity={0.3}/><stop offset="95%" stopColor="var(--color-success)" stopOpacity={0.02}/></linearGradient>
                  <linearGradient id="gradBase" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#38bdf8" stopOpacity={0.4}/><stop offset="95%" stopColor="#38bdf8" stopOpacity={0.05}/></linearGradient>
                  <linearGradient id="gradCons" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="var(--color-warning)" stopOpacity={0.3}/><stop offset="95%" stopColor="var(--color-warning)" stopOpacity={0.02}/></linearGradient>
                </defs>
                {grid}
                <XAxis dataKey="year" {...xAxisTextProps} />
                <YAxis {...yAxisProps} tickFormatter={(v: number) => `${v}%`} domain={[0, 100]} />
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Area type="monotone" dataKey="낙관(Bull)" stroke="var(--color-success)" fill="url(#gradBull)" strokeWidth={2} name="낙관 시나리오(%)" />
                <Area type="monotone" dataKey="기본(Base)" stroke="#38bdf8" fill="url(#gradBase)" strokeWidth={3} name="기본 시나리오(%)" strokeDasharray="0" />
                <Area type="monotone" dataKey="보수적(Conservative)" stroke="var(--color-warning)" fill="url(#gradCons)" strokeWidth={2} name="보수적 시나리오(%)" strokeDasharray="5 5" />
              </AreaChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="KREI TAM(순수입 11.4만톤) 기반 Bass Diffusion Model 적용 시, 기본(Base) 시나리오 5년 내 55%, 보수적 시나리오에서도 30%의 시장 침투가 전망됨. 한-베 FTA 0% 관세 + IQF 전처리 품질 최적화가 핵심 전제 조건."
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="80%" data={w12Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Radar name="베트남 달랏(역수출)" dataKey="베트남_달랏" stroke="var(--color-success)" fill="var(--color-success)" fillOpacity={0.6} />
                <Radar name="중국산(세척/일본종자)" dataKey="중국산_세척" stroke="var(--color-danger)" fill="var(--color-danger)" fillOpacity={0.4} />
              </RadarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
            <TakeawayBox
          situation="식약처 DB 및 현지 실측 결과, 중국산은 조리 편의성에 치중한 일본 종자를 써서 당도(6.2)와 베타카로틴 함량이 현저히 낮으나, 베트남 산지 모델은 압도적 고당도(12.8 Brix)와 무결점 안전성(VietGAP)을 증명함."
          actionPlan={`기존의 단순 '값싼 원재료' 프레임에서 탈피하여, "고당도·항산화 청정 프리미엄" 소구점을 통해 고수익성 B2C(프리미엄 밀키트, 유기농 이유식) 시장까지 마진(Target Margin) 확장이 가능함.`}
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'520px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: KAMIS & KCS & NOAA</span>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'300px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={w18Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Radar name="🇻🇳 달랏 농장 A (1순위)" dataKey="달랏농장A" stroke="var(--color-success)" fill="var(--color-success)" fillOpacity={0.35} strokeWidth={2} />
                <Radar name="🇨🇳 칭다오 공장 B" dataKey="칭다오공장B" stroke="var(--color-danger)" fill="var(--color-danger)" fillOpacity={0.15} strokeWidth={1} />
                <Radar name="🇰🇷 제주 산지 C" dataKey="제주산지C" stroke="#38bdf8" fill="#38bdf8" fillOpacity={0.15} strokeWidth={1} />
              </RadarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="단순 수입 유통업(Flat Margin)을 넘어, 확고한 산지 장악력과 전처리 밸류체인 내재화를 무기로 사모펀드(PEF) 딜 소싱(Deal Sourcing) 단계에서 엑시트 밸류에이션 매트릭스의 압도적 우위를 증명할 것."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              <Banknote size={17} />대형 벤더 장기 락인(Lock-in) 누적 생애가치 (LTV)
              <span style={{ fontSize:'0.75rem', color:'#64748b', fontWeight:400 }}>(단위: 누적 잉여현금흐름(FCF) 인덱스 및 이탈률 %)</span>
              <div style={{ marginLeft:'auto', flexShrink:0 }}>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={dynamicW23Data}>
                ...
                <Area yAxisId="left" type="monotone" dataKey="장기계약_누적수익" fill="#8b5cf6" stroke="#8b5cf6" fillOpacity={0.3} name="장기 락인 계약 (누적 FCF)" />
                <Line yAxisId="right" type="monotone" dataKey="고객이탈률" stroke="var(--color-danger)" strokeWidth={2} name="스팟 시장 고객 이탈률(%)" strokeDasharray="4 4" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="전처리 폼팩터(IQF 다이스 등)를 '연중 고정 단가'로 대형 벤더(신세계/CJ 등)에 공급하여 락인(Lock-in)시, 3년 차부터 잉여현금흐름(FCF) 기반 이익이 3배 이상 폭발적으로 누적 증대됨."
      ...
      <div className="ds-grid-2" style={{ marginBottom:'2.5rem' }}>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={faoLossLive} stackOffset="expand">
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar dataKey="식용소비" stackId="a" fill="var(--color-success)" name="식탁 소비재" />
                <Bar dataKey="가공용" stackId="a" fill="#38bdf8" name="고부가 가공 전환" />
                <Bar dataKey="사료용" stackId="a" fill="var(--color-warning)" name="사료 전환" />
                <Bar dataKey="수확후손실(폐기)" stackId="a" fill="var(--color-danger)" name="공급망 내 원물 손실(버려짐)" />
              </BarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="비규격 손실분(Waste) 전량을 펫푸드나 고순도 베타카로틴(메디푸드)용 추출 산업으로 업사이클링 시, 막대한 마진 창출 및 ESG 페널티 면제가 가능함."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: UN Comtrade</span>
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={w9Live} layout="vertical">
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar dataKey="수익창출(ROI)" fill="var(--color-success)" name="수익 창출 (ROI)" />
                <Bar dataKey="탄소배출(Penalty)" fill="var(--color-danger)" name="탄소 감축 효과 (양수=감축)" />
              </BarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="까다로운 신선 농산물 국경 방역 장벽을 완전히 우회하면서, 잔여 폐기물을 천연 베타카로틴 시장(USD 6.1억, GMI)이나 펫푸드 등 고부가가치 바이오 소재로 100% 전환하는 '푸드 업사이클링' 초격차 마진을 실현할 것."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={w10Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Radar name="기존 원물 소싱" dataKey="기존소싱" stroke="var(--color-danger)" fill="var(--color-danger)" fillOpacity={0.25} />
                <Radar name="업사이클링 계약 소싱" dataKey="업사이클링" stroke="var(--color-success)" fill="var(--color-success)" fillOpacity={0.35} />
              </RadarChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="투명하게 입증 가능한 친환경 베트남 농법 데이터 및 B품 업사이클링 수거율 데이터를 무기로, 단가 경쟁을 피하고 대기업 바이어의 ESG 실적 달성을 돕는 '독점적 파트너' 지위를 확보함."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={w14Live}>
                {grid}
                <XAxis dataKey="stage" {...xAxisTextProps} />
                ...
                <Area type="monotone" dataKey="PEF_수직계열화" stroke="#f43f5e" fill="#f43f5e" fillOpacity={0.2} strokeWidth={3} name="PEF 수직계열화 (Smile Curve)" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="전방의 고당도 종자 IP(라이선싱) 통제권과 후방의 스마트 가공(IQF) 및 바이오 업사이클링 역량을 동시에 내재화할 경우, '단순 유통업'에서 '푸드테크 유니콘'으로 재분류되어 15배(15x) 이상의 폭발적인 멀티플 차익거래가 실현됨."
        ...
        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'250px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={w24Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Bar yAxisId="left" dataKey="업사이클링_전환량_톤" fill="#ec4899" name="바이오소재 전환량(톤)" barSize={40} />
                <Line yAxisId="right" type="monotone" dataKey="바이오소재_프리미엄마진율" stroke="var(--color-warning)" strokeWidth={3} name="베타카로틴 마진율(%)" />
                <Line yAxisId="right" type="monotone" dataKey="Scope3_감축량_tCO2e" stroke="var(--color-success)" strokeWidth={3} strokeDasharray="3 3" name="Scope 3 탄소감축량(tCO2e)" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
          ...
          actionPlan="잉여 원물을 폐기 비용에서 메디푸드(고순도 베타카로틴) 소재로 업사이클링 전환 시, 기존 유통 마진의 한계를 돌파하는 70% 이상의 초격차 마진이 창출되며 확보된 Scope 3 탄소 감축량은 대기업 B2B 계약의 독점적 무기로 작용함."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'520px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0, display:'flex', alignItems:'center', gap:'0.6rem' }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>LIVE API: DART & PitchBook</span>
          ...
          <div style={{ height:'280px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={dynamicW19Data} margin={{ left: 10, right: 10 }}>
                {grid}
                <XAxis dataKey="stage" {...xAxisTextProps} interval={0} angle={-12} />
                <YAxis {...yAxisProps} />
                <RechartsTooltip content={<CustomTooltip />} />
          ...
          <div style={{ marginTop:'auto' }}>
            <TakeawayBox
              ...
              actionPlan="기후 리스크 헷징, 무관세 원가 우위, 잔류농약 Zero 락인, 푸드 업사이클링 신사업을 총망라한 동적 EBITDA 멀티플 워터폴을 대시보드에 즉각(Living) 전시하여 대형 기관 투자자(LP)의 투자의사결정을 이끌어 낼 것."
        ...
        <div style={{ gridColumn: '1 / -1', margin: '2.5rem 0 1rem', borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:'1.5rem' }}>
          <h2 style={{ fontSize:'1.15rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.3rem', display:'flex', alignItems:'center', gap:'0.5rem' }}>
            <Database size={20} color="var(--color-success)" />📊 Data Intelligence Upgrade — OEC · KAMIS · FAOSTAT SCL 실측 통합
          </h2>
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0 }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>OEC 실측</span>
              </div>
            </h3>
            <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'260px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <AreaChart data={w25Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Area type="monotone" dataKey="중국" stackId="1" stroke="var(--color-danger)" fill="#ef444466" name="중국" />
                <Area type="monotone" dataKey="네덜란드" stackId="1" stroke="#f97316" fill="#f9731666" name="네덜란드" />
                <Area type="monotone" dataKey="미국" stackId="1" stroke="var(--color-info)" fill="#3b82f666" name="미국" />
                <Area type="monotone" dataKey="이탈리아" stackId="1" stroke="var(--color-success)" fill="#10b98166" name="이탈리아" />
                <Area type="monotone" dataKey="스페인" stackId="1" stroke="var(--color-warning)" fill="#f59e0b66" name="스페인" />
                <Area type="monotone" dataKey="이스라엘" stackId="1" stroke="#8b5cf6" fill="#8b5cf666" name="이스라엘" />
              </AreaChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
            <TakeawayBox
              ...
              actionPlan="중국 단일 소싱 리스크를 완화하기 위한 베트남·호주·이스라엘 다변화 전략은 글로벌 수출 시장 구조상 충분히 실현 가능하며, 특히 이스라엘의 2020년 이후 급격한 수출 감소(전쟁 리스크)는 대체 공급원으로서 베트남의 포지셔닝 기회를 더욱 확대시킴."
          ...
          <div style={{ height:'260px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <LineChart data={w26Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Line type="monotone" dataKey="독일" stroke="var(--color-warning)" strokeWidth={2} dot={false} name="독일" />
                <Line type="monotone" dataKey="캐나다" stroke="var(--color-danger)" strokeWidth={2} dot={false} name="캐나다" />
                <Line type="monotone" dataKey="미국" stroke="var(--color-info)" strokeWidth={2} dot={false} name="미국" />
                <Line type="monotone" dataKey="프랑스" stroke="#8b5cf6" strokeWidth={2} dot={false} name="프랑스" />
                <Line type="monotone" dataKey="일본" stroke="#ec4899" strokeWidth={2} dot={false} name="일본" />
                <Line type="monotone" dataKey="한국" stroke="var(--color-success)" strokeWidth={3} dot={{ fill: 'var(--color-success)', r: 3 }} name="🇰🇷 한국" />
              </LineChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
            <TakeawayBox
              ...
              actionPlan="한국의 당근 수입은 구조적으로 확대 불가피(KREI 자급률 45.2% 전망). 중국 의존도 90%+ 상태에서 베트남을 제2공급원으로 확보하는 것은 단순 원가 절감을 넘어 국가 식량안보 차원의 전략적 포지셔닝이며, 일본 시장($38M)으로의 동시 진출로 규모의 경제를 달성할 수 있음."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <div style={{ marginLeft:'auto', flexShrink:0 }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>KAMIS 실측</span>
              </div>
            </h3>
            <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'260px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <LineChart data={w27Live}>
                ...
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{fontSize:'10px'}} />
                <Line type="monotone" dataKey="y2024" stroke="var(--color-danger)" strokeWidth={2.5} name="2024 (폭등)" dot={{ fill: 'var(--color-danger)', r: 2 }} />
                <Line type="monotone" dataKey="y2025" stroke="var(--color-warning)" strokeWidth={2} name="2025 (회복)" dot={{ fill: 'var(--color-warning)', r: 2 }} />
                <Line type="monotone" dataKey="y2026" stroke="var(--color-success)" strokeWidth={2.5} name="2026 (정상화)" dot={{ fill: 'var(--color-success)', r: 3 }} />
          ...
          <div style={{ marginTop:'auto' }}>
            <TakeawayBox
              ...
              actionPlan="2024년형 폭등 사이클(9월 피크)에 대비하여 7~8월 선제적 물량 확보 및 장기계약 단가 고정이 핵심. 2026년 저가 안정기(₩30K대)는 신규 바이어 진입에 최적의 타이밍이며, 중국산 대비 가격 경쟁력 확보 시 시장점유율 확대의 골든타임."
        ...
        <div className="ds-card" style={{ display:'flex', flexDirection:'column', minHeight:'480px' }}>
          <div style={{ marginBottom:'1.2rem', borderBottom:'1px solid rgba(255,255,255,0.05)', paddingBottom:'0.8rem' }}>
            <h3 style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontSize:'0.95rem', fontWeight:700, color:'var(--text-primary)', margin:'0 0 0.4rem' }}>
              ...
              <span style={{ fontSize:'0.75rem', color:'#64748b', fontWeight:400 }}>(단위: %, 톤)</span>
              <div style={{ marginLeft:'auto', flexShrink:0 }}>
                <span style={{ fontSize:'0.65rem', background:'var(--surface-2)', color:'var(--color-success)', padding:'2px 8px', borderRadius:'500px', border:'none', fontWeight:600 }}>FAOSTAT 실측</span>
              </div>
            </h3>
            <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
          ...
          <div style={{ height:'260px', width:'100%', marginBottom:'1rem' }}>
            <SafeResponsiveContainer width="100%" height="100%">
              <ComposedChart data={w28Live} margin={{ left: 10, right: 10 }}>
                {grid}
                <XAxis dataKey="country" {...xAxisTextProps} />
                ...
                <Line yAxisId="left" type="monotone" dataKey="손실률" stroke="var(--color-danger)" strokeWidth={3} dot={{ fill: 'var(--color-danger)', r: 5, stroke: 'var(--text-primary)', strokeWidth: 2 }} name="손실률(%)" />
              </ComposedChart>
            </SafeResponsiveContainer>
          </div>
          <div style={{ marginTop:'auto' }}>
            <TakeawayBox
              ...
              actionPlan="한국의 27.3% 손실률은 곧 3.1만톤의 비규격 원물을 의미하며, 이를 베타카로틴 추출(CalPoly Amin et al. 기준 11배 수율) 또는 IQF 전처리 원료로 전환 시 톤당 $200+ 부가가치 창출 가능. 중국 수준(5%)까지 손실률을 낮추면 연간 ~24,000톤의 식용 가능 물량이 추가 확보되어 수입 대체 효과까지 동시 달성."
