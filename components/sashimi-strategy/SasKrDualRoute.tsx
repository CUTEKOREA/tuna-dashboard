...
/* ── 한국의 두 경로: 태국 원물공급 vs 영국 완제품 직수출 ──
   ...
   태국 對美 19% 상호관세(2025-08, USTR 확정). ※영국 MFN '20%p 핸디캡'은 1차 출처 미확인 → 정성 헷지.
...
const ROUTES = [
  ...
  },
  {
    ...
    rows: [['Korea-UK FTA 관세', '완제품 0%'], ['태국 대비 우위', '영국 최혜국(MFN) 관세 우회'], ['타깃', '스시·투고 외식 식자재'], ['포지션', '태국 가공 우회·고부가']],
...
export default function SasKrDualRoute() {
  return (
    <WidgetCard
      ...
      takeaway={{
        situation: "한국은 영국·태국 밸류체인에서 두 갈래 경로를 동시에 갖습니다. 경로 A는 태국 가공 허브에 원물을 공급하는 것으로, 2024년 태국行 가다랑어 수출이 $150.1M(가치 14.8%, 태국 수입 3위 공급국 — UN Comtrade)이나 캔용 원물 단가가 $1.4~1.9/kg에 그치는 저마진 벌크입니다. 경로 B는 한·영 자유무역협정(Korea-UK FTA, 완제품 0%)으로 영국에 직수출하는 것으로, 영국이 태국산 완제품에 매기는 최혜국(MFN) 관세를 우회해 고부가 스시·투고 채널을 겨냥합니다(태국 대비 관세 격차의 정확한 폭은 영국 품목별 세율 1차 확인 필요). 태국의 對美 19% 상호관세(2025-08, USTR)가 원료가를 압박할수록 경로 A의 매력은 약화되고 경로 B의 상대 가치가 상승합니다.",
        ...
        source: "UN Comtrade 2024(태국行 한국 가다랑어 $150.1M·14.8%·태국 수입 3위, IndexBox 재가공과 일치) / 방콕 가다랑어 캔용 단가 $1.4~1.9/kg(업계 추정) / 한·영 자유무역협정(Korea-UK FTA, 완제품 0%) / 미 USTR 상호관세 19%(태국, 2025-08) / 영국 對태국 MFN 관세격차는 정성 추정 / 신라홀딩스 공식(선단 구성)",
      }}
      customBody={
