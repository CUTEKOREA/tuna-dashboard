...
import { getSmartRotation, truncateKoreanLabel, truncateXAxis } from '@/lib/chart-standards';
import {
  SKJ_HUBS,
  type PriceTimeline,
...
/** 운반선 국적별 척수. 파나마·바하마는 소유국이 아니라 깃발국이다 — 캡션이 그 사실을 진다. */
export function CarrierFlagChart({ rows }: { rows: CarrierFlagRow[] }) {
  const animate = !useReducedMotion();
  const rotation = getSmartRotation(rows.map((row) => row.국적));
  ...
  return (
    <SafeResponsiveContainer width="100%" height={300}>
      <BarChart data={rows} margin={{ top: 12, right: 16, left: 0, bottom: 8 }}>
        ...
        <YAxis {...AXIS} allowDecimals={false} />
        <Tooltip content={<Tip unit="척" />} />
        <Bar dataKey="척수" name="운반선 (척)" fill={TUNA_ROLE.volume} radius={[3, 3, 0, 0]} isAnimationActive={animate} />
      </BarChart>
    </SafeResponsiveContainer>
  );
}

/** 운반선 소유사 상위 — 개인 소유는 집계 단계에서 익명 처리돼 여기 없다. */
export function CarrierOwnerChart({ rows }: { rows: CarrierOwnerRow[] }) {
  const animate = !useReducedMotion();
    ...
    <SafeResponsiveContainer width="100%" height={320}>
      <BarChart data={rows} layout="vertical" margin={{ top: 8, right: 16, left: 8, bottom: 8 }}>
        ...
        <YAxis
          ...
          {...AXIS}
          width={150}
          tickFormatter={(value: string) => truncateKoreanLabel(value, 14)}
        />
        <Tooltip content={<Tip unit="척" />} />
        <Bar dataKey="척수" name="운반선 (척)" fill={TUNA_ROLE.processed} radius={[0, 3, 3, 0]} isAnimationActive={animate} />
      </BarChart>
    </SafeResponsiveContainer>
  );
}
