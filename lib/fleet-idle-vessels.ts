...
export const FLEET_IDLE_NOTES: Record<string, FleetIdleNote> = {
  MOAMARI: {
    asOf: '2026-09-16',
    headline: '프로펠러 볼트 3개 파손(선장 보고서 8/18 확정) 수리차 젠산 예인 중 · 계약 항해 24~26일 · 도착 예정이 보고마다 흔들린다(9/14 10/11 → 9/15 10/4 → 9/16 10/1) · 10/1 도착 기준 항해 31일 예상 · 예인료 계약 일 $41,000, 31일 가정 시 약 127만불(1,390원/$ 환산 17.7억원)으로 계약 대비 5~7일·$20.5만~28.7만 초과 예상 · 복귀 10/11~11/5 예상 · 조업손실 약 810~1,160 MT 예상',
    lines: [
      ...
      { label: '예인 진행 (예상)', text: '8/31 출발은 확정. 일일보고의 도착 예정이 9/7 10/4 → 9/8 10/15 → 9/9 10/16 → 9/10 10/11 → 9/11 10/20 → 9/14 10/11 → 9/15 10/4 → 9/16 10/1 로 계속 바뀐다. 최신 10/1 기준이면 항해 31일, 평균 6.2노트다. 계약 상한(26일)보다 5일·하한(24일)보다 7일 늦다. 보고 위치로 따로 재면 9/15 까지 약 1,820해리를 왔고 약 2,830해리가 남았다 — 직전 하루는 6.9노트로 빨라졌고(그 속도면 10/2 전후) 출발 이후 누적 평균은 5.0노트다(그 속도면 10/8 전후). 보고 예정일은 최근 속도가 유지된다는 가정에 가깝다.' },
      { label: '예인료 (계약 확정 / 총액 예상)', text: '일 $41,000은 계약 확정. 총액은 항해일수에 달려 있어 예상치다 — 계약 24~26일이면 $98.4만~106.6만, 10/1 도착 기준 31일이면 약 $127만(1,390원/$ 환산 17.7억원)이다. 도착 예정이 앞당겨지며 9/14 보고의 10/11(41일·약 $168만)에서 $41만이 줄었다. 자금팀 추산 「약 123만불(일 5,700만원·30일 약 17억원)」은 9/29 도착 기준인데, 지금 기준과는 $4만 차이로 가까워졌다. 계약 초과 5~7일분 $20.5만~28.7만(약 2.9~4.0억원)의 부담 주체가 용선 계약서상 누구인지 확인이 먼저다.' },
      ...
      { label: '수리 (긴 쪽, 예상)', text: '잠수부 점검에서 프로펠러 날개·축계까지 번진 것으로 나오면 젠산 입거 2~4주. 볼트 3개가 파손될 만큼의 하중이 걸렸다는 점이 이 쪽 가능성을 남긴다. GMTS 주간보고(9/9)는 젠산 조선소 입거 예정을 9/29~30 으로 적었다 — 일일보고의 도착 예정과 며칠 차이가 난다.' },
      { label: '복귀', text: '어장까지 1,945해리, 11노트 기준 7일. 10/1 도착 + 수리 3~28일 + 복귀 7일이면 어장 복귀는 10/11~11/5이다.' },
      { label: '조업손실', text: '마지막 어획 8/13부터 어장 복귀까지 59~84일이다. 보고 주기(157보고일 / 244달력일)로 환산하면 38~54보고일이고, 일평균 21.43 MT를 곱해 약 810~1,160 MT다. 8/14 이후 이미 발생한 분을 포함한 값이며 MOAMARI 연간 실적 3,365 MT의 24~34%에 해당한다.' },
      { label: '확정 / 예상 구분', text: '확정은 사고 경위(8/17 접안 중 로프 감김)·프로펠러 볼트 3개 파손·출발지·도착지·일 $41,000·계약 항해 24~26일·8/31 출발이다. 프로펠러 추가 손상 여부, 도착 10/1, 항해 31일, 총 예인료 약 127만불, 수리 3~28일, 복귀 10/11~11/5, 조업손실 810~1,160 MT은 전부 예상치 또는 미확인이다. 도착 예정일은 일일보고마다 바뀐다 — 9/7 이후 10/4·10/15·10/16·10/11·10/20·10/11·10/4·10/1 로 한 방향으로 밀리지도 않는다. 잠수부 점검 결과·젠산 조선소 견적·실제 도착일이 나오면 갱신한다.' },
    ],
  },
};
