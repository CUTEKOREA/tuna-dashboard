...
describe('fleet detail server authorization', () => {
  ...
  it('returns 401 when the server cannot verify a user', () => {
    expect(evaluateFleetAccess(null, null, new Set())).toEqual({
      ok: false,
      status: 401,
  ...
  it('rejects unconfirmed, non-allowlisted, app admin, and user metadata role claims', () => {
    ...
    expect(evaluateFleetAccess(CONFIRMED_USER, 'aal2', new Set())).toEqual({
      ok: false,
      status: 403,
  ...
  it('requires AAL2 after explicit fleet authorization', () => {
    const allowlist = new Set(['operator@example.com']);
    expect(evaluateFleetAccess(CONFIRMED_USER, 'aal1', allowlist)).toEqual({
      ok: false,
      status: 403,
      code: 'mfa_required',
    });
    expect(evaluateFleetAccess(CONFIRMED_USER, 'aal2', allowlist)).toEqual({
      ok: true,
      userId: 'fleet-user-1',
      email: 'operator@example.com',
      aal: 'aal2',
