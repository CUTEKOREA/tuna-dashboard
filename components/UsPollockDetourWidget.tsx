...
const UsPollockDetourWidget = () => {
  ...
  return (
    <WidgetCard
      ...
      iconColor="#ef4444"
      pillar="S3"
      cardDesc={`미국 대러 수산물 수입 금지(2022-03) 전후 ${HS_LABEL_KR['030475']} 의 중국 가공 우회 경로 추적 (HS 030475)`}
      unit="(단위: $M)"
      ...
      chart={
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
            ...
            <ReferenceLine x="2022-03" stroke="#ef4444" strokeDasharray="3 3" label={{ position: 'insideTopLeft', value: '대러 수산물 수입 금지', fill: '#ef4444', fontSize: 11 }} />
            <Line type="monotone" dataKey="RUSSIA" name={`${krCountry('RUSSIA')} (직접)`} stroke="#f59e0b" strokeWidth={2} dot={{ r: 2 }} isAnimationActive={false} />
            <Line type="monotone" dataKey="CHINA" name={`${krCountry('CHINA')} (이중냉동 가공)`} stroke="#ef4444" strokeWidth={2} dot={{ r: 2 }} isAnimationActive={false} />
            <Line type="monotone" dataKey="INDONESIA" name={`${krCountry('INDONESIA')} (신규)`} stroke="#10b981" strokeWidth={2} dot={{ r: 2 }} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      }
      takeaway={{
