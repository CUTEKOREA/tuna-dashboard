...
/* ── 한국의 두 경로: 태국 원물공급 vs 영국 완제품 직수출 ──
   검증: 태국行 가다랑어 수출 $150.1M(14.8%·3위 공급국, IndexBox 2024),
   원물 캔용 단가 $1.4~1.9/kg(Bangkok 가다랑어), 태국 對美 19% vs 영국 Korea-UK FTA 0%,
   신라 선단 선망17·연승9·운반1(공식). ※부가가치 '7~12x' 배수는 어종 비교오류로 삭제. */
const ROUTES = [
  {
    rc: '#38bdf8', tag: '경로 A — 원물 공급', title: '한국 → 태국 가공 허브',
    rows: [['태국行 가다랑어 수출', '$150.1M (14.8%)'], ['공급국 순위', '태국 수입 3위'], ['원물 캔용 단가', '$1.4~1.9/kg'], ['리스크', '태국 對美 19% 관세 압박 → 원료가 하방']],
    foot: '저마진 벌크 원물. 태국 단일 고객 의존도가 가격 협상력을 약화.',
  },
  {
    rc: '#10b981', tag: '경로 B — 완제품 직수출', title: '한국 → 영국 (FTA 우회)',
    rows: [['Korea-UK FTA 관세', '완제품 0%'], ['태국 대비 우위', '태국 MFN 20%p 핸디캡'], ['타깃', '스시·투고 외식 식자재'], ['포지션', '태국 가공 우회·고부가']],
...
export default function SasKrDualRoute() {
  return (
    <WidgetCard
      ...
      cardDesc="한국의 태국 원물공급·영국 완제품 직수출 두 경로 비교 — IndexBox 2024·신라 공식·FTA"
      takeaway={{
        situation: "한국은 영국/태국 밸류체인에서 두 갈래 경로를 동시에 갖습니다. 경로 A는 태국 가공 허브에 원물을 공급하는 것으로, 2024년 태국行 가다랑어 수출이 $150.1M(가치 14.8%, 태국 수입 3위 공급국)이나 캔용 원물 단가가 $1.4~1.9/kg에 그치는 저마진 벌크입니다. 경로 B는 Korea-UK FTA(완제품 0%)로 영국에 직수출하는 것으로, 태국의 영국 MFN 20% 핸디캡을 우회해 고부가 스시·투고 채널을 겨냥합니다. 태국의 對美 19% 관세가 원료가를 압박할수록 경로 A의 매력은 약화되고 경로 B의 상대 가치가 상승합니다.",
        actionPlan: "자원 배분을 경로 B(영국 직수출) 쪽으로 점진 이동하십시오. ① 경로 A는 태국 단일 고객 의존을 낮추고 EU·일본 직판을 병행해 원료 협상력을 방어 — 신라 선단(선망 17·연승 9·운반 1)의 원양 어획을 벌크 원물에만 묶지 마십시오. ② 경로 B는 무관세(0%) + MSC·스시그레이드 프리미엄을 결합해 영국 외식 식자재로 직진입 — 태국 가공 마진을 한국이 내재화하는 구조 전환입니다. 태국 관세·ESG 리스크 상승기에 경로 B 비중 확대가 정답입니다.",
        source: "IndexBox 2024(태국行 한국 가다랑어 $150.1M·14.8%·3위) / Bangkok 가다랑어 캔용 단가 $1.4~1.9/kg / Korea-UK FTA(완제품 0%) / 신라홀딩스 공식(선단 구성)",
      }}
      customBody={
