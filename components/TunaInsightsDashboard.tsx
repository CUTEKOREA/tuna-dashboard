      ...
      <div className={styles.insightCard}>
          ...
          <div className={styles.chartContainer}>
            <SafeResponsiveContainer width="100%" height="100%">
              <LineChart data={mockArbitrageData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <YAxis stroke="#94a3b8" />
                <RechartsTooltip contentStyle={{ backgroundColor: '#1e293b', border: 'none', borderRadius: '8px', color: 'var(--text-primary)' }} />
                <Legend />
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            ...
            actionPlan="[Capital Allocation Strategy] 단순 원물 트레이딩 볼륨을 축소하고, 가공 차익(Processing Margin)을 극대화하는 \'Value-Add\' 라인으로 자본을 전면 재배치해야 합니다. 특히 EU향 무관세 혜택(Tariff Advantage)이 있는 에콰도르 내 톨링(Tolling) 파트너십을 즉시 체결하여 규제 차익(Regulatory Arbitrage)까지 동시에 확보하십시오."
          />
        </div>
      </div>
      ...
      <div className={styles.insightCard}>
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            ...
            actionPlan="[ESG Compliance Audit] 무역 불일치 물량이 당사의 소싱 파이프라인(Supply Chain)에 1%라도 섞여 들어올 경우, 서구권 메이저 리테일러의 상장 폐지급 벤더 퇴출 리스크가 존재합니다. 즉시 제3자(Third-party) 블록체인 이력 추적 시스템을 도입하여 밸류체인의 무결성을 투명하게 증명(Auditability)하고, 이를 마케팅 무기로 역활용하십시오."
          />
        </div>
      </div>
    </>
      ...
      <div className={styles.insightCard}>
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            ...
            actionPlan="[Capex Reallocation] 불확실성이 극심한 원양 어선 건조(Hardware)에 대한 CAPEX 승인을 전면 보류하십시오. 조업 의존형 구조에서 탈피하여, 지중해 및 호주 등지의 최상위 지분 구조를 가진 양식(Ranching/Farming) 인프라 또는 배양 기술(Bio-tech) 스타트업으로 전사적 투자가용자본(Dry Powder)을 전면 이동시켜야 합니다."
          />
        </div>
      </div>
  ...
  const renderMacroTrack = () => (
    <>
      <div className={styles.insightCard}>
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            situation="[Supply Monopoly Risk \(HHI\)] 글로벌 수출국 상위 3개국의 어획 할당 통제력이 심화되며, 시장 집중도(HHI)가 위험 수위인 2,950(Danger Zone)을 돌파했습니다. 이는 글로벌 바이어(캔 제조사)들의 네고 권력이 붕괴되고 원자재 공급사들의 마진 스퀴즈(Margin Squeeze) 횡포가 본격화되었음을 시사합니다."
  ...
  const renderInnovationTrack = () => (
    <>
      <div className={styles.insightCard}>
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            situation="[AI-Driven Precision Fishing] 맹목적 탐색 조업(Blind Searching)의 시대가 종료되었습니다. 3D 소나 및 위성 통신 기반 AI 집어장치(FADs)의 결합으로 조업 성공률(CPUE)은 비약적으로 상승(+15%)하는 동시에 핵심 원가인 선박 연료비(MGO Cost)는 급감(-28%)하는 전형적인 기술-원가 구조 혁신(J-Curve)이 발생 중입니다."
            actionPlan="[Operational Capex Deployment] 구형 아날로그 선단의 퇴출이 임박했습니다. 즉각 전 선단에 대한 디지털 레트로핏(Retrofit) CAPEX 예산을 승인하십시오. 정밀 조업 시스템 장착은 단순 원가 절감을 넘어, 다가오는 해운업계 스코프 3(Scope 3) 탄소 배출 규제 페널티를 회피하는 가장 확실한 ESG 재무 헷징 수단입니다."
          />
        </div>
      </div>
      ...
      <div className={styles.insightCard}>
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            ...
            actionPlan="전통 라인업(아시아 가공->수출)에 의존하는 벤더망을 즉시 조정하고, 멕시코/에콰도르 및 미국 로컬 온쇼어 가공 설비를 인수한 업체들의 우회 공급망으로 구매 포트폴리오를 전환해야 합니다."
          />
        </div>
      </div>
      ...
      <div className={styles.insightCard}>
          ...
          <div className={styles.chartContainer}>
            <SafeResponsiveContainer width="100%" height="100%">
              <BarChart data={mockMSCPremium} margin={{ top: 20, right: 30, left: 0, bottom: 5 }} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="rgba(255,255,255,0.1)" />
                <XAxis type="number" domain={[0, 250]} stroke="#94a3b8" />
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            ...
            actionPlan="[Compliance & Pricing Strategy] 인증 확보는 더 이상 CSR 부서의 마케팅 비용이 아니라, 생존과 초과수익을 가르는 핵심 무형자산(Intangible Asset)입니다. 전 선단 및 가공 라인의 MSC-COC 인증 획득 및 갱신을 전사 최우선 CEO KPI로 락인(Lock-in)하고, 인증 획득에 소요되는 컨설팅 비용을 무제한 승인하십시오."
          />
        </div>
      </div>
      ...
      <div className={styles.insightCard}>
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            situation="[Alternative Protein Disruption] 양식 참치의 생물학적 한계(극악의 사료전환효율 FIFO)와 글로벌 어분(Fishmeal) 원가 폭등이 결합되어, 푸드테크 기반의 식물성(Vegan) 및 세포 배양(Cultivated) 대체 참치 시장이 연 7.8% CAGR의 구조적 메가 트렌드($1.59B)로 폭발하고 있습니다."
            actionPlan="[Future Food-Tech M&A] 레거시(Legacy) 어획 산업의 단백질 생산 한계치(Cap)에 도달했습니다. 벤처캐피털(CVC) 조직을 즉각 가동하여 글로벌 세포 배양 수산물 스타트업에 대한 시리즈 A/B 지분 투자를 단행하십시오. 단일 생물 단백질 회사를 넘어 글로벌 \'Alt-Protein\' 포트폴리오를 거느린 푸드테크 지주사로 기업가치(Valuation)를 재평가(Re-rating) 받아야 합니다."
          />
        </div>
      </div>
      ...
      <div className={styles.insightCard}>
        ...
        <div style={{ padding: '0 20px 20px 20px' }}>
          <TakeawayBox
            situation="[Downstream Margin Extraction] 가공 공정에서 폐기되거나 사료용으로 헐값에 넘겨지던 52%의 참치 부산물(머리, 뼈, 내장)이 고양이용 프리미엄 펫푸드 및 바이오/해양 콜라겐 시장으로 전용(Upcycling)되면서, 본업(Canned Tuna 8.5%)을 압도하는 28.5%의 비정상적 초과 영업이익률(Operating Margin)을 창출하고 있습니다."
            actionPlan="[Vertical Integration Execution] 부산물은 폐기물이 아니라 가장 수익성 높은 숨겨진 캐시카우(Hidden Cash-cow)입니다. 어분 라인 매각을 즉시 중단하고, 부산물 원료를 활용한 자체 \'하이엔드 펫 밀(Pet Meal) 팩토리\' 및 바이오-오메가3 추출 설비 구축에 즉각적인 조인트 벤처(JV) 자본을 투입하여 밸류체인 완전 수직 계열화를 달성하십시오."
          />
        </div>
      </div>
    </>
