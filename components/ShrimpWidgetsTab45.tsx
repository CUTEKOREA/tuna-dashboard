...
export const W41_ProcTimeline = () => (
  <WidgetCard title="가공식품화(Processing) 퀀텀 점프 지수" icon={Factory} term="Processed Timeline" desc="원물이 '공산품'으로 진화하는 우상향 곡선" source="FAO Processed Timeline" situation="단순 1차 원물(Raw Commodity) 시장을 벗어나, 글로벌 HMR(가정간편식) 메가트렌드 폭발과 동기화되어 고부가가치 2/3차 가공(Processed Value-Add) 제품군의 물동량이 폭발적으로 우상향(Quantum Jump) 중입니다." actionPlan="[R&D CAPEX Re-targeting] 원양어선 수리 등 구시대적 CAPEX 집행을 전면 중단하십시오. 전사 R&D 예산의 80% 이상을 고부가가치 '특제 소스 배합(Sauce Recipe)' 및 '튀김 공정(Batter & Breading)' 특허 출원에 집중 투하하여 딥테크 푸드 밸류 기업으로 전면 피벗해야 합니다." telemetry={{ status: 'STATIC', syncDate: '2026-05-29' }}>
...
export const W45_DiseaseRisk = () => {
  ...
  return (
    <WidgetCard title="EMS/WSSV 질병 아포칼립스 경보" icon={AlertTriangle} term="Disease Collapse" desc="치사율 100% 전염병 창궐에 의한 붕괴 차트" source="노트북LM 심층 리서치 - EMS/EHP 충격" situation="2013년 아시아 권역을 초토화시킨 조기폐사증후군(EMS) 사태 당시 글로벌 양식 물동량의 50%가 증발하며 기준 시세가 200% 수직 폭등하는 최악의 생물학적 블랙스완(Tail Risk)이 입증되었습니다." actionPlan="[Geographic Hedging Protocol] 양식업 최대 리스크인 팬데믹(WSSV/EHP) 리스크를 원천 봉쇄하십시오. 인도, 베트남, 에콰도르 3개 대륙에 소싱 파이프라인을 33%씩 완벽히 물리적 분산(Geographic Hedging)시켜, 경쟁사 공급망 마비 시 반사이익을 수취하는 시스템을 가동해야 합니다." telemetry={{ status: 'STATIC', syncDate: '2026-05-29' }}>
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W47_DualSourcing = () => {
  ...
  return (
    <WidgetCard title="신라교역 소싱 트라이앵글 작전판" icon={Compass} term="Dual Sourcing Radar" desc="국가별 장단점 시가전 배틀 카드" source="Strategic Internal Assessment" situation="에콰도르는 원물 캐파(Volume)는 이나 물류 리드타임과 1차 가공 퀄리티에 치명적 약점이 존재하며, 베트남은 물량 한계에도 불구하고 0% 무관세 혜택과 최고 수준의 가공 역량(Value-Add)을 보유한 비대칭(Asymmetric) 구조입니다." actionPlan="[Strategic Portfolio Balancing] 단일 국가 의존형 소싱 체계를 재검토 필요. 무한리필 B2B 및 식자재 마켓에는 에콰도르산 벌크(HLSO) 물량을 투하(Dumping)하고, 하이엔드 오프라인 리테일(초밥/밀키트)에는 베트남산 프리미엄 가공품을 꽂아 넣는 철저한 '투 트랙 듀얼 소싱(Dual-Sourcing)' 포트폴리오를 락인해야 합니다." telemetry={{ status: 'STATIC', syncDate: '2026-05-29' }}>
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W50_StratScorecard = () => {
  ...
  return (
    <WidgetCard title="신라교역 새우 유니버스 장악 스코어카드" icon={ShieldCheck} term="Strategy Scorecard" desc="모든 분석을 총망라한 현주소 및 목표점" source="AI Executive Summarizer (NotebookLM)" situation="대시보드 49개 퀀트 지표 종합 분석 결과, 당사의 유동성(Liquidity) 및 원물 소싱 바잉파워(Buying Power)는 Global Top-Tier 수준이나, 부가가치를 창출하는 '자체 브랜드 파워(IP)' 및 '하이엔드 가공 밸류에드 역량'은 심각한 언더퍼폼(Underperform) 상태로 확인되었습니다." actionPlan="[Bolt-on M&A & HMR Pivot] 전통적인 원양 1차 벤더(Tier-1)의 비즈니스 한계를 인정하고 껍질을 깨십시오. 즉시 가용 유동성을 총동원하여 푸드테크 기반 가공 스타트업에 대한 공격적 볼트온 M&A(Bolt-on M&A)를 결행하고, HMR 시장 직접 침투(Direct-to-Consumer)에 사활을 걸어야 차세대 밸류에이션 리레이팅이 가능합니다. (Re-rating Expected)" telemetry={{ status: 'STATIC', syncDate: '2026-05-29' }}>
      <SafeResponsiveContainer width="100%" height="100%">
        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
          <PolarGrid stroke="rgba(255,255,255,0.2)" />
