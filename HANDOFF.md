...
> - **docx 표 2개는 이미지로 들어 있어 텍스트 추출로는 안 나온다.** `word/media/image1·2.png` 를 꺼내 읽었고, 행·열 합계를 전부 재계산해 원문과 일치함을 확인했다(월별 처리량 26,118 MT / 230일 / 113톤, CBU 1,036·934·−102, FBU 48). 원문 오류 없음.
> - **9월 계획이 두 판본에서 다르다 — 확인 필요.** docx 표는 9월 **2,520 MT(21일 × 120톤)**, pptx 에서 읽어 둔 값은 **2,310 MT(21일 × 110톤)**. 후자는 8월 변경치와 같은 값이고, 그대로 두면 12개월 합이 25,908 이 되어 **두 판본이 함께 인쇄한 연간 개정치 26,118 과 어긋난다.** docx 를 정본으로 삼고 pptx 값은 `productionPlan.septemberPptxRaw` 로 남겨 테스트가 이 불일치를 고정한다. pptx 원본 재확인은 Drive 스트리밍이 느려 이번 세션에 못 했다.
