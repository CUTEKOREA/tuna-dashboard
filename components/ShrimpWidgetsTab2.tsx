...
export const W19_TradeBalance = () => (
  <WidgetCard title="주요 국가간 무역수지(Surplus) 흑적자 현황" icon={Scale} term="Trade Surplus" desc="수출액 - 수입액" source="FAO Data Processed" situation="절대적 흑자를 기록하는 공급 허브와, 블랙홀처럼 빨아들여 적자를 내는 거대 소비 시장(미/일/유)의 양극화 구조." actionPlan="수출 흑자국(공급 파워셀러)과 수입 적자국(소비 타겟)을 연결하는 크로스보더(Cross-border) 브로커리지 팀을 신설하여 물류 마진을 추가 창출.">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W20_TradeCAGR = () => (
  <WidgetCard title="무역 팽창 엔진 (CAGR 속도계)" icon={TrendingUp} term="Compound Annual Growth Rate" desc="수출 물동량 장기 연평균 성장률" source="FAO Data Processed" situation={`1976년 이래 새우 글로벌 무역 물동량은 연평균 성장률 약 8%라는 경이로운 장기 우상향 복리 엔진을 가동해 왔습니다.`} actionPlan="수산업 전 종목 중 가장 강력한 복리 방어력을 가진 상품입니다. 사모펀드 자금을 유치하여 창고 인프라를 무한 확장하더라도 미스매칭(Mismatching) 리스크가 0에 수렴합니다.">
