...
export default function SasUsMarginWaterfall() {
  return (
    <WidgetCard
      ...
      takeaway={{
        situation: "미국 냉동 사시미급 참치 단계별 단가는 단일 검증 출처가 없는 자체 추정치로, 원물 라운드 ~$2.5/kg에서 수입 CIF 로인 ~$8, 도매 사쿠블록 ~$12, 디스트리뷰터 ~$17을 거쳐 외식 식자재 환산가(수율 45~60%·식자재율 28~32% 보정) ~$28/kg까지 약 11배로 누적됩니다. 마진의 대부분은 가공(로인→사쿠)과 외식 단계의 수율 손실 보정에서 발생하며, 외식 메뉴가는 트림 수율 때문에 도매가의 1.7~2.2배로 재계산됩니다. 디스트리뷰터 단가는 소매 게시가 기준 근사치입니다.",
