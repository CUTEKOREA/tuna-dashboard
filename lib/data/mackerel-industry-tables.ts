 ...
 *
 * 발행본이 정본인 이유는 정정이 거기 쌓이기 때문이다. 이 편은 NEAFC 권고 제13호의
 * 「자원보호 유보분 1,495톤은 쿼터가 아니다」처럼 한 단어가 성격을 바꾸는 값이 많고,
 * 그 단서가 발행본 문장에 붙어 있다. 대시보드가 숫자만 다시 계산하면 단서가 떨어진다.
 *
 * 아키텍처 가드: 원본 JSON import 는 이 인테이크 모듈에서만 한다.
 */
import raw from './mackerel-tables.json';

export interface MackerelReportTable {
...
interface Stage {
  tables: MackerelReportTable[];
}

const STAGES = raw as Record<string, Stage>;

/** 단계 키(`s01`~`s09`)로 그 단계에 배치된 보고서 표를 가져온다. */
export function getMackerelTables(stage: string): MackerelReportTable[] {
  return STAGES[stage]?.tables ?? [];
}

export function getMackerelTableStages(): string[] {
  return Object.keys(STAGES);
}

/** 검증용 — 발행본에서 옮긴 표 총수. */
export function getMackerelTableCount(): number {
  let n = 0;
