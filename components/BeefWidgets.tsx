...
export function W1_ProductionTrend({ accent }: any) {
  return <W title="글로벌 소고기 생산량 10년 추이" icon={Globe} accent={accent} pillar="S1"
    ...
    sit="2024년 글로벌 생산량 7,386만톤 — 10년간 +8.9% 완만한 성장. 가격 지수는 100→156으로 +56% 폭등, 공급은 정체된 반면 신흥국 수요가 폭발."
    strat="중국·동남아 수요 급증으로 인한 구조적 가격 상승 사이클. 브라질·호주 산지 직매입선 확보로 중간 마진 30% 절감 가능."
    source="FAOSTAT 작물가축통계(QCL) Item 867 Meat, cattle">
    <ComposedChart data={D.productionTrendData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" vertical={false} />
...
export function W2_Top5Producers({ accent }: any) {
  return <W title="글로벌 Top 5 생산국 점유율" icon={Target} accent={accent} pillar="S1"
    ...
    telemetry="FAOSTAT QCL 2023"
    sit="미국(16.5%)·브라질(13.8%) 양강 30.3% 장악. 상위 5국 합산 48.2% — 돼지고기(중국 단독 44%) 대비 분산도 우수."
    strat="단일국 의존 리스크는 낮으나 브라질 BSE/벌목 환경 리스크는 EU 그린딜 규제 강화 시 즉시 발화 가능. 호주·미국 이중 헤지 권장."
    source="FAOSTAT 작물가축통계(QCL) Item 867">
    <BarChart data={D.top5ProducersData} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" horizontal vertical={false} />
      ...
      <RT content={<CT />} /><Legend verticalAlign="top" wrapperStyle={{ fontSize: '10px', paddingBottom: '10px' }} />
      <Bar dataKey="production" name="생산량 (천톤)" radius={[0, 4, 4, 0]}>{D.top5ProducersData.map((_, i) => <Cell key={i} fill={COLORS[i]} />)}</Bar>
    </BarChart>
  </W>;
}
