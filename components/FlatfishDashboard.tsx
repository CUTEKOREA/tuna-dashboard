          ...
          {PILLARS.map((s, idx) => {
            ...
            return (
              <button key={s.id} onClick={() => setActivePart(s.id as any)}
                ...
                {isActive && <span style={{ fontSize: '0.6rem', color: 'rgba(var(--w-slate-400-rgb), 0.7)', textAlign: 'center', lineHeight: 1.3, marginTop: '2px', padding: '0 4px' }}>{s.desc.slice(0, 24)}…</span>}
              </button>
            );
          })}
        </div>
