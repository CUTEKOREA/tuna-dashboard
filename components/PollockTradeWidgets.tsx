...
export function Widget6_UnitPrice() {
  ...
  return (
    <WidgetCard
      title="마진 스퀴즈: 수출가 vs 소비국 판가 단가 갭 축소선"
      icon={Activity}
      iconColor="var(--color-danger)"
      pillar="S3"
      telemetry={{ status: 'STATIC', syncDate: '2026-05-21' }}
      cardDesc="원산지(러시아)의 원물 폭등을 따라가지 못해 압사당하는 B2B 중간상인(한국향) 이익 구조"
      takeaway={{
...
export function Widget7_SankeyRoute() {
  return (
    <WidgetCard
      title="Double-Frozen: 제재 우회와 품질 훼손의 딜레마"
      icon={ShieldCheck}
...
export function Widget8_KoreaDeficit() {
  ...
  return (
    <WidgetCard
      title="대한민국 수산계 무역 수지 적자 늪 (Deficit Gap)"
      icon={DollarSign}
      iconColor="var(--color-danger)"
      pillar="S3"
      telemetry={{ status: 'STATIC', syncDate: '2026-05-21' }}
