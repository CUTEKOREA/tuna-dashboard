...
export const W21_KoreaImpTimeline = () => (
  <WidgetCard title="한국 새우 연간 수입 톤수 추이" icon={Target} term="Korea Import Yield" desc="1994년 이후 국내 도입 물량선" source="FAO Data - Korea Target" situation="[Korea Import J-Curve] 대한민국 새우 컨슈머 마켓은 2000년대 이후 구조적 폭발기를 맞이하여, 국가 총수입 물동량이 전례 없는 J커브(J-Curve) 상승 궤도를 그리는 메가 호황장입니다." actionPlan="**[Actionable Insight]** [Volume Game Aggression] 당사 본연의 자본력 기반 바잉 파워(Buying Power)를 극한으로 끌어올리십시오. 프랜차이즈 및 대형 패밀리 레스토랑 B2B 공급 라인을 공격적으로 탈취(Takeover)하여 절대적 소매 점유율 중심의 볼륨 게임(Volume Game)으로 경쟁사를 압살해야 합니다. (Conviction Buy)">
...
export const W22_KoreaSelfSuff = () => (
  <WidgetCard title="대한민국 자급률 (Self-sufficiency)" icon={ShieldCheck} term="Korea Self-Sufficiency" desc="내수 생산 대비 수입 의존율" source="FAO Data Derived" situation={`[Domestic Autonomy Deficit] 대한민국의 현재 로컬 새우 자급률은 ${koreaSelfSufficiency}%라는 절망적 수준으로, 사실상 글로벌 트레이더와 환율 변동성에 영혼이 묶여 있는 리스크 극대화(Maximum Exposure) 상태입니다.`} actionPlan="**[Actionable Insight]** [Strategic Self-Sufficiency CAPEX] 수입 덤핑에 의존하는 리스크를 타개해야 합니다. 즉각 스마트 바이오플락(Bio-floc) 육상 양식장 R&D에 벤처 투자를 단행하여, 국내 무균 프리미엄(SPF) 오프라인 라인업의 부분적 수직 자립화(Vertical Independence)를 이루십시오.">
...
export const W23_KoreaUnitPrice = () => {
  ...
  return (
    <WidgetCard title="한국 수입 도입 단가 경쟁력 갭" icon={DollarSign} term="C/P Gap" desc="글로벌 소싱 대비 매입 단가 퍼포먼스" source="Custom Analysis based on FAO/Korea Customs" situation="[Buying Power Squeeze] 퀀트 엔진 분석 결과, 대한민국은 글로벌 벤더 협상력에서 완벽히 패배하며 경쟁국 대비 톤당 7% 이상의 악성 프리미엄(Korea Discount/Penalty) 페널티를 강제로 지불하고 있습니다." actionPlan="**[Actionable Insight]** [Direct Sourcing Disintermediation] 구시대적 종합상사 의존 구조(Middleman)를 완전히 해체하십시오. 전사 구매 파트를 베트남 까마우(Ca Mau) 등 팩토리 현장에 상주시켜 다이렉트 프라이싱 라인(Direct Pricing Line)을 구축해 유통 마진 누수를 원천 봉쇄해야 합니다. (Conviction Buy)">
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W24_AsiaImpCompare = () => {
  ...
  return (
    <WidgetCard title="동북아 새우 소비 라이벌 타격전" icon={Flag} term="Asia Consumption" desc="한/일/대만 시장 규모 배틀" source="FAO Asian Market DB" situation="[Asian Hegemony Cannibalization] 극강의 하이엔드 소비 마켓이었던 일본의 파이를 한국 시장이 경이로운 속도로 잠식(Cannibalizing)하며 동북아시아 새우 컨슈머 헤게모니가 교체 중입니다." actionPlan="**[Actionable Insight]** [Predatory Sourcing Attack] 철저하게 일본 수출 라인만 태우던 베트남 최상위 팩토리의 고품질 물량(Panko, 초밥용 나비새우)에 프리미엄 웃돈을 얹어 탈취하십시오. 이를 국내 하이엔드 오마카세 프랜차이즈에 덤핑 투하(Dumping Strike)하여 신규 시장 지배력을 장악해야 합니다. (Conviction Buy)">
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W25_KoreaImpInflection = () => {
  ...
  return (
    <WidgetCard title="한국 수입 3단계 인플렉션(Inflection)" icon={Activity} term="Import Inflection" desc="폭증의 티핑포인트 분해" source="노트북LM 심층 리서치 요약 데이터" situation="[Triple Macro Inflection] 2010년 이전의 횡보장은 완전히 종료되었습니다. 1) FTA 무관세 발효 2) 디지털 미디어 먹방 신드롬 3) HMR/밀키트 혁명이라는 3대 매크로 부스터(Macro Boosters)가 연쇄 점화되며 시장 펀더멘털이 리빌딩되었습니다." actionPlan="**[Actionable Insight]** [IQF Sourcing Pivot] 블록 냉동 중심의 낡은 조달 시스템을 즉각 폐기하십시오. 메가트렌드인 HMR 확장에 대응하기 위해, 수입 물량의 80% 이상을 즉시 투입 가능한 하이엔드 개별급속냉동(IQF) 포트폴리오로 전면 피벗(Pivot)해야 합니다. (Conviction Buy)">
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W27_KoreaFTABenefit = () => {
  ...
  return (
    <WidgetCard title="FTA 무관세 파괴력 베네핏 뷰어" icon={ShieldCheck} term="Tariff Defense" desc="국가별 관세 장벽이 수입 마진에 미치는 충격" source="관세청 수출입 무역 통계 코드(0306)" situation="[Tariff Shield Monopoly] 대한민국 새우 수입 생태계 1위 헤게모니를 베트남이 철옹성처럼 방어하는 유일무이한 핵심 무기(Moat)는 VKFTA 발효에 따른 '관세 0% 프리미엄'의 극단적 비대칭성(Asymmetry) 덕분입니다." actionPlan="**[Actionable Insight]** [SECA Option Contingency Plan] 에콰도르와의 SECA 협상이 최종 타결될 경우 20% 관세 족쇄가 풀리며 베트남 카르텔은 붕괴(Bloodbath)됩니다. 관세 철폐 즉시 실행 가능한 남미산 대량 선물 매입 콜옵션(Call Option)을 즉각 세팅하여 사태 반전에 선제 대응하여 잉여현금흐름(FCF)을 극대화하십시오. (Strong Buy)">
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W28_KoreaPremiumGap = () => {
  ...
  return (
    <WidgetCard title="Korea Premium 단가 페널티 추적" icon={DollarSign} term="Country Premium Risk" desc="동일 원물 대비 한국 바이어가 지불하는 눈먼 프리미엄" source="내부 무역 거래장부 대조" situation="[Pricing Power Surrender] 국내 중소 브로커리지들의 자기 파괴적인 소싱 치킨게임(Chicken Game)으로 인해, 현지 팩토리(Packer)들에게 프라이싱 통제권을 완전히 헌납하며 연간 4%의 바보 비용(Korea Premium)을 지불 중입니다." actionPlan="**[Actionable Insight]** [Market Squeeze & Block Deal] 무의미한 중소 수입업자 카르텔을 시장에서 강제 퇴출(Squeeze-out)시키십시오. 사내 막대한 잉여 자금을 투입, 신라교역 명의로 베트남/인니 팩토리에 메가톤급 블록 딜(Block Deal)을 타결하여 현지 CAPA를 100% 독점, 매입 단가(COGS)를 인위적으로 박살내야 합니다. (Conviction Buy)">
      <SafeResponsiveContainer width="100%" height="100%">
...
export const W29_KoreaTradeBal = () => {
  ...
  return (
    <WidgetCard title="한국 새우 무역수지(Trade Balance)" icon={Scale} term="Deficit Gravity" desc="수입 결제 대금 vs 수출 획득 대금 갭" source="KITA 한국무역협회 데이터" situation="[Trade Deficit Anomaly] 수입 92 대 수출 5 라는, 국가 거시 경제 지표를 왜곡시킬 수준의 비정상적 무역수지 적자(Trade Deficit Anomaly)를 창출하는 치명적 자본 유출 핵심 품목입니다." actionPlan="**[Actionable Insight]** [K-Food Reverse Export Arbitrage] 수입한 원물에 밸류를 입혀 다시 수출하는 리버스 엔지니어링(Reverse Engineering)을 가동하십시오. 국내 클린룸에서 K-안주/하이엔드 간편식으로 밸류업(Value-up)한 뒤, 압도적인 K-팝 뷰티 패키징을 입혀 미주 H-Mart 등 교민 마켓에 수 배의 마진으로 역수출하는 라인을 뚫어야 합니다. (Conviction Buy)">
      <SafeResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" horizontal={false} />
...
export const W30_KoreaImpVsGDP = () => {
  ...
  return (
    <WidgetCard title="한국 경제성장률 대비 수입 팽창 속도" icon={TrendingUp} term="GDP Decoupling" desc="불황조차 씹어먹는 괴랄한 소비 지수" source="한국은행 GDP 지수 & 관세청 매시업" situation="[Macro Decoupling Phenomenon] 대한민국의 로컬 GDP 펀더멘털 침체(Recession) 여부와 완벽히 탈동조화(Decoupling)되어, 새우 소비 및 수입 팽창 지수만이 나홀로 우주로 솟구치는 괴랄한 메가트렌드 시그널이 감지됩니다." actionPlan="**[Actionable Insight]** [Recession-Proof Asset Allocation] 새우는 단순한 식자재를 넘어 2030 세대의 마지막 '소비 심리 방어선(Psychological Moat)'입니다. 극단적 불황에도 소비가 무너지지 않는 완벽한 경기방어재(Defensive Asset) 성격이 입증되었으므로, 그룹 차원의 메인 유동성을 새우 밸류체인 장악에 전액 몰빵 배정하여 잉여현금흐름(FCF)을 극대화하십시오. (Strong Buy)">
      <SafeResponsiveContainer width="100%" height="100%">
