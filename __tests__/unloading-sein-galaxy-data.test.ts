...
describe('SEIN GALAXY 방콕 항차', () => {
  ...
  it('9/24 하역(K GROUP Report No.6) - MOAMARI #3-C 어창 온도가 3도 높다', async () => {
    const vessel = await loadSeinGalaxy();
    const [day] = vessel.timeline.filter((entry: { date: string }) => entry.date === '9/24');

    expect(day).toMatchObject({ time: '08:00 ~ 16:00', dailyAmount: 276.69, consignee: 'UC · CMC' });
    expect(day.cumAmount).toBeCloseTo(1_450.82, 6);
    expect(day.allocations).toEqual([
      { consignee: 'UC', amount: 182.99, loads: [
        { sourceVessel: 'MOAKONA', hatch: '#2-B', amount: 94.59 },
        { sourceVessel: 'MOAKONA', hatch: '#2-C', amount: 88.4 },
      ] },
      { consignee: 'CMC', amount: 93.7, loads: [{ sourceVessel: 'MOAMARI', hatch: '#3-C', amount: 93.7 }] },
    ]);
    // 결과보고 XLS 09-24 시트: YF 82.10 · SJ 194.59
    expect(day.speciesAmounts).toEqual({ SJ: 194.59, YF: 82.1 });
    expect(day.remainingAmount).toBeCloseTo(395.18, 6);
    expect(day.remainingAmount + day.cumAmount).toBeCloseTo(vessel.reportedTotal, 6);
    /* MOAMARI #3-C 가 -17~-18℃ 다. 전일 같은 어창이 -20~-21℃ 였으니 3도 올랐다 -
     * 원문 값을 그대로 싣고 비고에도 남긴다(품질 판단은 현장 몫이다). */
  ...
  it('9/25 하역(K GROUP Report No.7) - 강우로 세 시간 늦게 시작했다', async () => {
    const vessel = await loadSeinGalaxy();
    const [day] = vessel.timeline.filter((entry: { date: string }) => entry.date === '9/25');

    expect(day).toMatchObject({ time: '11:10 ~ 14:30', dailyAmount: 195.52, consignee: 'UC · CMC' });
    expect(day.cumAmount).toBeCloseTo(1_646.34, 6);
    expect(day.allocations).toEqual([
      { consignee: 'UC', amount: 116.71, loads: [{ sourceVessel: 'MOAKONA', hatch: '#2-C', amount: 116.71 }] },
      { consignee: 'CMC', amount: 78.81, loads: [{ sourceVessel: 'MOAMARI', hatch: '#3-C', amount: 78.81 }] },
    ]);
    // 결과보고 XLS 09-25 시트: YF 60.30 · SJ 135.22
    expect(day.speciesAmounts).toEqual({ SJ: 135.22, YF: 60.3 });
    expect(day.remainingAmount).toBeCloseTo(199.66, 6);
    expect(day.remainingAmount + day.cumAmount).toBeCloseTo(vessel.reportedTotal, 6);
    expect(day.quality).toContain('08:00~11:10 강우');
    expect(day.quality).toContain('계획 200 MT 대비 -4.480 MT');
    // 9/26 은 UC 한 곳만 130 MT - 항차 마지막 며칠은 물량이 준다
  ...
  it('9/26 하역(K GROUP Report No.8) - MOAKONA 가 B/L 보다 56.69 MT 넘게 마감됐다', async () => {
    ...
    /* 계획은 최초 보고분(예측)이라 실제 작업분과 늘 다르다 - 9/27 계획 80 MT 는
     * 잔량 14.970 MT 와 어긋나지만 오류가 아니다. 잔량에 맞춰 고치지 않는다. */
