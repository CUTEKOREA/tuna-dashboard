  ...
  return (
    <>
      ...
      <div className={styles.card}>
        <CardHeader title="전체 글로벌 메가 트렌드 (1950~2024년 생산량 추이)" icon={Globe} term="Global Megatrend" desc="지난 75년간 명태의 전 세계 생산량 변동 시계열" />
        <div className={styles.cardBody}>
          ...
          <TakeawayBox 
            ...
            situation="[Macro Biomass Re-centering] 1980년대 정점 이후 장기 우하향하던 북태평양 명태 생물량이 최근 오호츠크해 및 베링해의 거시적 수온 이동(Thermal Shift)으로 인해 러시아 EEZ 내에서 폭발적으로 반등했습니다. 공급 패권이 다국적 분산 체제에서 '미국-러시아 복점(Duopoly)' 구조로 완전히 회귀하는 영구적 패러다임 시프트입니다."
            actionPlan="**[Actionable Insight]** [Geopolitical Supply Hedging] 단일 어종 세계 최대 생산량(350만 톤)임에도 불구하고 공급망이 미국·러시아에 독점된 극도의 '병목(Bottleneck) 디스카운트' 상황입니다. 러-우 전쟁에 따른 서구권의 러시아산 해산물 금수 제재(Sanctions) 리스크 및 알래스카 쿼터 감축 쇼크에 대비, 아시아 제3국 경유 우회 물량의 70% 이상을 2~3년 장기 선도계약(Forward Contract)으로 즉시 락인(Lock-in)하고, 12개월분 이상의 안전 재고 비축을 위한 창고 금융(Inventory Financing) 크레딧 라인을 즉각 확대하여 잉여현금흐름(FCF)을 극대화하십시오. (Strong Buy)" 
          />
        </div>
      </div>
      ...
      <div className={styles.card}>
        <CardHeader title="대한민국 명태 자급률 vs 수입 의존도 시계열 분석" icon={ShieldCheck} term="Self-Sufficiency vs Import" desc="국산 원양/연근해산 고갈 현상 및 100%에 달하는 수입 의존도 교차점 투영" />
        <div className={styles.cardBody}>
          <div className={styles.chartContainer}>
            <SafeResponsiveContainer width="100%" height="100%">
          ...
          <TakeawayBox 
            ...
            situation="[Supply Sovereignty Collapse] 국가 주도의 치어 방류 및 연근해 어장 복원 프로젝트(명태 살리기)가 생태학적 한계로 최종 실패하며, '국민 단백질'의 서플라이 체인이 100% 해외 수입에 의존하는 거시적 주권 상실(Sovereignty Crisis) 단계로 진입했습니다. 이제 국내 판매 단가는 철저히 러시아 어선단의 조업 쿼터 및 선가 네고(Pricing Power) 파워에 완전히 종속되었습니다."
            actionPlan="**[Actionable Insight]** [Margin Defense & Premiumization] 자원 회복에 대한 환상을 폐기하고, 원자재 수입 의존형 비즈니스의 치명적 약점인 '수입 물가 폭등 쇼크'를 상쇄할 B2C 마진 방어(Margin Defense)에 전사적 자원을 집중하십시오. 매입원가(COGS) 상승분(Inflation)을 소비자 판가로 즉시 전가(Pass-through)할 수 있도록, 기존의 저단가 탕거리/통마리 유통을 폐기하고 100% '초프리미엄 HMR(저염 명란, 영유아용 뼈없는 필레, 펫푸드 연육)' 중심으로 프로덕트 믹스(Product Mix)를 전면 강제 전환해야 합니다. (Conviction Buy)" 
          />
        </div>
      </div>
      ...
      <div className={styles.card}>
        <CardHeader title="최근 5개년 수입 블랙홀 국가 탐지 (2019-2023)" icon={Flag} term="Import Blackholes" desc="가장 공격적으로 명태 물량을 흡수하는 상위 국가들의 시계열 동향" />
        <div className={styles.cardBody}>
          <div className={styles.chartContainer}>
            <SafeResponsiveContainer width="100%" height="100%">
          ...
          <TakeawayBox 
            ...
            situation="[Processing Blackhole Dynamics] 글로벌 H&G 무역 흐름에서 중국 다롄/칭다오 콤플렉스는 압도적 인건비 경쟁력과 가공 인프라로 전 세계 원물의 70% 이상을 흡수하는 '재가공 블랙홀' 지위를 굳혔습니다. 러시아산 블록을 더블프로즌(Double-frozen) 필레로 전환하여 서구권에 역수출하는 이 거대한 공급망의 재고 회전율(Inventory Turnover)이 글로벌 명태 선물 가격을 통제하는 핵심 인덱스입니다."
            actionPlan="**[Actionable Insight]** [Dual-Hub Tolling Strategy] 단순 중간 무역상(Trader) 지위를 탈피하여, 중국 거대 가공 밸류체인에 직접 침투(Penetration)하십시오. 중국 현지 대형 가공장과의 독점적 톨링(Tolling, 위탁가공) 계약을 체결해 프라이빗 라벨(PB) 연육 및 필레 수출 물량을 락인함과 동시에, 거시적 '미중 디커플링(De-coupling) 관세 리스크' 폭발에 대비하여 베트남/인도네시아에 30%의 대체 가공 허브(Alt-Hub) 물량을 분산 배치하는 '투-트랙 리스크 헤징'을 즉각 이사회에 상정해야 합니다. (Conviction Buy)" 
          />
        </div>
      </div>
      ...
      <div className={styles.card}>
        <CardHeader title="마진율(Spread) 1위 2차 가공 국가 부가가치 스캔" icon={TrendingUp} term="Spread Winners" desc="수입 단가 대비 수출 단가의 차액을 극대화하여 2차 가공 이윤을 창출하는 승자 목록" />
        <div className={styles.cardBody}>
          <div className={styles.chartContainer}>
            <SafeResponsiveContainer width="100%" height="100%">
          ...
          <TakeawayBox 
            ...
            situation="[Value-Add Spread Extraction] 수입 대비 수출 단가 차액인 '가공 스프레드(Processing Margin Spread)' 스캐닝 결과, 독일·네덜란드 등 2차 가공 승자(Spread Winners)들은 자동화 설비를 통한 첨단 필레팅/수리미 추출로 원물 대비 톤당 $1,000~$2,500의 초과수익(Alpha)을 창출하고 있습니다. Bottom-line(순이익)의 핵심 축이 '1차 조업(Fishing)'에서 '수율 극대화 가공(High-Yield Processing)'으로 완전히 전이되었습니다."
            actionPlan="**[Actionable Insight]** [Smart Factory Capex] 재래식 내수용 동태·코다리 유통 모델은 마진 한계(Cap)에 봉착했습니다. 저숙련 인력 기반의 국내 영세 가공 인프라를 전면 폐기하고, Baader社 등 유럽형 초정밀 자동화 필레 머신 및 AI 광학 선별기 기반의 '하이엔드 스마트 팩토리' 건립을 위한 대규모 CAPEX를 승인하십시오. 가공 수율을 단 3%만 끌어올려도 B2B 더블프로즌 블록 수출 루트를 개척하여 부가가치를 서구 리테일 체인으로 전이시켜야 합니다. (Conviction Buy)" 
          />
        </div>
      </div>
      ...
      <div className={styles.card}>
        <CardHeader title="국가별 평균 수입 단가 산출을 통한 등급별 시장 분석" icon={Navigation} term="Price Tiers by Country" desc="Top 15 수입국의 톤당 매입 단가를 분석하여 프리미엄 시장과 대중 매스 시장 분류" />
        <div className={styles.cardBody}>
          <div className={styles.chartContainer}>
            <SafeResponsiveContainer width="100%" height="100%">
          ...
          <TakeawayBox 
            ...
            situation="[Pricing Power & Market Stratification] 글로벌 매입 단가 클러스터 분석 시, MSC 에코라벨에 프리미엄을 지불하는 서북유럽 최상위 그룹(Tier 1)과 단가 저항선에 극도로 민감한 아시아·중남미 볼륨 그룹(Tier 3) 간의 시장 양극화(Bifurcation)가 완성되었습니다. 전통적인 'One-size-fits-all' 가격 산정 모델의 붕괴를 의미합니다."
            actionPlan="**[Actionable Insight]** [Bifurcated Allocation Tactical Execution] 전사적 물량 배정(Allocation) 시스템을 티어별 '분할 타겟팅(Segmented Targeting)'으로 즉각 이원화하십시오. 선상 동결(FAS, Frozen at Sea)된 싱글프로즌 최상급 필레 물량은 전량 유럽/미국 Tier 1 채널로 몰아넣어 초프리미엄 독점 마진을 추출(Skimming)하고, 육상에서 재가공된 더블프로즌(Double-frozen) 물량 및 부산물 연육은 아시아/아프리카 저개발 국가의 스팟 볼륨 마켓(Spot Volume Market)에 고도 회전율로 덤핑(Dumping)하여 워킹캐피탈(Working Capital) 유동성을 즉각 확보하는 바벨(Barbell) 전략을 실행해야 합니다. (Conviction Buy)" 
          />
        </div>
      </div>
    </>
