...
ts_files = glob.glob('components/Cassava*.tsx')
...
def process_ts(filepath):
    ...
    content = re.sub(r"background:\s*'rgba\(0,\s*0,\s*0,\s*0\.2[5]?\)'", "background: '#181818'", content)
    ...
    content = re.sub(r"boxShadow:\s*'0 4px 12px rgba\([^)]+\)'", "boxShadow: 'rgba(0,0,0,0.3) 0px 8px 8px'", content)

    # 4. Background replacements for education and other panels
    ...
    content = re.sub(r"background:\s*'rgba\(255,\s*255,\s*255,\s*0\.05\)'", "background: '#252525'", content)
