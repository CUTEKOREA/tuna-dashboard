...
def s2_aquaculture():
    ...
    return {
        ...
        "strat": "'양식 블루오션' 서사는 통계가 받쳐주지 않는다. 다만 전환 가능성 자체가 없다는 뜻은 아니다 — "
                 "일본 규슈의 축양·시험양식은 FAO 통계에 잡히지 않는 규모로 실재한다(연 300톤대 추정). "
                 "현 시점에서 공급 리스크를 양식으로 헤지할 수는 없으므로, 원산지 다변화와 재고 정책이 실효 수단이다.",
