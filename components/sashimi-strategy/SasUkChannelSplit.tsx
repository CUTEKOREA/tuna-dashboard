...
/* ── 영국 참치 채널 이원화: 캔 리테일 코어 vs 스시·투고 외식 ──
   검증(The Grocer·Restaurant Online·MSC): Itsu £175.9M(+9%)·그로서리 +20%,
   Wasabi £121.6M, YO! £138.3M, 캔=상온수산물 가치66%/물량71%(MSC 2023, 2022.6~2023.6),
   PB MSC 100% 4사. 거시 MSC/FTA 집계는 SasUkMarket에 위임. */
const CHAINS = [
...
export default function SasUkChannelSplit() {
  return (
    <WidgetCard
      ...
      cardDesc="영국 스시·투고 체인 매출 + 캔 채널 점유 — The Grocer·Restaurant Online·MSC(2024-25)"
      takeaway={{
        situation: "영국에서 참치는 2023년 이래 연어를 제친 최다 소비 수산물로, 두 채널로 갈립니다. ① 캔 리테일 코어 — 상온 수산물 가치의 66%·물량의 71%(MSC 2022.6~2023.6 기준)를 점유하는 성숙 시장으로, 4대 리테일러(Waitrose·Sainsbury's·Tesco·Iceland)가 자체브랜드 참치 100% MSC를 달성해 차별화 여지가 좁아진 저마진 PB 구조입니다. ② 스시·투고 외식 성장축 — Itsu(그룹 £175.9M, +9%)·YO! Sushi(£138.3M)·Wasabi(£121.6M)가 주도하며, 특히 Itsu 그로서리가 +20%(£57M)로 외식-소매 경계를 허뭅니다. 단 Wasabi 세후이익이 £7.43M→£1.86M로 급감하는 등 외식 수익성 변동성은 큽니다.",
        ...
        source: "The Grocer·Insider Media(Itsu £175.9M·그로서리 +20%) / Restaurant Online·QSR Media(Wasabi £121.6M) / MSC·SeafoodSource(캔 66%/71%·PB MSC 100% 4사)",
      }}
      customBody={
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
            {[
              ...
              { k: 'PB MSC 100%', v: '4사', s: 'Waitrose·Sainsbury·Tesco·Iceland', c: '#38bdf8' },
            ].map((x) => (
