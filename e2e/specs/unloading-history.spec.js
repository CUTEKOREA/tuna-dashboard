...
async function runHappyPath(browser) {
  ...
  for (const pattern of [
    ...
    /TUM:\s+202\.590 MT/,
    /ISA:\s+148\.150 MT/,
    /일일\s+하역량:\s+350\.740 MT/,
    /하 역 누 계:\s+2013\.100 MT/,
    /명일\(8\/16\)은 공휴일로 하역작업이 없습니다\. 재개일\(8\/17\)은 약 300톤 하역 작업 예정입니다/,
  ]) {
    assert.match(generatedReport, pattern);
  }
  assert.doesNotMatch(generatedReport, /\* SJ:\s+350\.740 MT/);
  assert.doesNotMatch(generatedReport, /350톤/);
  assert.doesNotMatch(generatedReport, /명일\(8\/16\)은 약 300톤/);
...
async function runChunkFailureIsolation(browser) {
  ...
  assert.match(recoveredBody, /35,067\s+MT/);
