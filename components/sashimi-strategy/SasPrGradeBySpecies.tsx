...
/* ── 어종별 사시미 등급 결정 요인 (Catalina·Easyfish·ScienceDirect) ──
   ...
   오토로 지방 약 25~30%(업계 자료·계절 편차). ※법정표준 아닌 업계 관행. */
const SPECIES = [
  ...
  return (
    <WidgetCard
      ...
      takeaway={{
        ...
        source: "Catalina Offshore·Easyfish(황다·눈다=색/참다랑어=지방 판정) / 미오글로빈 산화(옥시→메트미오글로빈 갈변)는 식품과학 일반 / Erdaide 외, Food Chemistry 2016(PubMed 26868578: 황다·눈다·가다 어종별 야케 근단백 비교) / 오토로 지방 비율은 업계 자료(Sushi University 등, 25~30%·계절 편차) / 업계 관행·바이어별 척도(법정표준 아님)",
      }}
      customBody={
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%' }}>
          ...
          <div style={{ fontSize: '0.6rem', color: '#64748b', lineHeight: 1.5, textAlign: 'center' }}>
            색=미오글로빈 산화(옥시→메트=갈변, CO처리가 위장) · 야케=어획 발버둥 화상육 · 오토로 지방 약 25~30%(업계 자료, 계절·개체별 편차)
          </div>
        </div>
      }
    />
