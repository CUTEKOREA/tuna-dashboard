...
> - **Carrot Intelligence Hyper-Pipeline Sync** — 매일 실패. `data/당근/carrot_real_data_collector.py` 를 호출하는데 **`/data/` 가 gitignore 라 러너에 파일이 없다**(`.gitignore:53`). 그 스크립트는 worktree·다른 체크아웃·git 이력 어디에도 없다 — **CI 에서 한 번도 성공한 적이 없는 워크플로**다. 파일을 복원할 수 없어 고치지 못했다. 사용자 결정 필요: 수집기를 `scripts/` 로 커밋하거나, 매일 실패만 쌓는 스케줄을 끄거나.
