...
> ✅ **2026-08-13 17:10 KST — 오징어 v5 교차검증 정정 (P4·P5)** [Claude Code 검수 + Codex 구현 + Grok 교차검증]:
...
> - **P5 아르헨티나 신호**: `데이터공백` → **`어기외`**(기준일 2026-05-28). 주간공보 MD 짝이 없다는 이유로 공백 처리했으나, 같은 아카이브의 CTMFM 결의 2/2026(2026-04-01 공동어로수역 조업 허가)과 CFP 결의 6/2026(2026-05-28, "직전 어기 84척" 과거형)이 어기의 개시·종료를 이미 말하고 있었다. **자료가 없어서가 아니라 안 읽어서 생긴 공백.** 일부 2차 출처가 드는 2026-04-22 폐쇄일은 아카이브에 없어 채택하지 않았다.
...
> - Grok 이 위 두 결함을 제기했고 Claude Code 가 1차 출처로 재검증했다. **Grok 이 제시한 수치는 전부 틀렸다** — 오징어 집합에 문어(160555)를 넣고 조미오징어(160554)를 빼서 오염률 42%·중국 43.7%·2위 베트남으로 계산했다. 실제는 23.1%·46.15%·페루다. 결함 발견은 교차벤더가, 수치 확정은 원본 대조가 했다.
> - 검증: 검증기 self-test **19/19**, 빌더 자체검사 **20/20**, 프론트 **192/192**, `tsc` 0오류, 게이트 39위젯 위반 0, 빌더 재실행 산출물 동일(재현성).
