...
export const W11_TradeVolLong = () => (
  <WidgetCard title="수출입 물동량 롱텀 트렌드" icon={Truck} term="Trade Volume LT" desc="1976년 이후 물동량" source="FAO Trade Timeline Qty" situation="[Global Logistics Expansion] 스마트 양식 기술의 대량생산과 글로벌 콜드체인(Cold Chain) 물류망의 결합으로 거시적 관점의 수출 물동량(Trade Volume) 곡선이 구조적 메가 트렌드(Mega-trend)에 진입했습니다." actionPlan="**[Actionable Insight]** [Commoditization Arbitrage] 새우 원물 자체의 완벽한 글로벌 상품화(Commoditization)가 종료되었습니다. 복잡한 현지 생산 라인보다는 글로벌 환율 및 판가 스프레드를 타겟팅하는 환차익 무역(Arbitrage Trading) 데스크에 자본금(Capital)을 우선 배치하여 잉여현금흐름(FCF)을 극대화하십시오. (Strong Buy)">
...
export const W15_ExpTop10Qty = () => (
  <WidgetCard title="글로벌 수출 물량 Top 10" icon={Truck} term="Export Volume" desc="가장 많이 수출하는 국가" source="FAO Trade Recent" situation="[Duopoly Export Hegemony] 에콰도르(LATAM)와 인도(Asia) 양대 국가가 글로벌 덤핑 출하 물량의 50%를 장악하며 글로벌 공급을 쌍끌이하는 완벽한 양강 체제(Duopoly)를 구축했습니다." actionPlan="**[Actionable Insight]** [Targeted Long-Term Contracting] 신규 벤더 확충 시 동남아권 브로커리지를 패스하십시오. 물량의 정점인 에콰도르 과야킬(Guayaquil) 메이저 수출 팩토리(Supplier) 대표단과의 다이렉트 롱텀(Long-term) 독점 소싱 계약에 협상 자원을 전면 투입해야 합니다. (Conviction Buy)">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W16_ImpTop10Qty = () => (
  <WidgetCard title="글로벌 수입 물량 Top 10" icon={ShoppingCart} term="Import Volume" desc="가장 많이 블랙홀처럼 빨아들이는 국가" source="FAO Trade Recent" situation="[Tri-Polar Demand Vacuum] 미국, 중국, 유럽연합(EU) 3대 메가 컨슈머 마켓이 전 세계 새우 물동량을 진공청소기처럼 빨아들이는 극단적인 수요 쏠림(Demand Tri-Polarization) 상태입니다." actionPlan="**[Actionable Insight]** [Macro Disruption Arbitrage] 미중 무역 전쟁 텐션 및 중국의 내부 소비 셧다운 시 글로벌 시장에 강제 출하되는 '고아 물량(Orphaned Cargo)'을 실시간 모니터링 하십시오. 이를 초저가 덤핑으로 가로채어 한국 로컬 마켓에 쏟아붓는 극강의 아비트라지(Arbitrage) 채널을 세팅해야 합니다. (Conviction Buy)">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W17_ExpTop10Usd = () => (
  <WidgetCard title="글로벌 수출 달러 파워 Top 10" icon={DollarSign} term="Export Value" desc="수출 금액(USD) 기준 랭킹" source="FAO Trade Recent" situation="[Value-Add Disruption Matrix] 순수 톤수(Volume) 랭킹을 완전히 무시하는, 초고정밀 탈각/자숙 밸류에드(Value-Add) 마진 장착 국가들의 경이적인 달러 매출 점유율 전복(Disruption) 현상이 목격됩니다." actionPlan="**[Actionable Insight]** [Margin Defense via Value-Add Sourcing] 원물 단가 싸움에서 패배를 인정하십시오. 무식한 캐파를 앞세운 에콰도르 대신, 가공 기술의 정점에 선 베트남/태국 팩토리의 완제품(Finished Goods) 브랜드를 B2B로 다이렉트 꽂아 넣는 것이 로컬 마진(OPM) 수성에 절대적으로 유리합니다. (Conviction Buy)">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W18_ImpTop10Usd = () => (
  <WidgetCard title="글로벌 수입 지출 달러 파워 랭킹" icon={Flag} term="Import Value" desc="달러를 가장 많이 쓰는 수입국" source="FAO Trade Recent" situation="한국도 무시할 수 없는 수준(Top 10 끝자락)으로 많은 외환 기재를 수입 새우에 태우고 있습니다." actionPlan="**[Actionable Insight]** 단순 수입을 넘어, 한국 본사 자본으로 베트남이나 인니에 선급금(Pre-funding)을 지급하고 통물량을 장악하는 글로벌 트레이딩 하우스 입지를 굳히십시오.">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W19_TradeBalance = () => (
  <WidgetCard title="주요 국가간 무역수지(Surplus) 흑적자 현황" icon={Scale} term="Trade Surplus" desc="수출액 - 수입액" source="FAO Data Processed" situation="절대적 흑자를 기록하는 공급 허브와, 블랙홀처럼 빨아들여 적자를 내는 거대 소비 시장(미/일/유)의 양극화 구조." actionPlan="**[Actionable Insight]** 수출 흑자국(공급 파워셀러)과 수입 적자국(소비 타겟)을 연결하는 크로스보더(Cross-border) 브로커리지 팀을 신설하여 물류 마진을 추가 창출하여 잉여현금흐름(FCF)을 극대화하십시오. (Strong Buy)">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W20_TradeCAGR = () => (
  <WidgetCard title="무역 팽창 엔진 (CAGR 속도계)" icon={TrendingUp} term="Compound Annual Growth Rate" desc="수출 물동량 장기 연평균 성장률" source="FAO Data Processed" situation={`1976년 이래 새우 글로벌 무역 물동량은 연평균 성장률(CAGR) 약 8%라는 경이로운 장기 우상향 복리 엔진을 가동해 왔습니다.`} actionPlan="**[Actionable Insight]** 수산업 전 종목 중 가장 강력한 복리 방어력을 가진 상품입니다. 사모펀드 자금을 유치하여 창고 인프라를 무한 확장하더라도 미스매칭(Mismatching) 리스크가 0에 수렴합니다. (Conviction Buy)">
