...
export const W47_DualSourcing = () => {
  ...
  return (
    <WidgetCard title="국가별 소싱 역량 정성 평가" icon={Compass} term="Dual Sourcing Radar" desc="5개 축 국가별 소싱 역량 — 자체 정성 평가(0~100 상대 점수)" source="자체 정성 평가(Internal Qualitative Assessment) — 외부 1차 출처 아님, 0~100 상대 스코어 (STATIC 2026-05-29)" situation="에콰도르는 원물 생산능력(Volume)은 우수하나 물류 리드타임과 1차 가공 품질에 약점이 있고, 베트남은 물량 한계에도 0% 무관세 혜택과 높은 가공 역량(Value-Add)을 보유한 비대칭(Asymmetric) 구조입니다(점수는 자체 정성 평가 기준)." actionPlan="[Strategic Portfolio Balancing] 단일 국가 의존형 소싱 체계를 재검토 필요. 무한리필 B2B 및 식자재 마켓에는 에콰도르산 벌크(HLSO) 물량을 투하(Dumping)하고, 하이엔드 오프라인 리테일(초밥/밀키트)에는 베트남산 프리미엄 가공품을 꽂아 넣는 철저한 '투 트랙 듀얼 소싱(Dual-Sourcing)' 포트폴리오를 락인해야 합니다." telemetry={{ status: 'STATIC', syncDate: '2026-05-29' }}>
      <SafeResponsiveContainer width="100%" height="100%">
