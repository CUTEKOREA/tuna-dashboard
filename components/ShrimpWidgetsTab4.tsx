...
export const W31_PriceGap = () => (
  <WidgetCard title="수출국별 최종 인도(FOB) 단가 갭" icon={DollarSign} term="FOB Price Gap" desc="가장 비싸게 파는 놈과 싸게 파는 놈" source="FAO Trade Export Value/Volume Derived" situation="[Value-Add Premium Decoupling] 베트남의 딥테크 가공(Value-Add Processing) 믹스 전략이 적중하며, 단순 1차 원물 수출국인 중남미(LATAM) 국가들 대비 톤당 수출 단가(FOB) 곡선이 영구적으로 상방 이탈(Decoupling)했습니다." actionPlan="[Tier-3 Processing Evolution] 낡은 1차 벤더(Tier-1) 포지션을 즉시 포기하십시오. 원물을 껍질째(HLSO) 동결해 떠넘기는 에콰도르식 무식한 볼륨 게임을 버리고, 초정밀 탈각/빵가루 코팅까지 원스톱으로 종료하는 '3차 팩토리 벤더(Tier-3)' 로 전사 비즈니스 모델을 진화시켜야 프라이싱 방어가 가능합니다.">
...
export const W33_PriceTrend = () => (
  <WidgetCard title="글로벌 기준선(Benchmark) 단가 사이클" icon={Activity} term="Global Price Cycle" desc="1990년 이후 새우 톤당 평균 가격의 변동성" source="FAO Trade Master Index" situation="[Extreme Cyclical Volatility] 새우의 도매 베이스라인은 타 육류(가금류/돈육)와 달리, 수년 단위로 극단적 폭등과 폭락장(Boom & Bust)을 오가는 철저한 '사이클릭 펀더멘털(Cyclical Asset)' 성향을 노골적으로 드러냅니다." actionPlan="[Counter-Cyclical Arbitrage] 패닉 셀(Panic Sell)이 쏟아지는 폭락장 골짜기(Trough)야말로 진정한 알파(Alpha) 구간입니다. 공포를 역이용해 현지 팩토리 물량을 싹쓸이하는 선도 매입(Forward Buy)을 체결하고, 콜드체인 터미널 캐파를 120% 가동하는 역발상(Counter-cyclical) 롱 포지션을 지시합니다.">
    <SafeResponsiveContainer width="100%" height="100%">
...
export const W40_GoldenLockin = () => {
  ...
  return (
    <WidgetCard title="골든 타임 바텀 피싱(Bottom Fishing) 인디케이터" icon={Flame} term="Golden Lock-in" desc="장기 이동평균선 터치 시 발생하는 매수 우위 시그널" source="10-Year Moving Average Algorithm" situation="[10-Year MA Bottom-Fishing] 당사 알고리즘 딥-다이브 결과, 현 글로벌 새우 기준 시세는 10년 장기 이동평균선(MA)을 하향 돌파하여 데드 바닥(Absolute Bottom)을 기어 다니는 역사상 유례없는 언더밸류(Deep Under-valuation) 패닉장입니다." actionPlan="[Extreme Leverage Lock-in] C-레벨 즉각 재가를 발동해 회사의 가용 유동성(Liquidity) 영혼까지 끌어모으십시오(Max Leverage). 향후 24개월 소요 예정 B2B 물량을 현재의 쓰레기 덤핑 단가로 100% 롱-텀 락인(Long-term Lock-in) 때리십시오. 내년 반등장이 열리는 순간 조 단위 초과 수익(Alpha) 파티가 열립니다.">
      <SafeResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
          <XAxis dataKey="y" stroke="#94a3b8" fontSize={11} />
