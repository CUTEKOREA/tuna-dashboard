...
import { ComposedChart, AreaChart, Area, BarChart, Bar, Line, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer, Scatter, Legend, ScatterChart } from 'recharts';
import SafeResponsiveContainer from './SafeResponsiveContainer';
import { TrendingUp, AlertTriangle, Ship, Thermometer, ShieldAlert, Crosshair, Globe, AlertTriangle as AlertTriangleLucide } from 'lucide-react';
...
const CardHeader = ({ title, icon: Icon, term, desc }: any) => (
  <div className={styles.cardHeader} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
    <h3 className={styles.cardTitle} style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
      <Icon size={18} className={styles.cardIcon} color="#cbd5e1" /> {title}
    </h3>
...
export function WidgetTACGapTracker() {
  return (
    <div className={styles.card} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <CardHeader 
        title="조업 쿼터 지정학 갭 트래커" 
        icon={Globe} 
        term="Geopolitical TAC Gap" 
        desc="미국 베링해와 러시아 EEZ의 자원 고갈에 따른 어획 할당량(TAC)의 우하향 곡선 및 ス팟 가격 반응선" 
      />
      <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
...
export function WidgetOriginWashAlert() {
  return (
    <div className={styles.card} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <CardHeader 
        title="더블프로즌 원산지 세탁 관세 폭탄" 
        icon={AlertTriangle} 
        term="Russian-Origin Traceability" 
        desc="러시아산 원물이 중국에서 재가공되어 수입될 때 겪는 경제제재 원산지 단속 리스크의 지수화" 
      />
      <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div style={{ height: '260px', width: '100%', flexShrink: 0 }}>
          <SafeResponsiveContainer width="100%" height="100%">
        ...
        <TakeawayBox 
          ...
          actionPlan="원가 절감율이 높다고 하더라도 중국 우회 더블프로즌 라인을 일시 차단하십시오. 미국의 러 제재(Sanctions) 감시망 밖에 있으면서 관세 장벽이 느슨한 베트남(하이퐁) 및 인니 대체 가공 허브(Alt-Hub)로 필레 원물 가공 라인을 100% 리라우팅(Rerouting) 해야 합니다." 
        />
      </div>
    </div>
  );
...
export function WidgetBunkerArbitrage() {
  return (
    <div className={styles.card} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <CardHeader 
        title="합작선사 유류비-해상운임 타격" 
        icon={Ship} 
        term="Bunker-Logistics Arbitrage" 
        desc="선용 MGO(유류비)와 글로벌 해상 컨테이너 스페이스 운임 폭등이 갉아먹는 간접 원가의 마진 타격선" 
      />
      <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div style={{ height: '260px', width: '100%', flexShrink: 0 }}>
          <SafeResponsiveContainer width="100%" height="100%">
        ...
        <TakeawayBox 
          ...
          actionPlan="현지(러시아/미국) 수산업체와 단순 5:5 쿼터 배분 수익 셰어링 모델은 자본잠식의 위험이 높습니다. 유류비 상승 분을 상대 파트너사에 온전히 부담(Hedge)시키고, 한국 측은 Fबोर्ड(FOB) 조건의 인수 금액만 고정시키는 '운임 전가 파생 계약'을 추가 삽입하여 변동성을 조기에 끊어내야 합니다." 
        />
      </div>
    </div>
  );
...
export function WidgetBeringSeaMigration() {
  return (
    <div className={styles.card} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <CardHeader 
        title="기후변화 어장 북상 및 선단 연비 타격 지표" 
        icon={Thermometer} 
        term="Bering Sea Migration & Fuel Shock" 
        desc="수온 상승(Sea Surface Temperature)으로 인한 명태 어장의 극지방 북상 거리와 이에 비례하는 하역 왕복 연비(FC) 상승 지수" 
      />
      <div className={styles.cardBody} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div style={{ height: '260px', width: '100%', flexShrink: 0 }}>
          <SafeResponsiveContainer width="100%" height="100%">
  ...
  return (
    <div className={styles.insightCard} style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0 }}>
        ...
        <h3 style={{ margin: 0, fontSize: '1.2rem', color: '#f8fafc', fontWeight: 700 }}>[Macro] 미·러 무역 상호 관세 파급 맵</h3>
      </div>
      ...
      <div style={{ flexShrink: 0, marginTop: '12px' }}>
        <TakeawayBox 
          ...
          situation="EU 및 미국이 러시아산 직수입 명태에 35% 전후의 초고율 관세를 매기기 시작하면 직수입 원가가 급격히 상승합니다."
...
export const WidgetIUUShadowIndex = () => {
  return (
    <div className={styles.insightCard} style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
      {/* Title Area */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0 }}>
        <Crosshair size={18} color="#8b5cf6" />
        <h3 style={{ margin: 0, fontSize: '1.2rem', color: '#f8fafc', fontWeight: 700 }}>[Macro] 일명 '섀도우 플릿'의 IUU 덤핑 타격</h3>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
        <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', background: 'rgba(0, 0, 0, 0.2)', padding: '16px' }}>
          <SafeResponsiveContainer width="100%" height={260} style={{ flexShrink: 0 }}>
            {/* Note: In recharts, we must use LineChart, ComposedChart, AreaChart correctly. We use ComposedChart since we have lines */ }
      ...
      <div style={{ flexShrink: 0, marginTop: '12px' }}>
        <TakeawayBox 
          ...
          actionPlan="일시적인 IUU 물량 덤핑으로 시중 평균 단가가 하락하더라도 이에 현혹되어선 안 됩니다. EU 규제 도입 시 이 물량들은 일순간 통관 압류되어 공급 사슬을 무너뜨리므로, 반드시 조업 위성망(VDS) 이력이 증명된 원물만 매입하십시오." 
        />
      </div>
    </div>
  );
...
export const WidgetElNinoImpact = () => (
  <div className={styles.insightCard} style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0 }}>
      ...
      <h3 style={{ margin: 0, fontSize: '1.2rem', color: '#f8fafc', fontWeight: 700 }}>[Macro] 엘니뇨 기후 지수와 흉어기 상관망</h3>
    </div>
    <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', background: 'rgba(0, 0, 0, 0.2)', padding: '16px' }}>
        <SafeResponsiveContainer width="100%" height={260} style={{ flexShrink: 0 }}>
    ...
    <div style={{ flexShrink: 0, marginTop: '12px' }}>
      <TakeawayBox 
        ...
        actionPlan="WMO ENSO 경보가 +1.5를 돌파하는 즉시 공격적인 원물 선도 매입 계약을 체결하고, 6개월 후행하는 가격 폭등에 대비하여 안전 재고를 최대로 비축하십시오." 
      />
    </div>
  </div>
);
...
export const WidgetCBPDetentionRisk = () => (
  <div className={styles.insightCard} style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0 }}>
      <ShieldAlert size={18} color="var(--color-danger)" />
      <h3 style={{ margin: 0, fontSize: '1.2rem', color: '#f8fafc', fontWeight: 700 }}>[Macro] 주요 원산지 제재 통관 억류 리스크 지수</h3>
    </div>
    <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', background: 'rgba(0, 0, 0, 0.2)', padding: '16px' }}>
        <SafeResponsiveContainer width="100%" height={260} style={{ flexShrink: 0 }}>
    ...
    <div style={{ flexShrink: 0, marginTop: '12px' }}>
      <TakeawayBox 
        ...
        actionPlan="안전마진을 고려하더라도 다롄 발 가공품 입항 비중을 즉각 30% 이하로 축소하고, 베트남 등 제3국 우회 라인으로 물류 체인을 리디렉션 하십시오." 
      />
    </div>
  </div>
);
...
export const WidgetEEZConflict = () => (
  <div className={styles.insightCard} style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0 }}>
      ...
      <h3 style={{ margin: 0, fontSize: '1.2rem', color: '#f8fafc', fontWeight: 700 }}>[Macro] 권역별 EEZ 조업 분쟁 나포 빈도 계기판</h3>
    </div>
    <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', background: 'rgba(0, 0, 0, 0.2)', padding: '16px' }}>
        <SafeResponsiveContainer width="100%" height={260} style={{ flexShrink: 0 }}>
    ...
    <div style={{ flexShrink: 0, marginTop: '12px' }}>
      <TakeawayBox 
        ...
        actionPlan="영해 인접 조업 시 자동식별장치(AIS) 조작이 단 1건이라도 적발될 경우 즉각적인 본선 압수로 이어집니다. 선단장에게 VDS 컴플라이언스 원격 모니터링을 강제하십시오." 
      />
    </div>
  </div>
);
...
export const WidgetFoodSecurityPremium = () => (
  <div className={styles.insightCard} style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0 }}>
      ...
      <h3 style={{ margin: 0, fontSize: '1.2rem', color: '#f8fafc', fontWeight: 700 }}>[Macro] 위기 시 식량안보 프리미엄 스프레드</h3>
    </div>
    <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', background: 'rgba(0, 0, 0, 0.2)', padding: '16px' }}>
        <SafeResponsiveContainer width="100%" height={260} style={{ flexShrink: 0 }}>
    ...
    <div style={{ flexShrink: 0, marginTop: '12px' }}>
      <TakeawayBox 
        ...
        actionPlan="곡물 식량 안보 지수(FFPI)가 전년 대비 15% 이상 상승하면, 명태 재고를 방출하지 말고 강하게 묵혀 분기 말 스팟(Spot) 스파이크에 전량 매도하십시오." 
      />
    </div>
  </div>
);
...
export const WidgetPortCongestion = () => (
  <div className={styles.insightCard} style={{ display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexShrink: 0 }}>
      ...
      <h3 style={{ margin: 0, fontSize: '1.2rem', color: '#f8fafc', fontWeight: 700 }}>[Macro] 주요 거점 항만 파업 및 체화(Congestion) 달력</h3>
    </div>
    <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
      <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', background: 'rgba(0, 0, 0, 0.2)', padding: '16px' }}>
        <SafeResponsiveContainer width="100%" height={260} style={{ flexShrink: 0 }}>
    ...
    <div style={{ flexShrink: 0, marginTop: '12px' }}>
      <TakeawayBox 
        ...
        actionPlan="지연일이 10일을 초과하는 항만으로 향하는 화물은 로딩 마스트 전 즉각 부산(BUS)이나 베트남항으로 양하(Discharge) 목적지를 스위칭 하십시오." 
      />
    </div>
  </div>
);
