...
const CT = ({active,payload,label}:any) => {
  ...
  return (<div className={styles.customTooltip}><p className={styles.tooltipLabel}>{label}</p>
    {payload.map((e:any,i:number)=>(<div key={i} className={styles.tooltipValue}><span style={{color:e.color}}>■ {e.name}</span><strong>{typeof e.value==='number'?e.value.toLocaleString():e.value}</strong></div>))}
  </div>);
};
const COLORS = ['#f43f5e','#3b82f6','#10b981','#eab308','#8b5cf6','#ec4899','#06b6d4','#f97316','#6366f1','#14b8a6'];

const W = ({title,icon:Icon,sub,accent,telemetry,sit,strat,source,children}:any) => (
  <div className={styles.glassCard} style={{display:'flex',flexDirection:'column',minHeight:'500px'}}>
    <div style={{marginBottom:'1rem',borderBottom:'1px solid rgba(255,255,255,0.05)',paddingBottom:'0.8rem',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
      <div><h3 style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.95rem',fontWeight:700,color:accent||'#f43f5e',margin:'0 0 0.4rem'}}><Icon size={18}/>{title}</h3>
        {sub&&<p style={{margin:0,fontSize:'0.8rem',color:'#94a3b8',lineHeight:1.5}}>{sub}</p>}
      </div>
      <div style={{display:'inline-flex',alignItems:'center',gap:'6px',background:'rgba(16,185,129,0.1)',border:'1px solid rgba(16,185,129,0.3)',padding:'2px 8px',borderRadius:'12px',fontSize:'0.65rem',fontWeight:600,color:'#34d399'}}>{telemetry}</div>
    </div>
    <div style={{height:'375px',width:'100%',marginBottom:'1rem'}}>
      <SafeResponsiveContainer width="100%" height="100%">{children}</SafeResponsiveContainer>
    </div>
    <div style={{marginTop:'auto'}}><TakeawayBox situation={sit} actionPlan={strat} source={source}/></div>
  </div>
);

export function W1_ASFCycle({accent}:any) {
  return <W title="글로벌 생산량 및 질병(ASF) 사이클" icon={Globe} accent={accent} sub="중국 중심 글로벌 돈육 생산량(천 톤) 및 산지 가격 지수" telemetry="FAOSTAT QCL | 2015-2024"
...
export function W2_FeedMargin({accent}:any) {
  return <W title="곡물가(사료) 연동 마진 압박 지수" icon={TrendingUp} accent={accent} sub="사료곡물 가격 지수 대비 가공 마진율(%) 추이" telemetry="STATIC | 2022-2023"
...
export function W3_TradeSpread({accent}:any) {
  return <W title="주요 대륙간 무역 단가 스프레드" icon={Globe} accent={accent} sub="EU, 북미, 아시아 간 수출입 돈육 평균 단가(달러/톤)" telemetry="OEC | 2023"
...
export function W5_Top10({accent}:any) {
  return <W title="글로벌 Top 10 생산국 점유율" icon={Target} accent={accent} sub="2022년 기준 국가별 돈육 생산량(천 톤) 및 글로벌 점유율" telemetry="FAOSTAT QCL | 2022"
...
export function W6_Trend({accent}:any) {
  return <W title="주요 8개국 생산량 10년 추이" icon={TrendingUp} accent={accent} sub="주요 생산국 10년간 돈육 생산량(천 톤) 시계열" telemetry="FAOSTAT QCL | 2015-2024"
...
export function W7_KoreaSupply({accent}:any) {
  return <W title="한국 돈육 수급 구조 분석" icon={ShoppingCart} accent={accent} sub="한국 국내 생산, 수입, 1인당 소비 시계열" telemetry="FAOSTAT QCL+FBS | 2015-2023"
    sit="한국 1인당 돈육 소비 10년간 30.9→41.4kg(+34%) 폭증. 생산 증가율(+20%)이 소비를 못따라감."
    strat="기존 수산물 콜드체인을 돈육까지 확장하는 '단백질 Total Solution' 전략으로 시장 주도권 확보." source="FAOSTAT FBS">
...
export function W8_ImportPartners({accent}:any) {
  return <W title="한국 수입 파트너 의존도" icon={Globe} accent={accent} sub="2022년 한국 돈육(boneless) 수입 파트너 국가별 물량" telemetry="FAOSTAT TM | 2022"
...
export function W9_ASFSeafood({accent}:any) {
  return <W title="ASF 발병 → 수산물 반사수혜 시뮬레이션" icon={Zap} accent={accent} sub="중국 돈육 생산량 급감 시 수산물 도매가 지수 반등 상관관계" telemetry="FAOSTAT QCL | 2017-2023"
...
export function W10_Portfolio({accent}:any) {
  return <W title="단백질 포트폴리오 최적 배분 비교" icon={Target} accent={accent} sub="돈육 vs 수산물 vs 가금류 — 5개 핵심 지표 비교" telemetry="FAO/USDA 종합 | 2024"
...
export function W11_SelfSufficiency({accent}:any) {
  return <W title="한국 단백질 자급률 갭" icon={Target} accent={accent} sub="한국 주요 단백질 품목별 자급률(%) vs 수입 의존도(%)" telemetry="FAOSTAT FBS + USDA PSD | 2022"
