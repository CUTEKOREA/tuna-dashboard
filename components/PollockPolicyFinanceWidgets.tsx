...
import { ComposedChart, Bar, Line, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar, Scatter, ReferenceLine } from 'recharts';
...
export function WidgetHSKBottleneck() {
  return (
    <WidgetCard
      ...
      takeaway={{
        source: "식약처 수입식품 방사능 검사 동향(공개) 및 HSK 세분화 규제 업계추정",
...
export function WidgetWarehouseSniping() {
  return (
    <WidgetCard
      ...
      takeaway={{
        source: "자체추정 — 명절·금어기 주기 패턴 기반 시나리오 (실입출고 데이터 미연동)",
        situation: "명절 직전이나 금어기 막바지에는 시장 수요가 치솟으며 도매가격이 상승하는 경향이 있습니다. 반면, 정부가 물가 통제를 위해 비축 명태 방출이나 한시적 할당관세를 시행할 경우 시장가가 일시적으로 하락할 수 있습니다.",
...
export function WidgetVDSIntegrity() {
  return (
    <WidgetCard
      ...
      takeaway={{
        source: "업계추정 — MFDS 수입신고 처리 기준 및 VDS 운영 관행 (공개 DB 미연동)",
        situation: "러시아산 및 알래스카산 원물 수입 시, 불법/비보고 조업 및 방사능 오염 우려로 인해 통관 세관의 현장 실사가 극도로 까다로워졌습니다. 사전에 전자적으로 증명된 VDS 무결성이 부족하면 전량 반송됩니다.",
        actionPlan: "구매 파트너 선정 시 VDS 서류 완결율이 낮은 러시아 선사는 발주 우선순위에서 후순위로 조정하십시오. 1차 선상 동결(Single-frozen) 후 원산지·방사능 검사 이력이 전자 문서로 즉시 제공되는 선단 중심으로 오더를 집중하면 통관 지연 리스크를 줄일 수 있습니다."
      }}
    />
  );
}