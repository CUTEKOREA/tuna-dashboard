...
/* ── EU 참치 수입 관세 "체제" — 24% 장벽과 무관세 우회로 (EU 1차) ──
   검증(solid): 캔 MFN 24%, 로인 ATQ 35,000t/년 0%(Reg 2023/2720), EVFTA 캔 TRQ 11,500t 0%,
   Pacific EPA 글로벌소싱 0%(원산지 불문), 한-EU FTA 0%(수산 98.7% 철폐).
...
const CHANNELS = [
  ...
  { ch: '한-EU FTA', rate: 0, note: '수산 98.7% 철폐', color: '#f59e0b' },
];
...
export default function SasEuTariffRegime() {
  return (
    <WidgetCard
      ...
      pillar="S3"
      telemetry={{ status: 'STATIC', syncDate: '2024-26' }}
      cardDesc="EU 참치 수입 채널별 실효 관세 — EU 이사회 Reg.2023/2720·TARIC·Access2Markets"
      takeaway={{
        situation: "EU의 참치 수입 관세는 단일 숫자가 아니라 '체제'입니다. 명목상 캔참치 MFN은 24%로 수산물 최고이지만, 실제 시장은 무관세 우회로로 갈립니다. 가공원료용 냉동 로인은 자율관세쿼터(ATQ, Reg 2023/2720)로 연 35,000톤까지 0%(중국·동남아가 연초 조기 소진), 베트남은 EVFTA로 캔 TRQ 11,500톤 0%, 태평양 PNG·피지는 EPA 글로벌소싱 데로게이션으로 어획지 불문 0%, 한국은 한-EU FTA(수산 98.7% 철폐)로 0%입니다. 즉 24%를 온전히 무는 공급자는 협정·쿼터 어디에도 못 든 일부(2015년 GSP 잃은 태국 등, SasThaiSourcing 참조)뿐입니다.",
        ...
        source: "EU 이사회 Reg.2023/2720(로인 ATQ 35,000t) / TARIC·Europarl E-002775/2023(MFN 24%) / Access2Markets(EVFTA TRQ 11,500t·한-EU FTA 98.7%) / CFFA(Pacific EPA 글로벌소싱)",
      }}
      customBody={
