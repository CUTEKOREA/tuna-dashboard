 ...
 *
 * 근거는 「문어 OEM 제안계획」 제2판(2026-09-03)이다. 이 추출본은 시장 절(02·05·06·07·07b)만
 * 담고, 08장(신라에스지)·09장(OEM 제안)은 사내 검토라 싣지 않는다. 대시보드가 이미 같은 머리행으로
 * 옮긴 표도 뺐으므로 여기 남는 것은 그 밖의 표뿐이다.
 *
 * 아키텍처 가드: 원본 JSON import 는 이 인테이크 모듈에서만 한다.
 */
import raw from './octopus-tables.json';

export interface OctopusReportTable {
...
interface Stage {
  tables: OctopusReportTable[];
}

const STAGES = raw as Record<string, Stage>;

/** 단계 키(`s01`~`s09`)로 그 단계에 배치된 보고서 표를 가져온다. */
export function getOctopusTables(stage: string): OctopusReportTable[] {
  return STAGES[stage]?.tables ?? [];
}

export function getOctopusTableStages(): string[] {
  return Object.keys(STAGES);
}

/** 검증용 — 발행본에서 옮긴 표 총수. */
export function getOctopusTableCount(): number {
  let n = 0;
