...
export default function SalmonInsightMarginSqueeze() {
  ...
  [가공] EU 훈제 연어 마진 압박과 가격 결정력 (Pricing Power) <span style={{ display:'inline-flex', alignItems:'center', gap:'3px', background:'var(--surface-2)', color:'var(--color-success)', fontSize:'0.66rem', fontWeight:600, padding:'2px 8px', borderRadius:'500px', letterSpacing:'0.2px', marginLeft:'6px', textTransform: 'uppercase' }}>LIVE API</span>
</h3>
          <p style={{ margin: '8px 0 0 0', fontSize: '0.88rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>EU 훈제 연어 생산량(천 톤) vs 단가(EUR/kg) 및 폴란드 점유율 추이</p>
        </div>
      ...
      <TakeawayBox 
        situation="EU 훈제 연어 단가가 2019년 15 EUR/kg에서 2023년 19 EUR/kg로 급등(Spike)했습니다. 그러나 사료, 에너지, 물류 비용의 동반 폭등으로 실질 마진은 심각한 압박을 받고 있습니다. 폴란드는 7만 7천 톤(EU 전체의 48%)을 독점 처리하며 밸류체인 내 병목점(Chokepoint)으로 작용하고 있습니다."
        actionPlan="**[Actionable Insight]** 경직된 장기 고정가 공급 계약(Long-term contract) 비중을 대폭 축소하고, 매입원가(COGS) 연동형 유연 계약(Index-linked pricing) 모델을 즉각 도입하여 비용 상승분을 바이어에게 전가(Pass-through)할 수 있는 가격 결정력을 확보해야 합니다. 동시에 폴란드 집중 리스크를 분산하기 위해 동유럽 가공 허브 다변화를 추진하여 잉여현금흐름(FCF)을 극대화하십시오. (Strong Buy)"
