...
# 상품명에 「고등어」가 들어가지만 고등어가 아닌 것.
#
# ⚠ 제외 근거는 학명이 아니라 **실측 프로파일**이다. 「갈고등어」가 어느 학명에 대응하는지는
#   어명 대조 자료로 확인하지 못했다(국내에서는 전갱이과 Decapterus 속으로 통용된다).
#   다만 물량이 고등어 세 등급 합계의 10배가 넘고 단가대도 전혀 달라, 학명 확인 여부와
#   무관하게 등급 집계에 넣을 수 없다. 넣으면 물량의 91%를 차지해 구조가 통째로 뒤집힌다.
EXCLUDE_NAMES = {"갈고등어"}
...
def read_wipan() -> tuple[list[dict], dict]:
    ...
    meta = {
        ...
        "제외": (
            f"「갈고등어」 위판 물량 {round(sum(excluded.values())):,}kg 을 뺐다. "
            "넣으면 물량의 91%를 차지해 등급 구조가 통째로 왜곡된다. "
            "이 상품명의 학명은 어명 대조 자료로 확인하지 못했다 — 제외 근거는 학명이 아니라 "
            "물량·단가 프로파일이 고등어와 전혀 다르다는 실측이다."
        ),
    }
    return rows, meta
