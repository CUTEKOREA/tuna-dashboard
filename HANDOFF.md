...
> 🧹 **2026-08-22 21:5x KST — `/whelk` 한글 서술 윤문(humanize-korean strict)** [Claude Code]:
> - **완료된 것**: `WhelkDashboard.tsx`·`whelk_v2.json`이 화면에 내보내는 한글 문자열 174건을 뽑아 경로·코드·라벨을 걸러낸 **산문 80건 8,164자**에 strict 5인 파이프라인(탐지→윤문→무손실감사)을 돌렸다. 변경률 4.8%, 46/80 세그먼트.
> - 결측 안내가 한 틀로 10회 반복되던 블록을 갈랐고, 게이트 룰 슬롯 밖으로 번진 명사형 종결 13곳을 평서체로 통일했다. **화면 em대시 49→0**(남은 4회는 코드 주석).
> - 게이트 룰 문구는 6회·4회·2회·2회 중복이 곧 단일 출처라 **원형을 유지**했다. 각각 다르게 쓰면 출처가 갈린다.
> - **무손실 감사가 창작 11건을 잡아 되돌렸다.** 원문의 결측 표현은 「보유하지 않음」·「없음」 둘뿐인데 윤문이 「확보하지 못했다」(시도·실패 이력)와 「아카이브에 없다」(부정 주체 이동)를 만들어 냈다. 한정어 삭제로 사실이 굳은 3건(`시작됨`·`직접적 원인`·`독과점`)도 복원했다.
> - **검증**: 수치·HS 세번·학명·`${}` 템플릿 토큰 다중집합 문자 단위 일치, SSOT 갈림 0건, 빌드 성공(98/98), 골뱅이 Vitest **37/37**, JSON 파싱 OK.
> - 옛 문구를 문자열로 잠근 가드 테스트 5곳은 새 문구로 갱신했다. 동작은 그대로다.
> - **다음 단계**: 자연스러움 리뷰어 재탐지 결과 회수. 감사 경고 7건(특히 `'상승'하지만`→`올라 보이지만` 4중복)은 판단이 갈려 보류했다.
...
> ✅ **2026-08-13 23:39 KST — 골뱅이 v2 종 해상도 고지 구현 (PHASE 4b)** [Codex]:
> - `S1_global_capture_top_countries` 국가 행마다 FishStat 2024 상세행 기반 `species_composition`·지배 학명·종 해상도 여부를 산출했다. `Gastropods NEI` 종 코드를 지배 종군으로 쓰는 멕시코·한국·러시아는 미상으로 표시하고, Buccinum 속만 재집계한 상위 5개국(영국·프랑스·캐나다·아일랜드·노르웨이)을 `buccinum_only_ranking`에 추가했다. 한국은 Buccinum 0톤으로 순위에 없다.
> - 글로벌 어획 카드에 종 해상도 고지를 고정했다. 한국은 전량 `Gastropoda NEI`, 영국은 `Buccinum undatum`, 튀르키예는 다른 종인 `Rapana venosa`임을 같은 카드에서 데이터 기반으로 표시하며, 종 미상 행은 대각선 패턴과 경고 배지로 구분한다. 종 구성 계약이 누락되면 차트·`한국 3위` 단언을 닫고 오류 고지를 표시한다.
> - 신규 Vitest 회귀 5건과 빌더 assert 1건을 RED→GREEN으로 추가했다. 종 고지 제거, 종 구성 없는 `한국 3위` 단독 노출, Buccinum 순위의 한국 오포함, 하드코딩된 종별 톤수·비중, 종 미상 시각 구분 소실을 방어한다.
> - 게이트 7종 통과: `tsc` 0, Vitest **138/138 (28 파일)**, Next.js build 103페이지, 빌더 widgets=11·empty=0, 빌더 assert **22/22**, 최종 검증 위반 0, S-Grade closure 8개·GS 톤 위반 0·가짜 LIVE 0. 기존 앵커(한국 9,669.783 / 영국 16,511.020 / 세계 90,114.065 / 2024 수입 58,504,760 / 2024.1~5 20,952,354 / 2026.1~5 12,531,808) 유지. Google Drive는 읽기 전용으로 사용했고 커밋·배포하지 않았다. 다음 단계: 없음.

> ✅ **2026-08-13 23:25 KST — 골뱅이 v2 3차 정정 게이트 전수 재현 + 회귀 방어 (PHASE 4)** [CC]:
> - **게이트 7종 전수 통과.** `npx tsc --noEmit` 0, `npx vitest run` **133/133 (27 파일)**, `npm run build` exit 0(정적 페이지 103개), `python3 -m scripts.whelk_build` widgets=11 empty_cards=0, `scripts/whelk_build/tests/test_whelk_build.py` 21/21, `scripts/validate_whelk_v2.py` 위젯 11개·빈 카드 0·게이트 위반 0, `scripts/check_s_grade.py WhelkDashboard.tsx` closure 8개 파일(공허 통과 아님)·GS 톤 위반 0·가짜 LIVE 0. 수치나 단언을 완화한 곳 없음.
> - **3차 정정 내역(누적).** ① G-006 — HS6(1605.59) 하나가 원산지 구성이 겹치지 않는 HSK8 두 바구니(16055910 북해축 / 16055990 흑해축)의 합이므로 N1 원산지 위젯을 바구니별로 분해. 합산 분모에서 나오던 영국 47.2%·34.6% 서술을 제거하고 바구니 내 점유율(영국+아일랜드 97.3%→75.8%)만 서술. ② 캐나다 19.9%는 2개월·선적 2건 관측이라 표본 희박 주석을 같은 카드에 고정하고, 캐나다 제외 시 94.65%를 병기. ③ 단가 순위에 바구니별 물량 1% 하한을 적용해 미달 9개는 행을 보존한 채 순위에서 분리. ④ N10 냉동 축(0307.92) 신설 — 조제 대비 2.65배는 조개관자(0307921000, 10.3%) 제외 시 2.38배로 정정하고 HSK10 품명 분해를 노출. ⑤ 실측 근거가 없는 위젯 10종을 `WhelkHypothesisSection` 접힘 영역으로 격리(STATIC 고정), 폐기 대상 HS6 위젯 2종은 소스에 남기되 런타임 게이트로 차단.
> - **신규 회귀 테스트** `__tests__/whelk-v2-regression.test.ts` 17건 추가. G-006(바구니 분해·바구니 내 합 100%·HS6 분모 점유율 부재·`dataKey` 필드 고정), N10(배수를 원자료 나눗셈과 대조·계약 변형 시 화면 추종), 가설 10종 telemetry STATIC 고정(`status: metaStatus` 연결 금지 포함), 기본 화면 위젯 수(38 − 폐기 2 − 가설 10 = **26**) 검증. **뮤테이션 5종으로 RED 확인** — HS6 축 회귀·가설 SYNCED 승격·배수 하드코딩·cardDesc HS6 점유율 재유입·위젯 수 드리프트 모두 정확히 해당 테스트 1건만 실패시킴.
> - **잔여 리스크 (미해소).** ① **2025 공백** — 빌더 아카이브 보유 구간은 2023·2024·2026.01~05뿐이라 2024→2026 비교는 중간 경로를 보지 못한 구간 점프다. 전환 시점·속도를 특정할 수 없고, 화면에는 `WhelkCoverageGapNote`로 고지 중. ② **튀르키예 형태 전환 미확정** — 조제(1605.59) 이탈과 냉동(0307.92) 병존은 정황일 뿐이며, 아카이브에 0307.92 2024 원본 XML이 없어 기준선 자체가 없다. `data-whelk-hypothesis-block`으로 미검증 가설 표기 유지. ③ 관측 창 편향 — 같은 2024년도 1~5월 창(북해축 49.5%)과 연간 창(68.5%)에서 결론이 갈리므로 창 선택을 명시하지 않은 인용은 금지.
> - 커밋·배포하지 않음. 변경 파일: `__tests__/whelk-v2-regression.test.ts`(신규), `HANDOFF.md`. 다음 단계: 2025 KCS 원자료와 0307.92 2024 원본을 아카이브에 반영해 ①②를 가설에서 실측으로 승격.

> ✅ **2026-08-13 16:11 KST — 골뱅이 v2 P3 신규 위젯 N1~N9 완료** [Codex]:
> - `/api/whelk/live`를 v1 루트 계약과 `v2` 원문 계약의 병합 응답으로 전환하고 `isLive=false`를 유지했다. `WhelkDashboard`에는 v2 API만 소비하는 N1~N9 9종을 추가했으며, 기존 `WhelkFTAQuarterly`가 담당하는 10번째 위젯은 중복 생성하지 않았다.
> - 2022 순위·상위국 위젯 2종은 N4(FAO 2024 한국 3위), 한국 연안 시계열은 N5(2023년 10,757t 최고점), 양식 영구 불가 위젯은 N7 종별 분해로 대체했다. N3는 표본 하한 통과 5개와 미달 9개 원산지를 시각적으로 분리하고 미달 행을 모두 보존했다.
> - 신규 9종과 FTA 위젯의 telemetry를 `SYNCED`로 통일하되 빌드 시각이 아닌 원데이터 빈티지(`FAO FishStat 2024`, `관세청 2026.01~05`, `KMI 2026 Q1` 등)를 `syncDate`에 사용했다. `section`을 `pillar`에 그대로 전달하고 모든 설명 수치는 API 데이터에서 계산한다.
> - TDD RED 후 P3 대상 회귀 테스트 4/4 GREEN. `npx tsc --noEmit`, 기본 Turbopack `npm run build`(103개 정적 페이지), 요청된 S-Grade 명령을 통과했다. 제한 샌드박스의 포트·Google Fonts 차단 때문에 빌드 중에만 worker thread와 오프라인 폰트 조건을 적용했고 관련 전역 파일은 원상복구했다.
> - P4 격리·P2 산출물·아카이브는 건드리지 않았고 커밋·배포하지 않았다. 다음 단계: 없음. 이번 P3에서 중단.

> ✅ **2026-08-13 15:20 KST — 골뱅이 v2 P2 단가 표본 하한 정정** [Codex]:
> - `S3_origin_cif_ladder`에 기간 총 수입중량 1% 하한을 적용. 2026년 1~5월은 11,518.92kg 이상 5개 원산지만 단가순 `rank`를 부여하고 미달 9개는 행을 보존한 채 `rank=null`, `below_volume_floor=true`로 분리했다.
> - 2024 비교 단가에도 총 6,215,357kg의 1%(62,153.57kg)를 적용하고 기간별 중량 비중·미달 상태를 산출. G-004 `단가 표본`과 KCS 근거 경로를 거버넌스·위젯 restriction·독립 검증기에 연결했다.
> - 빌더 10위젯/빈 카드 0, 자체검사 11/11, 최종 게이트 위반 0. 기존 수입·어획 앵커는 유지했고 P3 TSX·`app/api`·아카이브는 수정하지 않았다. 사용자 지시대로 커밋·배포하지 않음.
> - 다음 단계: 없음. 이번 P2 정정만 반영하고 중단.

> ✅ **2026-08-13 15:07 KST — 골뱅이 v2 P2 아카이브 빌더 완료** [Codex]:
> - `scripts/whelk_build/`에 오징어 빌더 구조를 이식해 FishStat·KCS CSV/XML·KMI·HS 매핑 추출기, 파생 위젯, 10개 위젯 spec/config, sources·G-001~G-003·monitoring 계약을 구현. Drive 아카이브는 읽기 전용으로만 사용했다.
> - `public/data/whelk_v2.json` 생성: 위젯 10개, 빈 카드 0개, `meta.telemetry=SYNCED`. 앵커 2024 수입 58,504,760 / 2026년 1~5월 12,531,808 / 한국 9,669.783t / 영국 16,511.020t / 세계 90,114.065t 일치.
> - 검증: 빌더 실행 성공, assert 기반 자체검사 10/10, 검증기 self-test 12건, 최종 게이트 위반 0. P3 위젯·P4 정리·P6 작업은 하지 않았고 기존 P1 `app/api` 변경도 건드리지 않았다.
> - 다음 단계: 별도 지시에 따라 P3/P4를 진행하되 이번 P2 산출 계약을 소비한다. 사용자 지시대로 커밋·배포하지 않음.
